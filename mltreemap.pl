...
sub read_user_input {    
    ...
    my %option_defaults = (-b => 0, -f => "v", -o => "output/", -m => 0, -t => "p", -s => 60, -g => 50, -c => 0, -l => 2000);
...
sub read_cog_file {
    ...
    open (COGIN, "data/tree_data/cog_list.txt") or die "ERROR: can't open data/tree_data/cog_list.txt!\n";
    my $kind_of_cog = "";
    while (<COGIN>) {
        chomp $_;
        if (/\A#(.+)/) {
...
sub get_non_wag_cogs {
    my $package = shift;
    my %non_wag_cog_list = ();
    open (COGIN, "data/tree_data/non_wag_cogs.txt") or die "ERROR: can't open data/tree_data/non_wag_cogs.txt!\n";
    my $denominator = "";
    while (<COGIN>) {
        chomp $_;
        if (/\A#(.+)/) {
...
sub read_species_translation_files {
    ...
    foreach my $denominator (sort {$a cmp $b} keys %translation_files) {
        ...
        open (IN, "$filename") or die "ERROR: Can't open $filename!\n";
        while (<IN>) {
            chomp $_;
...
sub blast {
    ...
    my $alignment_data_dir = "data/$reference_data_prefix"."alignment_data/";
    ...
    foreach my $blast_input_file (sort {$a cmp $b} keys %$blast_input_files) {      
        foreach my $blast_type (@blast_types) {
            ...
            $database .= "\"";
            my $blast_command = "sub_binaries/blastall -p $blast_type -i $blast_input_file -M BLOSUM62 ";
...
sub start_genewise {   
    ...
    foreach my $shortened_sequence_file (sort {$a cmp $b} keys %$shortened_sequence_files) {
        ...
        foreach my $identifier (sort {$a cmp $b} keys %{$$blast_hits_purified{$contig}}) {            
            ...
            my $genewise_command = "sub_binaries/genewise data/$reference_data_prefix"."hmm_data/$cog.hmm $shortened_sequence_file ";
            $genewise_command .= " -init local -quiet -gene data/genewise_support_files/human.gf -matrix data/genewise_support_files/blosum62.bla";
            $genewise_command .= " -codon data/genewise_support_files/codon.table -hmmer -subs 0.01 -indel 0.01 -gap 11 -ext 1 -both -pep -sum > ";
    ...
    foreach my $contig (sort {$a cmp $b} keys %$genewise_summary_files) {
        foreach my $genewise_summary_file (sort {$a cmp $b} keys %{$$genewise_summary_files{$contig}}) {
            ...
            while (<IN>) {
                ...
                $hmmalign_command = "sub_binaries/hmmalign -m --mapali data/$reference_data_prefix"."alignment_data/$cog.fa ";
                $hmmalign_command .= "--outformat Clustal data/$reference_data_prefix"."hmm_data/$cog.hmm ";
...
sub start_gblocks {
    ...
    foreach my $f_contig (sort {$a cmp $b} keys %$concatenated_mfa_files) {
        ...
        my $gblocks_command = "sub_binaries/Gblocks ";
...
sub start_RAxML {
    ...
    foreach my $f_contig (sort {$a cmp $b} keys %$phy_files) {
        my $reference_tree_file = "data/tree_data/$$user_options{\"reference_tree\"}";
        ...
        my $raxml_command = "sub_binaries/raxmlHPC -m $model_to_be_used ";
