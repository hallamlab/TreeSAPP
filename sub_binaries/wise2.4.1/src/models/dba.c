...
#define DnaMatchBlock_SHATTER_SPECIAL(matrix,i,j,STATE) matrix->shatter->special[STATE][j]   
#define DnaMatchBlock_SHATTER_MATRIX(matrix,i,j,STATE)  fetch_cell_value_ShatterMatrix(mat->shatter,i,j,STATE)   
 ...
 *
 */
PackAln * PackAln_read_Shatter_DnaMatchBlock(DnaMatchBlock * mat) 
{
    DnaMatchBlock_access_func_holder holder;     


    holder.access_main    = DnaMatchBlock_shatter_access_main;   
    holder.access_special = DnaMatchBlock_shatter_access_special;    
    assert(mat);     
    assert(mat->shatter);    
    return PackAln_read_generic_DnaMatchBlock(mat,holder);   
}    


/* Function:  DnaMatchBlock_shatter_access_main(mat,i,j,state)
 *
 ...
 *
 */
int DnaMatchBlock_shatter_access_main(DnaMatchBlock * mat,int i,int j,int state) 
{
    return DnaMatchBlock_SHATTER_MATRIX(mat,i,j,state);  
}    


/* Function:  DnaMatchBlock_shatter_access_special(mat,i,j,state)
 *
 * Descrip: No Description
 *
 * Arg:          mat [UNKN ] Undocumented argument [DnaMatchBlock *]
 ...
 *
 */
int DnaMatchBlock_shatter_access_special(DnaMatchBlock * mat,int i,int j,int state) 
{
    return DnaMatchBlock_SHATTER_SPECIAL(mat,i,j,state); 
}    
 ...
 *
 */
boolean calculate_shatter_DnaMatchBlock(DnaMatchBlock * mat,DPEnvelope * dpenv) 
{
    int i;   
    int j;   
    int k;   
    ...
    for(j=startj;j<endj;j++) {  
      ...
      for(i=starti;i<endi;i++)   {  
        ...
        if( (i-1) == 0 && (j-0) == 0 )   {  
          ...
          temp = DnaMatchBlock_SHATTER_SPECIAL(mat,i-1,j-0,START) + 0;   
          if( temp  > score )    {  
            score = temp;    
            }  
          }  
        ...
        if( i == mat->leni-1 && j == mat->lenj-1 )   {  
          temp = score + (0) + (0) ;     
          if( temp > DnaMatchBlock_SHATTER_SPECIAL(mat,i,j,END) )    {  
            DnaMatchBlock_SHATTER_SPECIAL(mat,i,j,END) = temp;   
            }  
...
Search_Return_Type search_DnaMatchBlock(DBSearchImpl * dbsi,Hscore * out,ComplexSequence* query,ComplexSequence* target ,DnaMatrix* comp65,DnaMatrix* comp75,DnaMatrix* comp85,DnaMatrix* comp95,Score g,Score u,Score v,Score s,Score b) 
{
    ...
    if( dbsi->trace_level > 5 )  
      warn("Asking for trace level of %d in database search for DnaMatchBlock, but it was compiled with a trace level of -2139062144. Not all trace statements can be shown",dbsi->trace_level); 
...
PackAln * PackAln_bestmemory_DnaMatchBlock(ComplexSequence* query,ComplexSequence* target ,DnaMatrix* comp65,DnaMatrix* comp75,DnaMatrix* comp85,DnaMatrix* comp95,Score g,Score u,Score v,Score s,Score b,DPEnvelope * dpenv,DPRunImpl * dpri) 
{
    ...
    if( dpenv != NULL )  {  
      if( strategy == DPIM_Explicit) {  
        if( (mat=allocate_Expl_DnaMatchBlock(query, target , comp65, comp75, comp85, comp95, g, u, v, s, b,dpri)) == NULL )  {  
          warn("Unable to allocate large DnaMatchBlock version");    
          return NULL;   
          }  
        calculate_dpenv_DnaMatchBlock(mat,dpenv);    
        out =  PackAln_read_Expl_DnaMatchBlock(mat); 
        }  
      else   {  
        mat = allocate_DnaMatchBlock_only(query, target , comp65, comp75, comp85, comp95, g, u, v, s, b);    
        calculate_shatter_DnaMatchBlock(mat,dpenv);  
        out = PackAln_read_Shatter_DnaMatchBlock(mat);   
        }  
      }  
    else {  
 ...
 *
 */
DnaMatchBlock * allocate_Expl_DnaMatchBlock(ComplexSequence* query,ComplexSequence* target ,DnaMatrix* comp65,DnaMatrix* comp75,DnaMatrix* comp85,DnaMatrix* comp95,Score g,Score u,Score v,Score s,Score b,DPRunImpl * dpri) 
{
    DnaMatchBlock * out; 
...
#define DnaMatchBlock_DC_OPT_SHADOW_MATRIX(thismatrix,i,j,state) (score_pointers[(((j+1)% 1) * (leni+1) * 6) + ((i+1) * 6) + (state)])   
#define DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(thismatrix,i,j,state,shadow) (shadow_pointers[(((j+1)% 1) * (leni+1) * 48) + ((i+1) * 48) + (state * 8) + shadow+1])   
#define DnaMatchBlock_DC_OPT_SHADOW_SPECIAL(thismatrix,i,j,state) (thismatrix->basematrix->specmatrix[state*8][(j+1)])   
 ...
 *
 */
#define DnaMatchBlock_DC_OPT_SHADOW_SPECIAL_SP(thismatrix,i,j,state,shadow) (thismatrix->basematrix->specmatrix[state*8 +shadow+1][(j+1)])   
...
PackAln * PackAln_calculate_Small_DnaMatchBlock(DnaMatchBlock * mat,DPEnvelope * dpenv) 
{
    ...
    dc_optimised_start_end_calc_DnaMatchBlock(mat,dpenv);    
...
AlnRangeSet * AlnRangeSet_calculate_Small_DnaMatchBlock(DnaMatchBlock * mat) 
{
    ...
    start_reporting("Find start end points: ");  
    dc_optimised_start_end_calc_DnaMatchBlock(mat,NULL); 
...
/* Function:  dc_optimised_start_end_calc_DnaMatchBlock(*mat,dpenv)
 *
 ...
 *
 */
boolean dc_optimised_start_end_calc_DnaMatchBlock(DnaMatchBlock *mat,DPEnvelope * dpenv) 
{
    int i;   
    int j;   
    int k;   
    ...
    for(j=0;j<lenj;j++)  { /*for each j strip*/ 
      for(i=0;i<leni;i++)    { /*for each i position in strip*/ 
        num++;   
        if( dpenv != NULL && is_in_DPEnvelope(dpenv,i,j) == FALSE )  { /*Is not in envelope*/ 
          DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i,j,MATCH65) = NEGI;    
          DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i,j,MATCH75) = NEGI;    
          DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i,j,MATCH85) = NEGI;    
          DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i,j,MATCH95) = NEGI;    
          DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i,j,UNMATCHED_QUERY) = NEGI;    
          DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i,j,UNMATCHED_TARGET) = NEGI;   
          continue;  
          } /* end of Is not in envelope */ 
        if( num%1000 == 0)   
        ...
        /* For state MATCH65 */ 
        /* setting first movement to score */ 
        score = DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i-1,j-1,MATCH65) + (mat->comp65->score[CSEQ_DNA_BASE(mat->query,i)][CSEQ_DNA_BASE(mat->target,j)]+mat->s) + (0);  
        /* assign local shadown pointer */ 
        localsp = &(DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i - 1,j - 1,MATCH65,0));   
        /* From state MATCH65 to state MATCH65 */ 
        temp = DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i-0,j-1,MATCH65) + mat->g +(0);    
        if( temp  > score )  {  
          score = temp;  
          /* assign local shadown pointer */ 
          localsp = &(DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i - 0,j - 1,MATCH65,0)); 
          }  
        /* From state MATCH65 to state MATCH65 */ 
        temp = DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i-1,j-0,MATCH65) + mat->g +(0);    
        if( temp  > score )  {  
          score = temp;  
          /* assign local shadown pointer */ 
          localsp = &(DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i - 1,j - 0,MATCH65,0)); 
          }  
        /* From state UNMATCHED_TARGET to state MATCH65 */ 
        temp = DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i-1,j-1,UNMATCHED_TARGET) + (mat->comp65->score[CSEQ_DNA_BASE(mat->query,i)][CSEQ_DNA_BASE(mat->target,j)]+mat->v) +(0);   
        if( temp  > score )  {  
          score = temp;  
          /* assign local shadown pointer */ 
          localsp = &(DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i - 1,j - 1,UNMATCHED_TARGET,0));    
          }  
        ...
        /* Actually, already done inside scores */ 
         DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i,j,MATCH65) = score;    
        for(k=0;k<7;k++) 
          DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i,j,MATCH65,k) = localsp[k]; 
        /* Now figure out if any specials need this score */ 
        ...
        /* For state MATCH75 */ 
        /* setting first movement to score */ 
        score = DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i-1,j-1,MATCH75) + (mat->comp75->score[CSEQ_DNA_BASE(mat->query,i)][CSEQ_DNA_BASE(mat->target,j)]+mat->s) + (0);  
        /* assign local shadown pointer */ 
        localsp = &(DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i - 1,j - 1,MATCH75,0));   
        /* From state MATCH75 to state MATCH75 */ 
        temp = DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i-0,j-1,MATCH75) + mat->g +(0);    
        if( temp  > score )  {  
          score = temp;  
          /* assign local shadown pointer */ 
          localsp = &(DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i - 0,j - 1,MATCH75,0)); 
          }  
        /* From state MATCH75 to state MATCH75 */ 
        temp = DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i-1,j-0,MATCH75) + mat->g +(0);    
        if( temp  > score )  {  
          score = temp;  
          /* assign local shadown pointer */ 
          localsp = &(DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i - 1,j - 0,MATCH75,0)); 
          }  
        /* From state UNMATCHED_TARGET to state MATCH75 */ 
        temp = DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i-1,j-1,UNMATCHED_TARGET) + (mat->comp75->score[CSEQ_DNA_BASE(mat->query,i)][CSEQ_DNA_BASE(mat->target,j)]+mat->v) +(0);   
        if( temp  > score )  {  
          score = temp;  
          /* assign local shadown pointer */ 
          localsp = &(DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i - 1,j - 1,UNMATCHED_TARGET,0));    
        ...
        /* Add any movement independant score and put away */ 
        /* Actually, already done inside scores */ 
         DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i,j,MATCH75) = score;    
        for(k=0;k<7;k++) 
          DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i,j,MATCH75,k) = localsp[k]; 
        /* Now figure out if any specials need this score */ 
        ...
        /* For state MATCH85 */ 
        /* setting first movement to score */ 
        score = DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i-1,j-1,MATCH85) + (mat->comp85->score[CSEQ_DNA_BASE(mat->query,i)][CSEQ_DNA_BASE(mat->target,j)]+mat->s) + (0);  
        /* assign local shadown pointer */ 
        localsp = &(DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i - 1,j - 1,MATCH85,0));   
        /* From state MATCH85 to state MATCH85 */ 
        temp = DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i-0,j-1,MATCH85) + mat->g +(0);    
        if( temp  > score )  {  
          score = temp;  
          /* assign local shadown pointer */ 
          localsp = &(DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i - 0,j - 1,MATCH85,0)); 
          }  
        /* From state MATCH85 to state MATCH85 */ 
        temp = DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i-1,j-0,MATCH85) + mat->g +(0);    
        if( temp  > score )  {  
          score = temp;  
          /* assign local shadown pointer */ 
          localsp = &(DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i - 1,j - 0,MATCH85,0)); 
          }  
        /* From state UNMATCHED_TARGET to state MATCH85 */ 
        temp = DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i-1,j-1,UNMATCHED_TARGET) + (mat->comp85->score[CSEQ_DNA_BASE(mat->query,i)][CSEQ_DNA_BASE(mat->target,j)]+mat->v) +(0);   
        if( temp  > score )  {  
          score = temp;  
          /* assign local shadown pointer */ 
          localsp = &(DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i - 1,j - 1,UNMATCHED_TARGET,0));    
        ...
        /* Add any movement independant score and put away */ 
        /* Actually, already done inside scores */ 
         DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i,j,MATCH85) = score;    
        for(k=0;k<7;k++) 
          DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i,j,MATCH85,k) = localsp[k]; 
        /* Now figure out if any specials need this score */ 
        ...
        /* For state MATCH95 */ 
        /* setting first movement to score */ 
        score = DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i-1,j-1,MATCH95) + (mat->comp95->score[CSEQ_DNA_BASE(mat->query,i)][CSEQ_DNA_BASE(mat->target,j)]+mat->s) + (0);  
        /* assign local shadown pointer */ 
        localsp = &(DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i - 1,j - 1,MATCH95,0));   
        /* From state MATCH95 to state MATCH95 */ 
        temp = DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i-0,j-1,MATCH95) + mat->g +(0);    
        if( temp  > score )  {  
          score = temp;  
          /* assign local shadown pointer */ 
          localsp = &(DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i - 0,j - 1,MATCH95,0)); 
          }  
        /* From state MATCH95 to state MATCH95 */ 
        temp = DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i-1,j-0,MATCH95) + mat->g +(0);    
        if( temp  > score )  {  
          score = temp;  
          /* assign local shadown pointer */ 
          localsp = &(DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i - 1,j - 0,MATCH95,0)); 
          }  
        /* From state UNMATCHED_TARGET to state MATCH95 */ 
        temp = DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i-1,j-1,UNMATCHED_TARGET) + (mat->comp95->score[CSEQ_DNA_BASE(mat->query,i)][CSEQ_DNA_BASE(mat->target,j)]+mat->v) +(0);   
        if( temp  > score )  {  
          score = temp;  
          /* assign local shadown pointer */ 
          localsp = &(DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i - 1,j - 1,UNMATCHED_TARGET,0));    
        ...
        /* Add any movement independant score and put away */ 
        /* Actually, already done inside scores */ 
         DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i,j,MATCH95) = score;    
        for(k=0;k<7;k++) 
          DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i,j,MATCH95,k) = localsp[k]; 
        /* Now figure out if any specials need this score */ 
        ...
        /* For state UNMATCHED_QUERY */ 
        /* setting first movement to score */ 
        score = DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i-1,j-0,MATCH65) + mat->b + (0);  
        /* assign local shadown pointer */ 
        localsp = &(DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i - 1,j - 0,MATCH65,0));   
        /* From state MATCH75 to state UNMATCHED_QUERY */ 
        temp = DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i-1,j-0,MATCH75) + mat->b +(0);    
        if( temp  > score )  {  
          score = temp;  
          /* assign local shadown pointer */ 
          localsp = &(DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i - 1,j - 0,MATCH75,0)); 
        ...
        /* From state MATCH85 to state UNMATCHED_QUERY */ 
        temp = DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i-1,j-0,MATCH85) + mat->b +(0);    
        if( temp  > score )  {  
          score = temp;  
          /* assign local shadown pointer */ 
          localsp = &(DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i - 1,j - 0,MATCH85,0)); 
        ...
        /* From state MATCH95 to state UNMATCHED_QUERY */ 
        temp = DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i-1,j-0,MATCH95) + mat->b +(0);    
        if( temp  > score )  {  
          score = temp;  
          /* assign local shadown pointer */ 
          localsp = &(DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i - 1,j - 0,MATCH95,0)); 
        ...
        /* From state UNMATCHED_QUERY to state UNMATCHED_QUERY */ 
        temp = DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i-1,j-0,UNMATCHED_QUERY) + mat->u +(0);    
        if( temp  > score )  {  
          score = temp;  
          /* assign local shadown pointer */ 
          localsp = &(DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i - 1,j - 0,UNMATCHED_QUERY,0)); 
          }  
        /* From state START to state UNMATCHED_QUERY */ 
        temp = DnaMatchBlock_DC_OPT_SHADOW_SPECIAL(mat,i-1,j-0,START) + 0 + (0);     
        if( temp  > score )  {  
          score = temp;  
        ...
        /* Add any movement independant score and put away */ 
        /* Actually, already done inside scores */ 
         DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i,j,UNMATCHED_QUERY) = score;    
        for(k=0;k<7;k++) 
          DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i,j,UNMATCHED_QUERY,k) = localsp[k]; 
        /* Now figure out if any specials need this score */ 
        ...
        /* For state UNMATCHED_TARGET */ 
        /* setting first movement to score */ 
        score = DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i-0,j-1,UNMATCHED_QUERY) + mat->v + (0);  
        /* assign local shadown pointer */ 
        localsp = &(DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i - 0,j - 1,UNMATCHED_QUERY,0));   
        /* From state UNMATCHED_TARGET to state UNMATCHED_TARGET */ 
        temp = DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i-0,j-1,UNMATCHED_TARGET) + mat->u +(0);   
        if( temp  > score )  {  
          score = temp;  
          /* assign local shadown pointer */ 
          localsp = &(DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i - 0,j - 1,UNMATCHED_TARGET,0));    
          }  
        ...
        /* Add any movement independant score and put away */ 
        /* Actually, already done inside scores */ 
         DnaMatchBlock_DC_OPT_SHADOW_MATRIX(mat,i,j,UNMATCHED_TARGET) = score;   
        for(k=0;k<7;k++) 
          DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i,j,UNMATCHED_TARGET,k) = localsp[k];    
        /* Now figure out if any specials need this score */ 


        /* state UNMATCHED_TARGET is a source for special END */ 
        temp = score + (0) + (0) ;   
        if( temp > DnaMatchBlock_DC_OPT_SHADOW_SPECIAL(mat,i,j,END) )    {  
          DnaMatchBlock_DC_OPT_SHADOW_SPECIAL(mat,i,j,END) = temp;   
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            DnaMatchBlock_DC_OPT_SHADOW_SPECIAL_SP(mat,i,j,END,k) = DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i,j,UNMATCHED_TARGET,k);   
          DnaMatchBlock_DC_OPT_SHADOW_SPECIAL_SP(mat,i,j,END,6) = DnaMatchBlock_DC_OPT_SHADOW_MATRIX_SP(mat,i,j,UNMATCHED_TARGET,6); 
          DnaMatchBlock_DC_OPT_SHADOW_SPECIAL_SP(mat,i,j,END,3) = i; 
          DnaMatchBlock_DC_OPT_SHADOW_SPECIAL_SP(mat,i,j,END,4) = j; 
          DnaMatchBlock_DC_OPT_SHADOW_SPECIAL_SP(mat,i,j,END,5) = UNMATCHED_TARGET;  
          }  
...
PackAln * PackAln_read_Expl_DnaMatchBlock(DnaMatchBlock * mat) 
{
    ...
    holder.access_main    = DnaMatchBlock_explicit_access_main;  
    holder.access_special = DnaMatchBlock_explicit_access_special;   
    return PackAln_read_generic_DnaMatchBlock(mat,holder);   
}    


/* Function:  DnaMatchBlock_explicit_access_main(mat,i,j,state)
 *
 * Descrip: No Description
 *
 * Arg:          mat [UNKN ] Undocumented argument [DnaMatchBlock *]
 ...
 *
 */
int DnaMatchBlock_explicit_access_main(DnaMatchBlock * mat,int i,int j,int state) 
{
    return DnaMatchBlock_EXPL_MATRIX(mat,i,j,state); 
}    


/* Function:  DnaMatchBlock_explicit_access_special(mat,i,j,state)
 *
 * Descrip: No Description
 *
 * Arg:          mat [UNKN ] Undocumented argument [DnaMatchBlock *]
 ...
 *
 */
int DnaMatchBlock_explicit_access_special(DnaMatchBlock * mat,int i,int j,int state) 
{
    return DnaMatchBlock_EXPL_SPECIAL(mat,i,j,state);    
}    
 ...
 *
 */
PackAln * PackAln_read_generic_DnaMatchBlock(DnaMatchBlock * mat,DnaMatchBlock_access_func_holder h) 
{
    ...
    out->score =  find_end_DnaMatchBlock(mat,&i,&j,&state,&isspecial,h); 
    ...
    while( state != START || isspecial != TRUE)  { /*while state != START*/ 
      ...
      if( isspecial == TRUE )    
        max_calc_special_DnaMatchBlock(mat,i,j,state,isspecial,&i,&j,&state,&isspecial,&cellscore,h);    
      else   
        max_calc_DnaMatchBlock(mat,i,j,state,isspecial,&i,&j,&state,&isspecial,&cellscore,h);    
...
/* Function:  find_end_DnaMatchBlock(mat,ri,rj,state,isspecial,h)
 *
 * Descrip: No Description
 *
 * Arg:              mat [UNKN ] Undocumented argument [DnaMatchBlock *]
 ...
 *
 */
int find_end_DnaMatchBlock(DnaMatchBlock * mat,int * ri,int * rj,int * state,boolean * isspecial,DnaMatchBlock_access_func_holder h) 
{
    int j;   
    int max; 
    int maxj;    
...
/* Function:  max_calc_DnaMatchBlock(mat,i,j,state,isspecial,reti,retj,retstate,retspecial,cellscore,h)
 *
 * Descrip: No Description
 *
 * Arg:               mat [UNKN ] Undocumented argument [DnaMatchBlock *]
 ...
 *
 */
int max_calc_DnaMatchBlock(DnaMatchBlock * mat,int i,int j,int state,boolean isspecial,int * reti,int * retj,int * retstate,boolean * retspecial,int * cellscore,DnaMatchBlock_access_func_holder h) 
{
    register int temp;   
    register int cscore; 
...
/* Function:  max_calc_special_DnaMatchBlock(mat,i,j,state,isspecial,reti,retj,retstate,retspecial,cellscore,h)
 *
 * Descrip: No Description
 *
 * Arg:               mat [UNKN ] Undocumented argument [DnaMatchBlock *]
 ...
 *
 */
int max_calc_special_DnaMatchBlock(DnaMatchBlock * mat,int i,int j,int state,boolean isspecial,int * reti,int * retj,int * retstate,boolean * retspecial,int * cellscore,DnaMatchBlock_access_func_holder h) 
{
    register int temp;   
    register int cscore; 
 ...
 *
 */
boolean calculate_dpenv_DnaMatchBlock(DnaMatchBlock * mat,DPEnvelope * dpenv) 
{
    int i;   
    int j;   
    int k;   
