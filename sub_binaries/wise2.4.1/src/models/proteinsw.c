...
#define ProteinSW_SHATTER_SPECIAL(matrix,i,j,STATE) matrix->shatter->special[STATE][j]   
#define ProteinSW_SHATTER_MATRIX(matrix,i,j,STATE)  fetch_cell_value_ShatterMatrix(mat->shatter,i,j,STATE)   
...
{
    ProteinSW_access_func_holder holder;     


    holder.access_main    = ProteinSW_shatter_access_main;   
    holder.access_special = ProteinSW_shatter_access_special;    
    assert(mat);     
    assert(mat->shatter);    
    return PackAln_read_generic_ProteinSW(mat,holder);   
}    


/* Function:  ProteinSW_shatter_access_main(mat,i,j,state)
 *
...
{
    return ProteinSW_SHATTER_MATRIX(mat,i,j,state);  
}    


/* Function:  ProteinSW_shatter_access_special(mat,i,j,state)
 *
 * Descrip: No Description
 *
 * Arg:          mat [UNKN ] Undocumented argument [ProteinSW *]
...
{
    return ProteinSW_SHATTER_SPECIAL(mat,i,j,state); 
}    
...
boolean calculate_shatter_ProteinSW(ProteinSW * mat,DPEnvelope * dpenv) 
{
    ...
    for(j=startj;j<endj;j++) {  
      ...
      for(i=starti;i<endi;i++)   {  
        ...
        temp = ProteinSW_SHATTER_SPECIAL(mat,i-1,j-1,START) + 0;     
        if( temp  > score )  {  
          score = temp;  
          }  
        ...
        temp = score + (0) + (0) ;   
        if( temp > ProteinSW_SHATTER_SPECIAL(mat,i,j,END) )  {  
          ProteinSW_SHATTER_SPECIAL(mat,i,j,END) = temp;     
          }  
...
Search_Return_Type search_ProteinSW(DBSearchImpl * dbsi,Hscore * out,ProteinDB* querydb,ProteinDB* targetdb ,CompMat* comp,int gap,int ext) 
{
    ...
    int thr_no;  
    pthread_attr_t pat;  
    struct thread_pool_holder_ProteinSW * holder;    
#endif   
    if( out == NULL )    {  
      warn("Passed in a null Hscore object into search_ProteinSW. Can't process results!");  
      return SEARCH_ERROR;   
      }  
    if( dbsi == NULL )   {  
      warn("Passed in a null DBSearchImpl object into search_ProteinSW. Can't process results!");    
      return SEARCH_ERROR;   
      }  
    if( dbsi->trace_level > 5 )  
      warn("Asking for trace level of %d in database search for ProteinSW, but it was compiled with a trace level of -2139062144. Not all trace statements can be shown",dbsi->trace_level); 
    switch(dbsi->type)   { /*switch on implementation*/ 
      case DBSearchImpl_Serial : 
        return serial_search_ProteinSW(out,querydb, targetdb ,comp,gap,ext); 
      case DBSearchImpl_Pthreads :   
#ifdef PTHREAD   
        holder = (struct thread_pool_holder_ProteinSW *) ckalloc(sizeof(struct thread_pool_holder_ProteinSW));   
        if( holder == NULL )     {  
        ...
        for(i=0;i<thr_no;i++)    {  
          if( pthread_create(holder->pool+i,&pat,thread_loop_ProteinSW,(void *)holder) ) 
        ...
        warn("You did not specifiy the PTHREAD compile when compiled the C code for ProteinSW"); 
#endif /* finished threads */    
      default :  
        warn("database search implementation %s was not provided in the compiled dynamite file from ProteinSW",impl_string_DBSearchImpl(dbsi));  
        return SEARCH_ERROR; 
...
/* Function:  thread_loop_ProteinSW(ptr)
 *
 * Descrip:    Infinite loop code foreach thread for ProteinSW
 *
 *
 ...
 */
#ifdef PTHREAD   
void * thread_loop_ProteinSW(void * ptr) 
{
    struct thread_pool_holder_ProteinSW * holder;    
    int db_status;   
    int score;   
    ...
    holder = (struct thread_pool_holder_ProteinSW *) ptr;    
    ...
    while(1) { /*Infinite loop over all models*/ 
      ...
      if( pthread_mutex_lock(&(holder->input_lock))!= 0 )    
        fatal("Error on getting input lock for ProteinSW");  
      if ( holder->dbsi->trace_level >= 2 )  
      ...
      if( db_status == DB_RETURN_ERROR ) {  
        fatal("In searching ProteinSW, Reload error on database target, in threads");    
        }  
      ...
      if( db_status == DB_RETURN_END)    { /*End of target database*/ 
        ...
        if( db_status == DB_RETURN_ERROR )   
          fatal("In searching ProteinSW, reload error on database query, in threads");   
      ...
      score = score_only_ProteinSW(query, target ,holder->comp,holder->gap,holder->ext);     
      ...
      if( pthread_mutex_lock(&(holder->output_lock))!= 0 )   
        fatal("Error on getting output lock for ProteinSW"); 
      ...
      if( pthread_mutex_unlock(&(holder->output_lock))!= 0 ) 
        fatal("Error on releasing output lock for ProteinSW");   
      if ( holder->dbsi->trace_level >= 2 )  
...
PackAln * PackAln_bestmemory_ProteinSW(ComplexSequence* query,ComplexSequence* target ,CompMat* comp,int gap,int ext,DPEnvelope * dpenv,DPRunImpl * dpri) 
{
    ...
    if( dpenv != NULL )  {  
      if( strategy == DPIM_Explicit) {  
        if( (mat=allocate_Expl_ProteinSW(query, target , comp, gap, ext,dpri)) == NULL ) {  
          warn("Unable to allocate large ProteinSW version");    
          return NULL;   
          }  
        calculate_dpenv_ProteinSW(mat,dpenv);    
        out =  PackAln_read_Expl_ProteinSW(mat); 
        }  
      else   {  
        mat = allocate_ProteinSW_only(query, target , comp, gap, ext);   
        calculate_shatter_ProteinSW(mat,dpenv);  
        out = PackAln_read_Shatter_ProteinSW(mat);   
        }  
      }  
    else {  
...
#define ProteinSW_DC_OPT_SHADOW_MATRIX(thismatrix,i,j,state) (score_pointers[(((j+1)% 1) * (leni+1) * 3) + ((i+1) * 3) + (state)])   
#define ProteinSW_DC_OPT_SHADOW_MATRIX_SP(thismatrix,i,j,state,shadow) (shadow_pointers[(((j+1)% 1) * (leni+1) * 24) + ((i+1) * 24) + (state * 8) + shadow+1])   
#define ProteinSW_DC_OPT_SHADOW_SPECIAL(thismatrix,i,j,state) (thismatrix->basematrix->specmatrix[state*8][(j+1)])   
 ...
 *
 */
#define ProteinSW_DC_OPT_SHADOW_SPECIAL_SP(thismatrix,i,j,state,shadow) (thismatrix->basematrix->specmatrix[state*8 +shadow+1][(j+1)])   
...
PackAln * PackAln_calculate_Small_ProteinSW(ProteinSW * mat,DPEnvelope * dpenv) 
{
    ...
    dc_optimised_start_end_calc_ProteinSW(mat,dpenv);    
...
AlnRangeSet * AlnRangeSet_calculate_Small_ProteinSW(ProteinSW * mat) 
{
    ...
    start_reporting("Find start end points: ");  
    dc_optimised_start_end_calc_ProteinSW(mat,NULL); 
...
boolean read_special_strip_ProteinSW(ProteinSW * mat,int stopi,int stopj,int stopstate,int * startj,int * startstate,PackAln * out) 
{
    ...
    while( j > ProteinSW_DC_SHADOW_SPECIAL_SP(mat,i,j,state,4) && state != START)    { /*while more specials to eat up*/ 
...
/* Function:  dc_optimised_start_end_calc_ProteinSW(*mat,dpenv)
 *
...
boolean dc_optimised_start_end_calc_ProteinSW(ProteinSW *mat,DPEnvelope * dpenv) 
{
    ...
    for(j=0;j<lenj;j++)  { /*for each j strip*/ 
      for(i=0;i<leni;i++)    { /*for each i position in strip*/ 
        num++;   
        if( dpenv != NULL && is_in_DPEnvelope(dpenv,i,j) == FALSE )  { /*Is not in envelope*/ 
          ProteinSW_DC_OPT_SHADOW_MATRIX(mat,i,j,MATCH) = NEGI;  
          ProteinSW_DC_OPT_SHADOW_MATRIX(mat,i,j,INSERT) = NEGI;     
          ProteinSW_DC_OPT_SHADOW_MATRIX(mat,i,j,DELETE) = NEGI;     
          continue;  
          } /* end of Is not in envelope */ 
        if( num%1000 == 0)   
        ...
        /* For state MATCH */ 
        /* setting first movement to score */ 
        score = ProteinSW_DC_OPT_SHADOW_MATRIX(mat,i-1,j-1,MATCH) + 0 + (CompMat_AAMATCH(mat->comp,CSEQ_PROTEIN_AMINOACID(mat->query,i),CSEQ_PROTEIN_AMINOACID(mat->target,j)));     
        /* assign local shadown pointer */ 
        localsp = &(ProteinSW_DC_OPT_SHADOW_MATRIX_SP(mat,i - 1,j - 1,MATCH,0)); 
        /* From state INSERT to state MATCH */ 
        temp = ProteinSW_DC_OPT_SHADOW_MATRIX(mat,i-1,j-1,INSERT) + 0 +(CompMat_AAMATCH(mat->comp,CSEQ_PROTEIN_AMINOACID(mat->query,i),CSEQ_PROTEIN_AMINOACID(mat->target,j)));  
        if( temp  > score )  {  
          score = temp;  
          /* assign local shadown pointer */ 
          localsp = &(ProteinSW_DC_OPT_SHADOW_MATRIX_SP(mat,i - 1,j - 1,INSERT,0));  
          }  
        /* From state DELETE to state MATCH */ 
        temp = ProteinSW_DC_OPT_SHADOW_MATRIX(mat,i-1,j-1,DELETE) + 0 +(CompMat_AAMATCH(mat->comp,CSEQ_PROTEIN_AMINOACID(mat->query,i),CSEQ_PROTEIN_AMINOACID(mat->target,j)));  
        if( temp  > score )  {  
          score = temp;  
          /* assign local shadown pointer */ 
          localsp = &(ProteinSW_DC_OPT_SHADOW_MATRIX_SP(mat,i - 1,j - 1,DELETE,0));  
          }  
        /* From state START to state MATCH */ 
        temp = ProteinSW_DC_OPT_SHADOW_SPECIAL(mat,i-1,j-1,START) + 0 + (CompMat_AAMATCH(mat->comp,CSEQ_PROTEIN_AMINOACID(mat->query,i),CSEQ_PROTEIN_AMINOACID(mat->target,j)));     
        if( temp  > score )  {  
          score = temp;  
        ...
        /* Actually, already done inside scores */ 
         ProteinSW_DC_OPT_SHADOW_MATRIX(mat,i,j,MATCH) = score;  
        for(k=0;k<7;k++) 
          ProteinSW_DC_OPT_SHADOW_MATRIX_SP(mat,i,j,MATCH,k) = localsp[k];   
        /* Now figure out if any specials need this score */ 


        /* state MATCH is a source for special END */ 
        temp = score + (0) + (0) ;   
        if( temp > ProteinSW_DC_OPT_SHADOW_SPECIAL(mat,i,j,END) )    {  
          ProteinSW_DC_OPT_SHADOW_SPECIAL(mat,i,j,END) = temp;   
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            ProteinSW_DC_OPT_SHADOW_SPECIAL_SP(mat,i,j,END,k) = ProteinSW_DC_OPT_SHADOW_MATRIX_SP(mat,i,j,MATCH,k);  
          ProteinSW_DC_OPT_SHADOW_SPECIAL_SP(mat,i,j,END,6) = ProteinSW_DC_OPT_SHADOW_MATRIX_SP(mat,i,j,MATCH,6);    
          ProteinSW_DC_OPT_SHADOW_SPECIAL_SP(mat,i,j,END,3) = i; 
          ProteinSW_DC_OPT_SHADOW_SPECIAL_SP(mat,i,j,END,4) = j; 
          ProteinSW_DC_OPT_SHADOW_SPECIAL_SP(mat,i,j,END,5) = MATCH; 
          }  
        ...
        /* For state INSERT */ 
        /* setting first movement to score */ 
        score = ProteinSW_DC_OPT_SHADOW_MATRIX(mat,i-0,j-1,MATCH) + mat->gap + (0);  
        /* assign local shadown pointer */ 
        localsp = &(ProteinSW_DC_OPT_SHADOW_MATRIX_SP(mat,i - 0,j - 1,MATCH,0)); 
        /* From state INSERT to state INSERT */ 
        temp = ProteinSW_DC_OPT_SHADOW_MATRIX(mat,i-0,j-1,INSERT) + mat->ext +(0);   
        if( temp  > score )  {  
          score = temp;  
          /* assign local shadown pointer */ 
          localsp = &(ProteinSW_DC_OPT_SHADOW_MATRIX_SP(mat,i - 0,j - 1,INSERT,0));  
          }  
        ...
        /* Add any movement independant score and put away */ 
        /* Actually, already done inside scores */ 
         ProteinSW_DC_OPT_SHADOW_MATRIX(mat,i,j,INSERT) = score; 
        for(k=0;k<7;k++) 
          ProteinSW_DC_OPT_SHADOW_MATRIX_SP(mat,i,j,INSERT,k) = localsp[k];  
        /* Now figure out if any specials need this score */ 
        ...
        /* For state DELETE */ 
        /* setting first movement to score */ 
        score = ProteinSW_DC_OPT_SHADOW_MATRIX(mat,i-1,j-0,MATCH) + mat->gap + (0);  
        /* assign local shadown pointer */ 
        localsp = &(ProteinSW_DC_OPT_SHADOW_MATRIX_SP(mat,i - 1,j - 0,MATCH,0)); 
        /* From state DELETE to state DELETE */ 
        temp = ProteinSW_DC_OPT_SHADOW_MATRIX(mat,i-1,j-0,DELETE) + mat->ext +(0);   
        if( temp  > score )  {  
          score = temp;  
          /* assign local shadown pointer */ 
          localsp = &(ProteinSW_DC_OPT_SHADOW_MATRIX_SP(mat,i - 1,j - 0,DELETE,0));  
          }  
        ...
        /* Add any movement independant score and put away */ 
        /* Actually, already done inside scores */ 
         ProteinSW_DC_OPT_SHADOW_MATRIX(mat,i,j,DELETE) = score; 
        for(k=0;k<7;k++) 
          ProteinSW_DC_OPT_SHADOW_MATRIX_SP(mat,i,j,DELETE,k) = localsp[k];  
        /* Now figure out if any specials need this score */ 
...
PackAln * PackAln_read_Expl_ProteinSW(ProteinSW * mat) 
{
    ...
    holder.access_main    = ProteinSW_explicit_access_main;  
    holder.access_special = ProteinSW_explicit_access_special;   
    return PackAln_read_generic_ProteinSW(mat,holder);   
}    


/* Function:  ProteinSW_explicit_access_main(mat,i,j,state)
 *
 * Descrip: No Description
 *
 * Arg:          mat [UNKN ] Undocumented argument [ProteinSW *]
...
{
    return ProteinSW_EXPL_MATRIX(mat,i,j,state); 
}    


/* Function:  ProteinSW_explicit_access_special(mat,i,j,state)
 *
 * Descrip: No Description
 *
 * Arg:          mat [UNKN ] Undocumented argument [ProteinSW *]
...
{
    return ProteinSW_EXPL_SPECIAL(mat,i,j,state);    
}    
...
PackAln * PackAln_read_generic_ProteinSW(ProteinSW * mat,ProteinSW_access_func_holder h) 
{
    ...
    out->score =  find_end_ProteinSW(mat,&i,&j,&state,&isspecial,h); 
    ...
    while( state != START || isspecial != TRUE)  { /*while state != START*/ 
      ...
      if( isspecial == TRUE )    
        max_calc_special_ProteinSW(mat,i,j,state,isspecial,&i,&j,&state,&isspecial,&cellscore,h);    
      else   
        max_calc_ProteinSW(mat,i,j,state,isspecial,&i,&j,&state,&isspecial,&cellscore,h);    
...
/* Function:  find_end_ProteinSW(mat,ri,rj,state,isspecial,h)
 *
 * Descrip: No Description
 *
 * Arg:              mat [UNKN ] Undocumented argument [ProteinSW *]
...
/* Function:  max_calc_ProteinSW(mat,i,j,state,isspecial,reti,retj,retstate,retspecial,cellscore,h)
 *
 * Descrip: No Description
 *
 * Arg:               mat [UNKN ] Undocumented argument [ProteinSW *]
...
/* Function:  max_calc_special_ProteinSW(mat,i,j,state,isspecial,reti,retj,retstate,retspecial,cellscore,h)
 *
 * Descrip: No Description
 *
 * Arg:               mat [UNKN ] Undocumented argument [ProteinSW *]
