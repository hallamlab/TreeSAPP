...
/* Function:  search_EstLoop3(dbsi,out,query,targetdb,cp)
 *
 ...
 *
 */
Search_Return_Type search_EstLoop3(DBSearchImpl * dbsi,Hscore * out,GeneWiseScore * query,cDNADB* targetdb ,cDNAParserScore * cp) 
{
#ifdef PTHREAD   
    int i;   
    int thr_no;  
    ...
    if( dbsi->trace_level > 5 )  
      warn("Asking for trace level of %d in database search for EstLoop3, but it was compiled with a trace level of 416. Not all trace statements can be shown",dbsi->trace_level);  
    switch(dbsi->type)   { /*switch on implementation*/ 
      case DBSearchImpl_Serial : 
        return serial_search_EstLoop3(out,query, targetdb ,cp);  
...
/* Function:  serial_search_EstLoop3(out,query,targetdb,cp)
 *
 * Descrip:    This function makes a database search of EstLoop3
 ...
 *
 */
Search_Return_Type serial_search_EstLoop3(Hscore * out,GeneWiseScore * query,cDNADB* targetdb ,cDNAParserScore * cp) 
{
...
PackAln * PackAln_bestmemory_EstLoop3(GeneWiseScore * query,ComplexSequence* target ,cDNAParserScore * cp,DPEnvelope * dpenv,DPRunImpl * dpri) 
{
    ...
    if( strategy == DPIM_Linear )    {  
      ...
      if( (mat=allocate_Expl_EstLoop3(query, target , cp)) == NULL ) {  
...
/* Function:  allocate_Expl_EstLoop3(query,target,cp)
 *
 ...
 *
 */
EstLoop3 * allocate_Expl_EstLoop3(GeneWiseScore * query,ComplexSequence* target ,cDNAParserScore * cp) 
{
    EstLoop3 * out;  
...
PackAln * PackAln_calculate_Small_EstLoop3(EstLoop3 * mat,DPEnvelope * dpenv) 
{
    ...
    dc_start_end_calculate_EstLoop3(mat,dpenv);  
...
AlnRangeSet * AlnRangeSet_calculate_Small_EstLoop3(EstLoop3 * mat) 
{
    ...
    start_reporting("Find start end points: ");  
    dc_start_end_calculate_EstLoop3(mat,NULL);   
...
/* Function:  dc_start_end_calculate_EstLoop3(mat,dpenv)
 *
 ...
 *
 */
boolean dc_start_end_calculate_EstLoop3(EstLoop3 * mat,DPEnvelope * dpenv) 
{
    int i;   
    int j;   
    int k;   
    ...
    init_start_end_linear_EstLoop3(mat);     
    ...
    for(j=0;j<lenj;j++)  { /*for each j strip*/ 
      for(i=0;i<leni;i++)    { /*for each i position in strip*/ 
        ...
        /* For state MATCH */ 
        /* setting first movement to score */ 
        score = EstLoop3_DC_SHADOW_MATRIX(mat,i-1,j-3,MATCH) + (mat->query->seg[i]->transition[GW_MATCH2MATCH]+mat->query->seg[i]->match[CSEQ_CDNA_CODON(mat->target,j)]) + (0);     
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 3,MATCH,k);    
        /* From state INSERT to state MATCH */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-1,j-3,INSERT) + (mat->query->seg[i]->transition[GW_INSERT2MATCH]+mat->query->seg[i]->match[CSEQ_CDNA_CODON(mat->target,j)]) +(0);     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 3,INSERT,k); 
        ...
        /* From state DELETE to state MATCH */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-1,j-3,DELETE) + (mat->query->seg[i]->transition[GW_DELETE2MATCH]+mat->query->seg[i]->match[CSEQ_CDNA_CODON(mat->target,j)]) +(0);     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 3,DELETE,k); 
        ...
        /* From state LOOP to state MATCH */ 
        temp = EstLoop3_DC_SHADOW_SPECIAL(mat,i-1,j-3,LOOP) + (mat->query->seg[i]->transition[GW_START2MATCH]+mat->query->seg[i]->match[CSEQ_CDNA_CODON(mat->target,j)]) + (0);  
        if( temp  > score )  {  
          score = temp;  
        ...
        /* From state START to state MATCH */ 
        temp = EstLoop3_DC_SHADOW_SPECIAL(mat,i-1,j-3,START) + (mat->query->seg[i]->transition[GW_START2MATCH]+mat->query->seg[i]->match[CSEQ_CDNA_CODON(mat->target,j)]) + (0);     
        if( temp  > score )  {  
          score = temp;  
        ...
        /* From state MATCH to state MATCH */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-1,j-2,MATCH) + mat->cp->trans[PCD_DELETE_1_BASE] +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 2,MATCH,k);  
        ...
        /* From state MATCH to state MATCH */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-1,j-1,MATCH) + mat->cp->trans[PCD_DELETE_2_BASE] +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 1,MATCH,k);  
        ...
        /* From state MATCH to state MATCH */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-1,j-4,MATCH) + mat->cp->trans[PCD_INSERT_1_BASE] +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 4,MATCH,k);  
        ...
        /* From state MATCH to state MATCH */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-1,j-5,MATCH) + mat->cp->trans[PCD_INSERT_2_BASE] +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 5,MATCH,k);  
        ...
        /* From state INSERT to state MATCH */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-1,j-2,INSERT) + mat->cp->trans[PCD_DELETE_1_BASE] +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 2,INSERT,k); 
        ...
        /* From state INSERT to state MATCH */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-1,j-1,INSERT) + mat->cp->trans[PCD_DELETE_2_BASE] +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 1,INSERT,k); 
        ...
        /* From state INSERT to state MATCH */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-1,j-4,INSERT) + mat->cp->trans[PCD_INSERT_1_BASE] +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 4,INSERT,k); 
        ...
        /* From state INSERT to state MATCH */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-1,j-5,INSERT) + mat->cp->trans[PCD_INSERT_2_BASE] +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 5,INSERT,k); 
        ...
        /* From state DELETE to state MATCH */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-1,j-2,DELETE) + mat->cp->trans[PCD_DELETE_1_BASE] +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 2,DELETE,k); 
        ...
        /* From state DELETE to state MATCH */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-1,j-1,DELETE) + mat->cp->trans[PCD_DELETE_2_BASE] +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 1,DELETE,k); 
        ...
        /* From state DELETE to state MATCH */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-1,j-4,DELETE) + mat->cp->trans[PCD_INSERT_1_BASE] +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 4,DELETE,k); 
        ...
        /* From state DELETE to state MATCH */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-1,j-5,DELETE) + mat->cp->trans[PCD_INSERT_2_BASE] +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 5,DELETE,k); 
        ...
        /* state MATCH is a source for special LOOP */ 
        temp = score + (mat->query->seg[i]->transition[GW_MATCH2END]) + (0) ;    
        if( temp > EstLoop3_DC_SHADOW_SPECIAL(mat,i,j,LOOP) )    {  
          EstLoop3_DC_SHADOW_SPECIAL(mat,i,j,LOOP) = temp;   
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,k) = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i,j,MATCH,k);   
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,6) = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i,j,MATCH,6); 
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,3) = i; 
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,4) = j; 
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,5) = MATCH; 
          }  
        ...
        /* state MATCH is a source for special END */ 
        temp = score + (mat->query->seg[i]->transition[GW_MATCH2END]) + (0) ;    
        if( temp > EstLoop3_DC_SHADOW_SPECIAL(mat,i,j,END) )     {  
          EstLoop3_DC_SHADOW_SPECIAL(mat,i,j,END) = temp;    
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,END,k) = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i,j,MATCH,k);    
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,END,6) = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i,j,MATCH,6);  
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,END,3) = i;  
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,END,4) = j;  
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,END,5) = MATCH;  
          }  
        ...
        /* For state INSERT */ 
        /* setting first movement to score */ 
        score = EstLoop3_DC_SHADOW_MATRIX(mat,i-0,j-3,MATCH) + (mat->query->seg[i]->transition[GW_MATCH2INSERT]+mat->query->seg[i]->insert[CSEQ_CDNA_CODON(mat->target,j)]) + (0);   
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 3,MATCH,k);    
        /* From state INSERT to state INSERT */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-0,j-3,INSERT) + (mat->query->seg[i]->transition[GW_INSERT2INSERT]+mat->query->seg[i]->insert[CSEQ_CDNA_CODON(mat->target,j)]) +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 3,INSERT,k); 
        ...
        /* From state DELETE to state INSERT */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-0,j-3,DELETE) + (mat->query->seg[i]->transition[GW_DELETE2INSERT]+mat->query->seg[i]->insert[CSEQ_CDNA_CODON(mat->target,j)]) +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 3,DELETE,k); 
        ...
        /* From state LOOP to state INSERT */ 
        temp = EstLoop3_DC_SHADOW_SPECIAL(mat,i-0,j-3,LOOP) + (mat->query->seg[i]->transition[GW_START2INSERT]+mat->query->seg[i]->insert[CSEQ_CDNA_CODON(mat->target,j)]) + (0);    
        if( temp  > score )  {  
          score = temp;  
        ...
        /* From state INSERT to state INSERT */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-0,j-2,INSERT) + mat->cp->trans[PCD_DELETE_1_BASE] +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 2,INSERT,k); 
        ...
        /* From state INSERT to state INSERT */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-0,j-1,INSERT) + mat->cp->trans[PCD_DELETE_2_BASE] +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,INSERT,k); 
        ...
        /* From state INSERT to state INSERT */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-0,j-4,INSERT) + mat->cp->trans[PCD_INSERT_1_BASE] +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 4,INSERT,k); 
        ...
        /* From state INSERT to state INSERT */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-0,j-5,INSERT) + mat->cp->trans[PCD_INSERT_2_BASE] +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 5,INSERT,k); 
        ...
        /* From state MATCH to state INSERT */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-0,j-2,MATCH) + mat->cp->trans[PCD_DELETE_1_BASE] +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 2,MATCH,k);  
        ...
        /* From state MATCH to state INSERT */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-0,j-1,MATCH) + mat->cp->trans[PCD_DELETE_2_BASE] +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,MATCH,k);  
        ...
        /* From state MATCH to state INSERT */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-0,j-4,MATCH) + mat->cp->trans[PCD_INSERT_1_BASE] +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 4,MATCH,k);  
        ...
        /* From state MATCH to state INSERT */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-0,j-5,MATCH) + mat->cp->trans[PCD_INSERT_2_BASE] +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 5,MATCH,k);  
        ...
        /* From state DELETE to state INSERT */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-0,j-2,DELETE) + mat->cp->trans[PCD_DELETE_1_BASE] +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 2,DELETE,k); 
        ...
        /* From state DELETE to state INSERT */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-0,j-1,DELETE) + mat->cp->trans[PCD_DELETE_2_BASE] +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,DELETE,k); 
        ...
        /* From state DELETE to state INSERT */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-0,j-4,DELETE) + mat->cp->trans[PCD_INSERT_1_BASE] +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 4,DELETE,k); 
        ...
        /* From state DELETE to state INSERT */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-0,j-5,DELETE) + mat->cp->trans[PCD_INSERT_2_BASE] +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 5,DELETE,k); 
        ...
        if( temp > EstLoop3_DC_SHADOW_SPECIAL(mat,i,j,LOOP) )    {  
          ...
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,k) = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i,j,INSERT,k);  
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,6) = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i,j,INSERT,6);    
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,3) = i; 
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,4) = j; 
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,5) = INSERT;    
          }  
        ...
        if( temp > EstLoop3_DC_SHADOW_SPECIAL(mat,i,j,END) )     {  
          ...
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,END,k) = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i,j,INSERT,k);   
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,END,6) = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i,j,INSERT,6); 
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,END,3) = i;  
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,END,4) = j;  
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,END,5) = INSERT; 
          }  
        ...
        /* For state DELETE */ 
        /* setting first movement to score */ 
        score = EstLoop3_DC_SHADOW_MATRIX(mat,i-1,j-0,MATCH) + mat->query->seg[i]->transition[GW_MATCH2DELETE] + (0);    
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 0,MATCH,k);    
        /* From state INSERT to state DELETE */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-1,j-0,INSERT) + mat->query->seg[i]->transition[GW_INSERT2DELETE] +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 0,INSERT,k); 
        ...
        /* From state DELETE to state DELETE */ 
        temp = EstLoop3_DC_SHADOW_MATRIX(mat,i-1,j-0,DELETE) + mat->query->seg[i]->transition[GW_DELETE2DELETE] +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 0,DELETE,k); 
        ...
        /* From state LOOP to state DELETE */ 
        temp = EstLoop3_DC_SHADOW_SPECIAL(mat,i-1,j-0,LOOP) + mat->query->seg[i]->transition[GW_START2DELETE] + (0);     
        if( temp  > score )  {  
          score = temp;  
        ...
        if( temp > EstLoop3_DC_SHADOW_SPECIAL(mat,i,j,LOOP) )    {  
          ...
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,k) = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i,j,DELETE,k);  
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,6) = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i,j,DELETE,6);    
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,3) = i; 
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,4) = j; 
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,5) = DELETE;    
          }  
        ...
        if( temp > EstLoop3_DC_SHADOW_SPECIAL(mat,i,j,END) )     {  
          ...
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,END,k) = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i,j,DELETE,k);   
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,END,6) = EstLoop3_DC_SHADOW_MATRIX_SP(mat,i,j,DELETE,6); 
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,END,3) = i;  
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,END,4) = j;  
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,END,5) = DELETE; 
          }  
      ...
      /* Special state LOOP has special to speical */ 
      /* Set score to current score (remember, state probably updated during main loop */ 
      score = EstLoop3_DC_SHADOW_SPECIAL(mat,0,j,LOOP);  


      /* Source START is a special source for LOOP */ 
      temp = EstLoop3_DC_SHADOW_SPECIAL(mat,0,j - 1,START) + (0) + (0);  
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,k) = EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,START,k);    
        }  
      ...
      /* Source DELETE for state LOOP is not special... already calculated */ 
      /* Source LOOP is a special source for LOOP */ 
      temp = EstLoop3_DC_SHADOW_SPECIAL(mat,0,j - 1,LOOP) + (0) + (0);   
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,k) = EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,LOOP,k); 
        }  


      /* Put back score... (now updated!) */ 
      EstLoop3_DC_SHADOW_SPECIAL(mat,0,j,LOOP) = score;  
      /* Finished updating state LOOP */ 
      ...
      /* Special state END has special to speical */ 
      /* Set score to current score (remember, state probably updated during main loop */ 
      score = EstLoop3_DC_SHADOW_SPECIAL(mat,0,j,END);   
      ...
      if( temp > score ) {  
        ...
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i,j,END,k) = EstLoop3_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,LOOP,k);  
        }  
      ...
      /* Source DELETE for state END is not special... already calculated */ 
      /* Put back score... (now updated!) */ 
      EstLoop3_DC_SHADOW_SPECIAL(mat,0,j,END) = score;   
      /* Finished updating state END */ 
...
PackAln * PackAln_read_Expl_EstLoop3(EstLoop3 * mat) 
{
    ...
    if( mat->basematrix->type != BASEMATRIX_TYPE_EXPLICIT)   {  
      warn("In EstLoop3_basic_read You have asked for an alignment from a non-explicit matrix: c'est impossible [current type is %d - %s]", mat->basematrix->type,basematrix_type_to_string(mat->basematrix->type)); 
      return NULL;   
      }  
    ...
    out->score =  find_end_EstLoop3(mat,&i,&j,&state,&isspecial);    
    ...
    while( state != START || isspecial != TRUE)  { /*while state != START*/ 
      ...
      if( isspecial == TRUE )    
        max_calc_special_EstLoop3(mat,i,j,state,isspecial,&i,&j,&state,&isspecial,&cellscore);   
      else   
        max_calc_EstLoop3(mat,i,j,state,isspecial,&i,&j,&state,&isspecial,&cellscore);   
...
/* Function:  find_end_EstLoop3(mat,ri,rj,state,isspecial)
 *
 * Descrip: No Description
 *
 * Arg:              mat [UNKN ] Undocumented argument [EstLoop3 *]
 ...
 *
 */
int find_end_EstLoop3(EstLoop3 * mat,int * ri,int * rj,int * state,boolean * isspecial) 
{
    register int j;  
    register int max;    
    register int maxj;   
    ...
    max = EstLoop3_EXPL_SPECIAL(mat,0,mat->target->seq->len-1,END);  
    maxj = mat->target->seq->len-1;  
    for(j= mat->target->seq->len-2 ;j >= 0 ;j--) {  
      if( EstLoop3_EXPL_SPECIAL(mat,0,j,END) > max ) {  
        max = EstLoop3_EXPL_SPECIAL(mat,0,j,END);    
        maxj = j;    
        }  
      }  
...
/* Function:  max_calc_EstLoop3(mat,i,j,state,isspecial,reti,retj,retstate,retspecial,cellscore)
 *
 * Descrip: No Description
 *
 * Arg:               mat [UNKN ] Undocumented argument [EstLoop3 *]
 ...
 *
 */
int max_calc_EstLoop3(EstLoop3 * mat,int i,int j,int state,boolean isspecial,int * reti,int * retj,int * retstate,boolean * retspecial,int * cellscore) 
{
    register int temp;   
    register int cscore; 
    ...
    *reti = (*retj) = (*retstate) = EstLoop3_READ_OFF_ERROR; 
    ...
    /* Then you have to select the correct switch statement to figure out the readoff      */ 
    /* Somewhat odd - reverse the order of calculation and return as soon as it is correct */ 
    cscore = EstLoop3_EXPL_MATRIX(mat,i,j,state);    
    switch(state)    { /*Switch state */ 
      case MATCH :   
        temp = cscore - (mat->cp->trans[PCD_INSERT_2_BASE]) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 1,j - 5,DELETE) )   {  
          *reti = i - 1; 
          *retj = j - 5; 
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-1,j-5,DELETE);  
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 1,j - 5,DELETE);   
          }  
        temp = cscore - (mat->cp->trans[PCD_INSERT_1_BASE]) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 1,j - 4,DELETE) )   {  
          *reti = i - 1; 
          *retj = j - 4; 
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-1,j-4,DELETE);  
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 1,j - 4,DELETE);   
          }  
        temp = cscore - (mat->cp->trans[PCD_DELETE_2_BASE]) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 1,j - 1,DELETE) )   {  
          *reti = i - 1; 
          *retj = j - 1; 
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-1,j-1,DELETE);  
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 1,j - 1,DELETE);   
          }  
        temp = cscore - (mat->cp->trans[PCD_DELETE_1_BASE]) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 1,j - 2,DELETE) )   {  
          *reti = i - 1; 
          *retj = j - 2; 
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-1,j-2,DELETE);  
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 1,j - 2,DELETE);   
          }  
        temp = cscore - (mat->cp->trans[PCD_INSERT_2_BASE]) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 1,j - 5,INSERT) )   {  
          *reti = i - 1; 
          *retj = j - 5; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-1,j-5,INSERT);  
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 1,j - 5,INSERT);   
          }  
        temp = cscore - (mat->cp->trans[PCD_INSERT_1_BASE]) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 1,j - 4,INSERT) )   {  
          *reti = i - 1; 
          *retj = j - 4; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-1,j-4,INSERT);  
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 1,j - 4,INSERT);   
          }  
        temp = cscore - (mat->cp->trans[PCD_DELETE_2_BASE]) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 1,j - 1,INSERT) )   {  
          *reti = i - 1; 
          *retj = j - 1; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-1,j-1,INSERT);  
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 1,j - 1,INSERT);   
          }  
        temp = cscore - (mat->cp->trans[PCD_DELETE_1_BASE]) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 1,j - 2,INSERT) )   {  
          *reti = i - 1; 
          *retj = j - 2; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-1,j-2,INSERT);  
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 1,j - 2,INSERT);   
          }  
        temp = cscore - (mat->cp->trans[PCD_INSERT_2_BASE]) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 1,j - 5,MATCH) )    {  
          *reti = i - 1; 
          *retj = j - 5; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-1,j-5,MATCH);   
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 1,j - 5,MATCH);    
          }  
        temp = cscore - (mat->cp->trans[PCD_INSERT_1_BASE]) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 1,j - 4,MATCH) )    {  
          *reti = i - 1; 
          *retj = j - 4; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-1,j-4,MATCH);   
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 1,j - 4,MATCH);    
          }  
        temp = cscore - (mat->cp->trans[PCD_DELETE_2_BASE]) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 1,j - 1,MATCH) )    {  
          *reti = i - 1; 
          *retj = j - 1; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-1,j-1,MATCH);   
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 1,j - 1,MATCH);    
          }  
        temp = cscore - (mat->cp->trans[PCD_DELETE_1_BASE]) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 1,j - 2,MATCH) )    {  
          *reti = i - 1; 
          *retj = j - 2; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-1,j-2,MATCH);   
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 1,j - 2,MATCH);    
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_START2MATCH]+mat->query->seg[i]->match[CSEQ_CDNA_CODON(mat->target,j)])) -  (0); 
        if( temp == EstLoop3_EXPL_SPECIAL(mat,i - 1,j - 3,START) )   {  
          *reti = i - 1; 
          *retj = j - 3; 
          *retstate = START; 
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_SPECIAL(mat,i-1,j-3,START);  
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 1,j - 3,START);    
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_START2MATCH]+mat->query->seg[i]->match[CSEQ_CDNA_CODON(mat->target,j)])) -  (0); 
        if( temp == EstLoop3_EXPL_SPECIAL(mat,i - 1,j - 3,LOOP) )    {  
          *reti = i - 1; 
          *retj = j - 3; 
          *retstate = LOOP;  
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_SPECIAL(mat,i-1,j-3,LOOP);   
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 1,j - 3,LOOP);     
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_DELETE2MATCH]+mat->query->seg[i]->match[CSEQ_CDNA_CODON(mat->target,j)])) -  (0);    
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 1,j - 3,DELETE) )   {  
          *reti = i - 1; 
          *retj = j - 3; 
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-1,j-3,DELETE);  
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 1,j - 3,DELETE);   
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_INSERT2MATCH]+mat->query->seg[i]->match[CSEQ_CDNA_CODON(mat->target,j)])) -  (0);    
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 1,j - 3,INSERT) )   {  
          *reti = i - 1; 
          *retj = j - 3; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-1,j-3,INSERT);  
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 1,j - 3,INSERT);   
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_MATCH2MATCH]+mat->query->seg[i]->match[CSEQ_CDNA_CODON(mat->target,j)])) -  (0); 
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 1,j - 3,MATCH) )    {  
          *reti = i - 1; 
          *retj = j - 3; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-1,j-3,MATCH);   
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 1,j - 3,MATCH);    
          }  
        warn("Major problem (!) - in EstLoop3 read off, position %d,%d state %d no source found!",i,j,state);    
        return (-1); 
      case INSERT :  
        temp = cscore - (mat->cp->trans[PCD_INSERT_2_BASE]) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 0,j - 5,DELETE) )   {  
          *reti = i - 0; 
          *retj = j - 5; 
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-0,j-5,DELETE);  
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 0,j - 5,DELETE);   
          }  
        temp = cscore - (mat->cp->trans[PCD_INSERT_1_BASE]) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 0,j - 4,DELETE) )   {  
          *reti = i - 0; 
          *retj = j - 4; 
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-0,j-4,DELETE);  
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 0,j - 4,DELETE);   
          }  
        temp = cscore - (mat->cp->trans[PCD_DELETE_2_BASE]) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 0,j - 1,DELETE) )   {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-0,j-1,DELETE);  
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 0,j - 1,DELETE);   
          }  
        temp = cscore - (mat->cp->trans[PCD_DELETE_1_BASE]) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 0,j - 2,DELETE) )   {  
          *reti = i - 0; 
          *retj = j - 2; 
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-0,j-2,DELETE);  
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 0,j - 2,DELETE);   
          }  
        temp = cscore - (mat->cp->trans[PCD_INSERT_2_BASE]) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 0,j - 5,MATCH) )    {  
          *reti = i - 0; 
          *retj = j - 5; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-0,j-5,MATCH);   
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 0,j - 5,MATCH);    
          }  
        temp = cscore - (mat->cp->trans[PCD_INSERT_1_BASE]) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 0,j - 4,MATCH) )    {  
          *reti = i - 0; 
          *retj = j - 4; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-0,j-4,MATCH);   
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 0,j - 4,MATCH);    
          }  
        temp = cscore - (mat->cp->trans[PCD_DELETE_2_BASE]) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 0,j - 1,MATCH) )    {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-0,j-1,MATCH);   
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 0,j - 1,MATCH);    
          }  
        temp = cscore - (mat->cp->trans[PCD_DELETE_1_BASE]) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 0,j - 2,MATCH) )    {  
          *reti = i - 0; 
          *retj = j - 2; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-0,j-2,MATCH);   
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 0,j - 2,MATCH);    
          }  
        temp = cscore - (mat->cp->trans[PCD_INSERT_2_BASE]) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 0,j - 5,INSERT) )   {  
          *reti = i - 0; 
          *retj = j - 5; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-0,j-5,INSERT);  
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 0,j - 5,INSERT);   
          }  
        temp = cscore - (mat->cp->trans[PCD_INSERT_1_BASE]) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 0,j - 4,INSERT) )   {  
          *reti = i - 0; 
          *retj = j - 4; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-0,j-4,INSERT);  
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 0,j - 4,INSERT);   
          }  
        temp = cscore - (mat->cp->trans[PCD_DELETE_2_BASE]) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 0,j - 1,INSERT) )   {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-0,j-1,INSERT);  
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 0,j - 1,INSERT);   
          }  
        temp = cscore - (mat->cp->trans[PCD_DELETE_1_BASE]) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 0,j - 2,INSERT) )   {  
          *reti = i - 0; 
          *retj = j - 2; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-0,j-2,INSERT);  
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 0,j - 2,INSERT);   
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_START2INSERT]+mat->query->seg[i]->insert[CSEQ_CDNA_CODON(mat->target,j)])) -  (0);   
        if( temp == EstLoop3_EXPL_SPECIAL(mat,i - 0,j - 3,LOOP) )    {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = LOOP;  
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_SPECIAL(mat,i-0,j-3,LOOP);   
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 0,j - 3,LOOP);     
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_DELETE2INSERT]+mat->query->seg[i]->insert[CSEQ_CDNA_CODON(mat->target,j)])) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 0,j - 3,DELETE) )   {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-0,j-3,DELETE);  
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 0,j - 3,DELETE);   
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_INSERT2INSERT]+mat->query->seg[i]->insert[CSEQ_CDNA_CODON(mat->target,j)])) -  (0);  
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 0,j - 3,INSERT) )   {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-0,j-3,INSERT);  
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 0,j - 3,INSERT);   
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_MATCH2INSERT]+mat->query->seg[i]->insert[CSEQ_CDNA_CODON(mat->target,j)])) -  (0);   
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 0,j - 3,MATCH) )    {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-0,j-3,MATCH);   
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 0,j - 3,MATCH);    
          }  
        warn("Major problem (!) - in EstLoop3 read off, position %d,%d state %d no source found!",i,j,state);    
        return (-1); 
      case DELETE :  
        temp = cscore - (mat->query->seg[i]->transition[GW_START2DELETE]) -  (0);    
        if( temp == EstLoop3_EXPL_SPECIAL(mat,i - 1,j - 0,LOOP) )    {  
          *reti = i - 1; 
          *retj = j - 0; 
          *retstate = LOOP;  
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_SPECIAL(mat,i-1,j-0,LOOP);   
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 1,j - 0,LOOP);     
          }  
        temp = cscore - (mat->query->seg[i]->transition[GW_DELETE2DELETE]) -  (0);   
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 1,j - 0,DELETE) )   {  
          *reti = i - 1; 
          *retj = j - 0; 
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-1,j-0,DELETE);  
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 1,j - 0,DELETE);   
          }  
        temp = cscore - (mat->query->seg[i]->transition[GW_INSERT2DELETE]) -  (0);   
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 1,j - 0,INSERT) )   {  
          *reti = i - 1; 
          *retj = j - 0; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-1,j-0,INSERT);  
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 1,j - 0,INSERT);   
          }  
        temp = cscore - (mat->query->seg[i]->transition[GW_MATCH2DELETE]) -  (0);    
        if( temp == EstLoop3_EXPL_MATRIX(mat,i - 1,j - 0,MATCH) )    {  
          *reti = i - 1; 
          *retj = j - 0; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-1,j-0,MATCH);   
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 1,j - 0,MATCH);    
          }  
        warn("Major problem (!) - in EstLoop3 read off, position %d,%d state %d no source found!",i,j,state);    
        return (-1); 
      default:   
...
/* Function:  max_calc_special_EstLoop3(mat,i,j,state,isspecial,reti,retj,retstate,retspecial,cellscore)
 *
 * Descrip: No Description
 *
 * Arg:               mat [UNKN ] Undocumented argument [EstLoop3 *]
 ...
 *
 */
int max_calc_special_EstLoop3(EstLoop3 * mat,int i,int j,int state,boolean isspecial,int * reti,int * retj,int * retstate,boolean * retspecial,int * cellscore) 
{
    register int temp;   
    register int cscore; 
    ...
    *reti = (*retj) = (*retstate) = EstLoop3_READ_OFF_ERROR; 
    ...
    cscore = EstLoop3_EXPL_SPECIAL(mat,i,j,state);   
    switch(state)    { /*switch on special states*/ 
      case LOOP :    
        /* source LOOP is a special */ 
        temp = cscore - (0) - (0);   
        if( temp == EstLoop3_EXPL_SPECIAL(mat,i - 0,j - 1,LOOP) )    {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = LOOP;  
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_SPECIAL(mat,i-0,j-1,LOOP);   
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 0,j - 1,LOOP) ;    
          }  
        ...
        for(i= mat->query->len-1;i >= 0 ;i--)    { /*for i >= 0*/ 
          ...
          if( temp == EstLoop3_EXPL_MATRIX(mat,i - 0,j - 0,DELETE) ) {  
            *reti = i - 0;   
            *retj = j - 0;   
            *retstate = DELETE;  
            *retspecial = FALSE; 
            if( cellscore != NULL)   {  
              *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-0,j-0,DELETE);    
              }  
            return EstLoop3_EXPL_MATRIX(mat,i - 0,j - 0,DELETE) ;    
            }  
          } /* end of for i >= 0 */ 
        /* source INSERT is from main matrix */ 
        for(i= mat->query->len-1;i >= 0 ;i--)    { /*for i >= 0*/ 
          temp = cscore - (mat->query->seg[i]->transition[GW_INSERT2END]) - (0);     
          if( temp == EstLoop3_EXPL_MATRIX(mat,i - 0,j - 0,INSERT) ) {  
            *reti = i - 0;   
            *retj = j - 0;   
            *retstate = INSERT;  
            *retspecial = FALSE; 
            if( cellscore != NULL)   {  
              *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-0,j-0,INSERT);    
              }  
            return EstLoop3_EXPL_MATRIX(mat,i - 0,j - 0,INSERT) ;    
            }  
          } /* end of for i >= 0 */ 
        /* source MATCH is from main matrix */ 
        for(i= mat->query->len-1;i >= 0 ;i--)    { /*for i >= 0*/ 
          temp = cscore - (mat->query->seg[i]->transition[GW_MATCH2END]) - (0);  
          if( temp == EstLoop3_EXPL_MATRIX(mat,i - 0,j - 0,MATCH) )  {  
            *reti = i - 0;   
            *retj = j - 0;   
            *retstate = MATCH;   
            *retspecial = FALSE; 
            if( cellscore != NULL)   {  
              *cellscore = cscore - EstLoop3_EXPL_MATRIX(mat,i-0,j-0,MATCH);     
              }  
            return EstLoop3_EXPL_MATRIX(mat,i - 0,j - 0,MATCH) ;     
            }  
          } /* end of for i >= 0 */ 
        /* source START is a special */ 
        temp = cscore - (0) - (0);   
        if( temp == EstLoop3_EXPL_SPECIAL(mat,i - 0,j - 1,START) )   {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = START; 
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - EstLoop3_EXPL_SPECIAL(mat,i-0,j-1,START);  
            }  
          return EstLoop3_EXPL_MATRIX(mat,i - 0,j - 1,START) ;   
          }  
      case START :   
      case END :     
        /* source DELETE is from main matrix */ 
