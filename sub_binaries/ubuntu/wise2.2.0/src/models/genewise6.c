...
Search_Return_Type search_GeneWise6(DBSearchImpl * dbsi,Hscore * out,GeneWiseDB* querydb,GenomicDB* targetdb ,GeneParser4Score * gp) 
{
    ...
    if( dbsi->trace_level > 5 )  
      warn("Asking for trace level of %d in database search for GeneWise6, but it was compiled with a trace level of 416. Not all trace statements can be shown",dbsi->trace_level); 
...
int score_only_GeneWise6(GeneWiseScore* query,ComplexSequence* target ,GeneParser4Score * gp) 
{
    ...
    for(j=0;j<mat->lenj;j++) { /*for all target positions*/ 
      ...
      for(i=0;i<mat->leni;i++)   { /*for all query positions*/ 
        ...
        temp = GeneWise6_VSMALL_MATRIX(mat,i-1,j-6,INTRON_0) + ((((mat->query->seg[i]->transition[GW_MATCH2MATCH]+mat->gp->transition[GP4_INTRON2CDS])+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS]);     
        if( temp  > score )  {  
          score = temp;  
          }  
        ...
        temp = GeneWise6_VSMALL_MATRIX(mat,i-0,j-6,INTRON_0) + ((((mat->query->seg[i]->transition[GW_INSERT2INSERT]+mat->gp->transition[GP4_INTRON2CDS])+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_INSERT_BALANCE_3SS]);  
        if( temp  > score )  {  
          score = temp;  
          }  
        ...
        /* For state INTRON_0 */ 
        /* setting first movement to score */ 
        score = GeneWise6_VSMALL_MATRIX(mat,i-0,j-8,MATCH) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);     
        /* From state INSERT to state INTRON_0 */ 
        temp = GeneWise6_VSMALL_MATRIX(mat,i-0,j-8,INSERT) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_INSERT_BALANCE_5SS]);    
        if( temp  > score )  {  
          score = temp;  
          }  
...
PackAln * PackAln_bestmemory_GeneWise6(GeneWiseScore* query,ComplexSequence* target ,GeneParser4Score * gp,DPEnvelope * dpenv,DPRunImpl * dpri) 
{
    ...
    if( strategy == DPIM_Linear )    {  
      ...
      if( (mat=allocate_Expl_GeneWise6(query, target , gp)) == NULL )    {  
...
/* Function:  allocate_Expl_GeneWise6(query,target,gp)
 *
 ...
 *
 */
GeneWise6 * allocate_Expl_GeneWise6(GeneWiseScore* query,ComplexSequence* target ,GeneParser4Score * gp) 
{
    GeneWise6 * out; 
...
PackAln * PackAln_calculate_Small_GeneWise6(GeneWise6 * mat,DPEnvelope * dpenv) 
{
    ...
    dc_start_end_calculate_GeneWise6(mat,dpenv); 
...
AlnRangeSet * AlnRangeSet_calculate_Small_GeneWise6(GeneWise6 * mat) 
{
    ...
    start_reporting("Find start end points: ");  
    dc_start_end_calculate_GeneWise6(mat,NULL);  
...
void calculate_hidden_GeneWise6(GeneWise6 * mat,int starti,int startj,int startstate,int stopi,int stopj,int stopstate,DPEnvelope * dpenv) 
{
    ...
    for(j=startj;j<=stopj;j++)   {  
      for(i=starti;i<=stopi;i++) {  
        ...
        /* From state INTRON_0 to state MATCH */ 
        temp = GeneWise6_HIDDEN_MATRIX(mat,i-1,j-6,INTRON_0) + ((((mat->query->seg[i]->transition[GW_MATCH2MATCH]+mat->gp->transition[GP4_INTRON2CDS])+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS]);     
        if( temp  > score )  {  
          score = temp;  
          }  
        /* From state INTRON_1 to state MATCH */ 
        ...
        /* From state INTRON_0 to state INSERT */ 
        temp = GeneWise6_HIDDEN_MATRIX(mat,i-0,j-6,INTRON_0) + ((((mat->query->seg[i]->transition[GW_INSERT2INSERT]+mat->gp->transition[GP4_INTRON2CDS])+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_INSERT_BALANCE_3SS]);  
        if( temp  > score )  {  
          score = temp;  
          }  
        /* From state INTRON_1 to state INSERT */ 
        ...
        /* For state INTRON_0 */ 
        /* setting first movement to score */ 
        score = GeneWise6_HIDDEN_MATRIX(mat,i-0,j-8,MATCH) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);     
        /* From state INSERT to state INTRON_0 */ 
        temp = GeneWise6_HIDDEN_MATRIX(mat,i-0,j-8,INSERT) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_INSERT_BALANCE_5SS]);    
        if( temp  > score )  {  
          score = temp;  
          }  
        /* From state INTRON_0 to state INTRON_0 */ 
...
void push_dc_at_merge_GeneWise6(GeneWise6 * mat,int starti,int stopi,int startj,int * stopj,DPEnvelope * dpenv) 
{
    ...
    for(count=0,j=startj;count<10;count++,j++)   {  
      for(i=starti;i<=stopi;i++) {  
        ...
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-1,j-6,INTRON_0) + ((((mat->query->seg[i]->transition[GW_MATCH2MATCH]+mat->gp->transition[GP4_INTRON2CDS])+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS]);  
        if( temp > score)    {  
          score = temp;  
        ...
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-0,j-6,INTRON_0) + ((((mat->query->seg[i]->transition[GW_INSERT2INSERT]+mat->gp->transition[GP4_INTRON2CDS])+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_INSERT_BALANCE_3SS]);   
        if( temp > score)    {  
          score = temp;  
        ...
        score = GeneWise6_DC_SHADOW_MATRIX(mat,i-0,j-8,MATCH) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);  
        if( j - 8 <= mergej) {  
        ...
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-0,j-8,INSERT) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_INSERT_BALANCE_5SS]);     
        if( temp > score)    {  
          score = temp;  
...
/* Function:  dc_start_end_calculate_GeneWise6(mat,dpenv)
 *
 ...
 *
 */
boolean dc_start_end_calculate_GeneWise6(GeneWise6 * mat,DPEnvelope * dpenv) 
{
    int i;   
    int j;   
    int k;   
    ...
    init_start_end_linear_GeneWise6(mat);    
    ...
    for(j=0;j<lenj;j++)  { /*for each j strip*/ 
      for(i=0;i<leni;i++)    { /*for each i position in strip*/ 
        ...
        /* For state MATCH */ 
        /* setting first movement to score */ 
        score = GeneWise6_DC_SHADOW_MATRIX(mat,i-1,j-3,MATCH) + (mat->query->seg[i]->transition[GW_MATCH2MATCH]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)]) + (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 3,MATCH,k);   
        /* From state INSERT to state MATCH */ 
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-1,j-3,INSERT) + (mat->query->seg[i]->transition[GW_INSERT2MATCH]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)]) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 3,INSERT,k);    
        ...
        /* From state DELETE to state MATCH */ 
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-1,j-3,DELETE) + (mat->query->seg[i]->transition[GW_DELETE2MATCH]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)]) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 3,DELETE,k);    
        ...
        /* From state START to state MATCH */ 
        temp = GeneWise6_DC_SHADOW_SPECIAL(mat,i-1,j-3,START) + (mat->query->seg[i]->transition[GW_START2MATCH]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)]) + (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp  > score )  {  
          score = temp;  
        ...
        /* From state INTRON_0 to state MATCH */ 
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-1,j-6,INTRON_0) + ((((mat->query->seg[i]->transition[GW_MATCH2MATCH]+mat->gp->transition[GP4_INTRON2CDS])+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS]) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 6,INTRON_0,k);  
        ...
        /* From state INTRON_1 to state MATCH */ 
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-1,j-5,INTRON_1) + ((mat->query->seg[i]->transition[GW_MATCH2MATCH]+mat->gp->transition[GP4_INTRON2CDS])+CSEQ_GENOMIC_3SS(mat->target,(j-2))) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 5,INTRON_1,k);  
        ...
        /* From state INTRON_2 to state MATCH */ 
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-1,j-4,INTRON_2) + ((mat->query->seg[i]->transition[GW_MATCH2MATCH]+mat->gp->transition[GP4_INTRON2CDS])+CSEQ_GENOMIC_3SS(mat->target,(j-1))) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 4,INTRON_2,k);  
        ...
        /* From state MATCH to state MATCH */ 
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-1,j-2,MATCH) + mat->gp->transition[GP4_DELETE_1_BASE] +(CSEQ_GENOMIC_CDSPOT(mat->target,j));     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 2,MATCH,k); 
        ...
        /* From state MATCH to state MATCH */ 
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-1,j-1,MATCH) + mat->gp->transition[GP4_DELETE_2_BASE] +(CSEQ_GENOMIC_CDSPOT(mat->target,j));     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 1,MATCH,k); 
        ...
        /* From state MATCH to state MATCH */ 
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-1,j-4,MATCH) + mat->gp->transition[GP4_INSERT_1_BASE] +(CSEQ_GENOMIC_CDSPOT(mat->target,j));     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 4,MATCH,k); 
        ...
        /* From state MATCH to state MATCH */ 
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-1,j-5,MATCH) + mat->gp->transition[GP4_INSERT_2_BASE] +(CSEQ_GENOMIC_CDSPOT(mat->target,j));     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 5,MATCH,k); 
        ...
        /* state MATCH is a source for special END */ 
        temp = score + (mat->query->seg[i]->transition[GW_MATCH2END]) + (0) ;    
        if( temp > GeneWise6_DC_SHADOW_SPECIAL(mat,i,j,END) )    {  
          GeneWise6_DC_SHADOW_SPECIAL(mat,i,j,END) = temp;   
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            GeneWise6_DC_SHADOW_SPECIAL_SP(mat,i,j,END,k) = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i,j,MATCH,k);  
          GeneWise6_DC_SHADOW_SPECIAL_SP(mat,i,j,END,6) = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i,j,MATCH,6);    
          GeneWise6_DC_SHADOW_SPECIAL_SP(mat,i,j,END,3) = i; 
          GeneWise6_DC_SHADOW_SPECIAL_SP(mat,i,j,END,4) = j; 
          GeneWise6_DC_SHADOW_SPECIAL_SP(mat,i,j,END,5) = MATCH; 
          }  
        ...
        /* For state INSERT */ 
        /* setting first movement to score */ 
        score = GeneWise6_DC_SHADOW_MATRIX(mat,i-0,j-3,MATCH) + (mat->query->seg[i]->transition[GW_MATCH2INSERT]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)]) + (CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 3,MATCH,k);   
        /* From state INSERT to state INSERT */ 
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-0,j-3,INSERT) + (mat->query->seg[i]->transition[GW_INSERT2INSERT]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)]) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 3,INSERT,k);    
        ...
        /* From state DELETE to state INSERT */ 
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-0,j-3,DELETE) + (mat->query->seg[i]->transition[GW_DELETE2INSERT]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)]) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 3,DELETE,k);    
        ...
        /* From state START to state INSERT */ 
        temp = GeneWise6_DC_SHADOW_SPECIAL(mat,i-0,j-3,START) + (mat->query->seg[i]->transition[GW_START2INSERT]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)]) + (CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp  > score )  {  
          score = temp;  
        ...
        /* From state INTRON_0 to state INSERT */ 
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-0,j-6,INTRON_0) + ((((mat->query->seg[i]->transition[GW_INSERT2INSERT]+mat->gp->transition[GP4_INTRON2CDS])+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_INSERT_BALANCE_3SS]) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 6,INTRON_0,k);  
        ...
        /* From state INTRON_1 to state INSERT */ 
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-0,j-5,INTRON_1) + ((mat->query->seg[i]->transition[GW_INSERT2INSERT]+mat->gp->transition[GP4_INTRON2CDS])+CSEQ_GENOMIC_3SS(mat->target,(j-2))) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 5,INTRON_1,k);  
        ...
        /* From state INTRON_2 to state INSERT */ 
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-0,j-4,INTRON_2) + ((mat->query->seg[i]->transition[GW_INSERT2INSERT]+mat->gp->transition[GP4_INTRON2CDS])+CSEQ_GENOMIC_3SS(mat->target,(j-1))) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 4,INTRON_2,k);  
        ...
        /* From state INSERT to state INSERT */ 
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-0,j-2,INSERT) + mat->gp->transition[GP4_DELETE_1_BASE] +(CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 2,INSERT,k);    
        ...
        /* From state INSERT to state INSERT */ 
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-0,j-1,INSERT) + mat->gp->transition[GP4_DELETE_2_BASE] +(CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,INSERT,k);    
        ...
        if( temp > GeneWise6_DC_SHADOW_SPECIAL(mat,i,j,END) )    {  
          ...
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            GeneWise6_DC_SHADOW_SPECIAL_SP(mat,i,j,END,k) = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i,j,INSERT,k); 
          GeneWise6_DC_SHADOW_SPECIAL_SP(mat,i,j,END,6) = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i,j,INSERT,6);   
          GeneWise6_DC_SHADOW_SPECIAL_SP(mat,i,j,END,3) = i; 
          GeneWise6_DC_SHADOW_SPECIAL_SP(mat,i,j,END,4) = j; 
          GeneWise6_DC_SHADOW_SPECIAL_SP(mat,i,j,END,5) = INSERT;    
          }  
        ...
        /* For state DELETE */ 
        /* setting first movement to score */ 
        score = GeneWise6_DC_SHADOW_MATRIX(mat,i-1,j-0,MATCH) + mat->query->seg[i]->transition[GW_MATCH2DELETE] + (0);   
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 0,MATCH,k);   
        /* From state INSERT to state DELETE */ 
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-1,j-0,INSERT) + mat->query->seg[i]->transition[GW_INSERT2DELETE] +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 0,INSERT,k);    
        ...
        /* From state DELETE to state DELETE */ 
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-1,j-0,DELETE) + mat->query->seg[i]->transition[GW_DELETE2DELETE] +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 0,DELETE,k);    
        ...
        /* From state START to state DELETE */ 
        temp = GeneWise6_DC_SHADOW_SPECIAL(mat,i-1,j-0,START) + mat->query->seg[i]->transition[GW_START2DELETE] + (0);   
        if( temp  > score )  {  
          score = temp;  
        ...
        if( temp > GeneWise6_DC_SHADOW_SPECIAL(mat,i,j,END) )    {  
          ...
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            GeneWise6_DC_SHADOW_SPECIAL_SP(mat,i,j,END,k) = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i,j,DELETE,k); 
          GeneWise6_DC_SHADOW_SPECIAL_SP(mat,i,j,END,6) = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i,j,DELETE,6);   
          GeneWise6_DC_SHADOW_SPECIAL_SP(mat,i,j,END,3) = i; 
          GeneWise6_DC_SHADOW_SPECIAL_SP(mat,i,j,END,4) = j; 
          GeneWise6_DC_SHADOW_SPECIAL_SP(mat,i,j,END,5) = DELETE;    
          }  
        ...
        /* For state INTRON_0 */ 
        /* setting first movement to score */ 
        score = GeneWise6_DC_SHADOW_MATRIX(mat,i-0,j-8,MATCH) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]) + (0);    
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 8,MATCH,k);   
        /* From state INSERT to state INTRON_0 */ 
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-0,j-8,INSERT) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_INSERT_BALANCE_5SS]) +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 8,INSERT,k);    
        ...
        /* From state INTRON_0 to state INTRON_0 */ 
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-0,j-1,INTRON_0) + (mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp->transition[GP4_INTRON2INTRON]) +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,INTRON_0,k);  
        ...
        /* For state INTRON_1 */ 
        /* setting first movement to score */ 
        score = GeneWise6_DC_SHADOW_MATRIX(mat,i-0,j-9,MATCH) + (mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7))) + (0);   
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 9,MATCH,k);   
        /* From state INSERT to state INTRON_1 */ 
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-0,j-9,INSERT) + (mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7))) +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 9,INSERT,k);    
        ...
        /* From state INTRON_1 to state INTRON_1 */ 
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-0,j-1,INTRON_1) + (mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp->transition[GP4_INTRON2INTRON]) +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,INTRON_1,k);  
        ...
        /* For state INTRON_2 */ 
        /* setting first movement to score */ 
        score = GeneWise6_DC_SHADOW_MATRIX(mat,i-0,j-10,MATCH) + (mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7))) + (0);  
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 10,MATCH,k);  
        /* From state INSERT to state INTRON_2 */ 
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-0,j-10,INSERT) + (mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7))) +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 10,INSERT,k);   
        ...
        /* From state INTRON_2 to state INTRON_2 */ 
        temp = GeneWise6_DC_SHADOW_MATRIX(mat,i-0,j-1,INTRON_2) + (mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp->transition[GP4_INTRON2INTRON]) +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,INTRON_2,k);  
...
PackAln * PackAln_read_Expl_GeneWise6(GeneWise6 * mat) 
{
    ...
    if( mat->basematrix->type != BASEMATRIX_TYPE_EXPLICIT)   {  
      warn("In GeneWise6_basic_read You have asked for an alignment from a non-explicit matrix: c'est impossible [current type is %d - %s]", mat->basematrix->type,basematrix_type_to_string(mat->basematrix->type));    
      return NULL;   
      }  
    ...
    out->score =  find_end_GeneWise6(mat,&i,&j,&state,&isspecial);   
    ...
    while( state != START || isspecial != TRUE)  { /*while state != START*/ 
      ...
      if( isspecial == TRUE )    
        max_calc_special_GeneWise6(mat,i,j,state,isspecial,&i,&j,&state,&isspecial,&cellscore);  
      else   
        max_calc_GeneWise6(mat,i,j,state,isspecial,&i,&j,&state,&isspecial,&cellscore);  
...
/* Function:  find_end_GeneWise6(mat,ri,rj,state,isspecial)
 *
 * Descrip: No Description
 *
 * Arg:              mat [UNKN ] Undocumented argument [GeneWise6 *]
 ...
 *
 */
int find_end_GeneWise6(GeneWise6 * mat,int * ri,int * rj,int * state,boolean * isspecial) 
{
    register int j;  
    register int max;    
    register int maxj;   
    ...
    max = GeneWise6_EXPL_SPECIAL(mat,0,mat->target->seq->len-1,END); 
    maxj = mat->target->seq->len-1;  
    for(j= mat->target->seq->len-2 ;j >= 0 ;j--) {  
      if( GeneWise6_EXPL_SPECIAL(mat,0,j,END) > max )    {  
        max = GeneWise6_EXPL_SPECIAL(mat,0,j,END);   
        maxj = j;    
        }  
      }  
...
/* Function:  max_calc_GeneWise6(mat,i,j,state,isspecial,reti,retj,retstate,retspecial,cellscore)
 *
 * Descrip: No Description
 *
 * Arg:               mat [UNKN ] Undocumented argument [GeneWise6 *]
 ...
 *
 */
int max_calc_GeneWise6(GeneWise6 * mat,int i,int j,int state,boolean isspecial,int * reti,int * retj,int * retstate,boolean * retspecial,int * cellscore) 
{
    register int temp;   
    register int cscore; 
    ...
    *reti = (*retj) = (*retstate) = GeneWise6_READ_OFF_ERROR;    
    ...
    /* Then you have to select the correct switch statement to figure out the readoff      */ 
    /* Somewhat odd - reverse the order of calculation and return as soon as it is correct */ 
    cscore = GeneWise6_EXPL_MATRIX(mat,i,j,state);   
    switch(state)    { /*Switch state */ 
      case MATCH :   
        temp = cscore - (mat->gp->transition[GP4_INSERT_2_BASE]) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 1,j - 5,MATCH) )   {  
          *reti = i - 1; 
          *retj = j - 5; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-1,j-5,MATCH);  
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 1,j - 5,MATCH);   
          }  
        temp = cscore - (mat->gp->transition[GP4_INSERT_1_BASE]) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 1,j - 4,MATCH) )   {  
          *reti = i - 1; 
          *retj = j - 4; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-1,j-4,MATCH);  
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 1,j - 4,MATCH);   
          }  
        temp = cscore - (mat->gp->transition[GP4_DELETE_2_BASE]) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 1,j - 1,MATCH) )   {  
          *reti = i - 1; 
          *retj = j - 1; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-1,j-1,MATCH);  
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 1,j - 1,MATCH);   
          }  
        temp = cscore - (mat->gp->transition[GP4_DELETE_1_BASE]) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 1,j - 2,MATCH) )   {  
          *reti = i - 1; 
          *retj = j - 2; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-1,j-2,MATCH);  
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 1,j - 2,MATCH);   
          }  
        temp = cscore - (((mat->query->seg[i]->transition[GW_MATCH2MATCH]+mat->gp->transition[GP4_INTRON2CDS])+CSEQ_GENOMIC_3SS(mat->target,(j-1)))) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 1,j - 4,INTRON_2) )    {  
          *reti = i - 1; 
          *retj = j - 4; 
          *retstate = INTRON_2;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-1,j-4,INTRON_2);   
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 1,j - 4,INTRON_2);    
          }  
        temp = cscore - (((mat->query->seg[i]->transition[GW_MATCH2MATCH]+mat->gp->transition[GP4_INTRON2CDS])+CSEQ_GENOMIC_3SS(mat->target,(j-2)))) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 1,j - 5,INTRON_1) )    {  
          *reti = i - 1; 
          *retj = j - 5; 
          *retstate = INTRON_1;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-1,j-5,INTRON_1);   
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 1,j - 5,INTRON_1);    
          }  
        temp = cscore - (((((mat->query->seg[i]->transition[GW_MATCH2MATCH]+mat->gp->transition[GP4_INTRON2CDS])+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 1,j - 6,INTRON_0) )    {  
          *reti = i - 1; 
          *retj = j - 6; 
          *retstate = INTRON_0;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-1,j-6,INTRON_0);   
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 1,j - 6,INTRON_0);    
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_START2MATCH]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j)); 
        if( temp == GeneWise6_EXPL_SPECIAL(mat,i - 1,j - 3,START) )  {  
          *reti = i - 1; 
          *retj = j - 3; 
          *retstate = START; 
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_SPECIAL(mat,i-1,j-3,START); 
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 1,j - 3,START);   
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_DELETE2MATCH]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 1,j - 3,DELETE) )  {  
          *reti = i - 1; 
          *retj = j - 3; 
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-1,j-3,DELETE); 
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 1,j - 3,DELETE);  
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_INSERT2MATCH]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 1,j - 3,INSERT) )  {  
          *reti = i - 1; 
          *retj = j - 3; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-1,j-3,INSERT); 
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 1,j - 3,INSERT);  
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_MATCH2MATCH]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j)); 
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 1,j - 3,MATCH) )   {  
          *reti = i - 1; 
          *retj = j - 3; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-1,j-3,MATCH);  
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 1,j - 3,MATCH);   
          }  
        warn("Major problem (!) - in GeneWise6 read off, position %d,%d state %d no source found!",i,j,state);   
        return (-1); 
      case INSERT :  
        temp = cscore - (mat->gp->transition[GP4_DELETE_2_BASE]) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 0,j - 1,INSERT) )  {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-0,j-1,INSERT); 
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 0,j - 1,INSERT);  
          }  
        temp = cscore - (mat->gp->transition[GP4_DELETE_1_BASE]) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 0,j - 2,INSERT) )  {  
          *reti = i - 0; 
          *retj = j - 2; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-0,j-2,INSERT); 
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 0,j - 2,INSERT);  
          }  
        temp = cscore - (((mat->query->seg[i]->transition[GW_INSERT2INSERT]+mat->gp->transition[GP4_INTRON2CDS])+CSEQ_GENOMIC_3SS(mat->target,(j-1)))) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 0,j - 4,INTRON_2) )    {  
          *reti = i - 0; 
          *retj = j - 4; 
          *retstate = INTRON_2;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-0,j-4,INTRON_2);   
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 0,j - 4,INTRON_2);    
          }  
        temp = cscore - (((mat->query->seg[i]->transition[GW_INSERT2INSERT]+mat->gp->transition[GP4_INTRON2CDS])+CSEQ_GENOMIC_3SS(mat->target,(j-2)))) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 0,j - 5,INTRON_1) )    {  
          *reti = i - 0; 
          *retj = j - 5; 
          *retstate = INTRON_1;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-0,j-5,INTRON_1);   
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 0,j - 5,INTRON_1);    
          }  
        temp = cscore - (((((mat->query->seg[i]->transition[GW_INSERT2INSERT]+mat->gp->transition[GP4_INTRON2CDS])+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_INSERT_BALANCE_3SS])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));   
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 0,j - 6,INTRON_0) )    {  
          *reti = i - 0; 
          *retj = j - 6; 
          *retstate = INTRON_0;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-0,j-6,INTRON_0);   
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 0,j - 6,INTRON_0);    
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_START2INSERT]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));   
        if( temp == GeneWise6_EXPL_SPECIAL(mat,i - 0,j - 3,START) )  {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = START; 
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_SPECIAL(mat,i-0,j-3,START); 
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 0,j - 3,START);   
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_DELETE2INSERT]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 0,j - 3,DELETE) )  {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-0,j-3,DELETE); 
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 0,j - 3,DELETE);  
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_INSERT2INSERT]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 0,j - 3,INSERT) )  {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-0,j-3,INSERT); 
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 0,j - 3,INSERT);  
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_MATCH2INSERT]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));   
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 0,j - 3,MATCH) )   {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-0,j-3,MATCH);  
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 0,j - 3,MATCH);   
          }  
        warn("Major problem (!) - in GeneWise6 read off, position %d,%d state %d no source found!",i,j,state);   
        return (-1); 
      case DELETE :  
        temp = cscore - (mat->query->seg[i]->transition[GW_START2DELETE]) -  (0);    
        if( temp == GeneWise6_EXPL_SPECIAL(mat,i - 1,j - 0,START) )  {  
          *reti = i - 1; 
          *retj = j - 0; 
          *retstate = START; 
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_SPECIAL(mat,i-1,j-0,START); 
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 1,j - 0,START);   
          }  
        temp = cscore - (mat->query->seg[i]->transition[GW_DELETE2DELETE]) -  (0);   
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 1,j - 0,DELETE) )  {  
          *reti = i - 1; 
          *retj = j - 0; 
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-1,j-0,DELETE); 
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 1,j - 0,DELETE);  
          }  
        temp = cscore - (mat->query->seg[i]->transition[GW_INSERT2DELETE]) -  (0);   
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 1,j - 0,INSERT) )  {  
          *reti = i - 1; 
          *retj = j - 0; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-1,j-0,INSERT); 
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 1,j - 0,INSERT);  
          }  
        temp = cscore - (mat->query->seg[i]->transition[GW_MATCH2DELETE]) -  (0);    
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 1,j - 0,MATCH) )   {  
          *reti = i - 1; 
          *retj = j - 0; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-1,j-0,MATCH);  
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 1,j - 0,MATCH);   
          }  
        warn("Major problem (!) - in GeneWise6 read off, position %d,%d state %d no source found!",i,j,state);   
        return (-1); 
      case INTRON_0 :    
        temp = cscore - ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp->transition[GP4_INTRON2INTRON])) -  (0); 
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 0,j - 1,INTRON_0) )    {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = INTRON_0;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-0,j-1,INTRON_0);   
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 0,j - 1,INTRON_0);    
          }  
        temp = cscore - (((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_INSERT_BALANCE_5SS])) -  (0);    
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 0,j - 8,INSERT) )  {  
          *reti = i - 0; 
          *retj = j - 8; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-0,j-8,INSERT); 
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 0,j - 8,INSERT);  
          }  
        temp = cscore - (((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])) -  (0); 
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 0,j - 8,MATCH) )   {  
          *reti = i - 0; 
          *retj = j - 8; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-0,j-8,MATCH);  
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 0,j - 8,MATCH);   
          }  
        warn("Major problem (!) - in GeneWise6 read off, position %d,%d state %d no source found!",i,j,state);   
        return (-1); 
      case INTRON_1 :    
        temp = cscore - ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp->transition[GP4_INTRON2INTRON])) -  (0); 
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 0,j - 1,INTRON_1) )    {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = INTRON_1;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-0,j-1,INTRON_1);   
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 0,j - 1,INTRON_1);    
          }  
        temp = cscore - ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))) -  (0);    
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 0,j - 9,INSERT) )  {  
          *reti = i - 0; 
          *retj = j - 9; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-0,j-9,INSERT); 
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 0,j - 9,INSERT);  
          }  
        temp = cscore - ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))) -  (0);    
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 0,j - 9,MATCH) )   {  
          *reti = i - 0; 
          *retj = j - 9; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-0,j-9,MATCH);  
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 0,j - 9,MATCH);   
          }  
        warn("Major problem (!) - in GeneWise6 read off, position %d,%d state %d no source found!",i,j,state);   
        return (-1); 
      case INTRON_2 :    
        temp = cscore - ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp->transition[GP4_INTRON2INTRON])) -  (0); 
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 0,j - 1,INTRON_2) )    {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = INTRON_2;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-0,j-1,INTRON_2);   
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 0,j - 1,INTRON_2);    
          }  
        temp = cscore - ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))) -  (0);    
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 0,j - 10,INSERT) ) {  
          *reti = i - 0; 
          *retj = j - 10;    
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-0,j-10,INSERT);    
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 0,j - 10,INSERT);     
          }  
        temp = cscore - ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))) -  (0);    
        if( temp == GeneWise6_EXPL_MATRIX(mat,i - 0,j - 10,MATCH) )  {  
          *reti = i - 0; 
          *retj = j - 10;    
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-0,j-10,MATCH); 
            }  
          return GeneWise6_EXPL_MATRIX(mat,i - 0,j - 10,MATCH);  
          }  
        warn("Major problem (!) - in GeneWise6 read off, position %d,%d state %d no source found!",i,j,state);   
        return (-1); 
      default:   
...
/* Function:  max_calc_special_GeneWise6(mat,i,j,state,isspecial,reti,retj,retstate,retspecial,cellscore)
 *
 * Descrip: No Description
 *
 * Arg:               mat [UNKN ] Undocumented argument [GeneWise6 *]
 ...
 *
 */
int max_calc_special_GeneWise6(GeneWise6 * mat,int i,int j,int state,boolean isspecial,int * reti,int * retj,int * retstate,boolean * retspecial,int * cellscore) 
{
    register int temp;   
    register int cscore; 
    ...
    *reti = (*retj) = (*retstate) = GeneWise6_READ_OFF_ERROR;    
    ...
    cscore = GeneWise6_EXPL_SPECIAL(mat,i,j,state);  
    switch(state)    { /*switch on special states*/ 
      case START :   
      case END :     
        ...
        for(i= mat->query->len-1;i >= 0 ;i--)    { /*for i >= 0*/ 
          ...
          if( temp == GeneWise6_EXPL_MATRIX(mat,i - 0,j - 0,DELETE) )    {  
            *reti = i - 0;   
            *retj = j - 0;   
            *retstate = DELETE;  
            *retspecial = FALSE; 
            if( cellscore != NULL)   {  
              *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-0,j-0,DELETE);   
              }  
            return GeneWise6_EXPL_MATRIX(mat,i - 0,j - 0,DELETE) ;   
            }  
          } /* end of for i >= 0 */ 
        /* source INSERT is from main matrix */ 
        for(i= mat->query->len-1;i >= 0 ;i--)    { /*for i >= 0*/ 
          temp = cscore - (mat->query->seg[i]->transition[GW_INSERT2END]) - (0);     
          if( temp == GeneWise6_EXPL_MATRIX(mat,i - 0,j - 0,INSERT) )    {  
            *reti = i - 0;   
            *retj = j - 0;   
            *retstate = INSERT;  
            *retspecial = FALSE; 
            if( cellscore != NULL)   {  
              *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-0,j-0,INSERT);   
              }  
            return GeneWise6_EXPL_MATRIX(mat,i - 0,j - 0,INSERT) ;   
            }  
          } /* end of for i >= 0 */ 
        /* source MATCH is from main matrix */ 
        for(i= mat->query->len-1;i >= 0 ;i--)    { /*for i >= 0*/ 
          temp = cscore - (mat->query->seg[i]->transition[GW_MATCH2END]) - (0);  
          if( temp == GeneWise6_EXPL_MATRIX(mat,i - 0,j - 0,MATCH) ) {  
            *reti = i - 0;   
            *retj = j - 0;   
            *retstate = MATCH;   
            *retspecial = FALSE; 
            if( cellscore != NULL)   {  
              *cellscore = cscore - GeneWise6_EXPL_MATRIX(mat,i-0,j-0,MATCH);    
              }  
            return GeneWise6_EXPL_MATRIX(mat,i - 0,j - 0,MATCH) ;    
            }  
          } /* end of for i >= 0 */ 
      default:   
...
boolean calculate_GeneWise6(GeneWise6 * mat) 
{
    ...
    for(j=0;j<lenj;j++)  {  
      ...
      for(i=0;i<leni;i++)    {  
        ...
        /* From state INTRON_0 to state MATCH */ 
        temp = GeneWise6_EXPL_MATRIX(mat,i-1,j-6,INTRON_0) + ((((mat->query->seg[i]->transition[GW_MATCH2MATCH]+mat->gp->transition[GP4_INTRON2CDS])+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS]);   
        if( temp  > score )  {  
          score = temp;  
          }  
        /* From state INTRON_1 to state MATCH */ 
        ...
        /* From state INTRON_0 to state INSERT */ 
        temp = GeneWise6_EXPL_MATRIX(mat,i-0,j-6,INTRON_0) + ((((mat->query->seg[i]->transition[GW_INSERT2INSERT]+mat->gp->transition[GP4_INTRON2CDS])+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_INSERT_BALANCE_3SS]);    
        if( temp  > score )  {  
          score = temp;  
          }  
        /* From state INTRON_1 to state INSERT */ 
        ...
        /* For state INTRON_0 */ 
        /* setting first movement to score */ 
        score = GeneWise6_EXPL_MATRIX(mat,i-0,j-8,MATCH) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);   
        /* From state INSERT to state INTRON_0 */ 
        temp = GeneWise6_EXPL_MATRIX(mat,i-0,j-8,INSERT) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_INSERT_BALANCE_5SS]);  
        if( temp  > score )  {  
          score = temp;  
          }  
        /* From state INTRON_0 to state INTRON_0 */ 
