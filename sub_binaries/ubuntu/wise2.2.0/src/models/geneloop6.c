...
/* Function:  search_GeneLoop6(dbsi,out,query,targetdb,gp)
 *
 ...
 *
 */
Search_Return_Type search_GeneLoop6(DBSearchImpl * dbsi,Hscore * out,GeneWiseScore * query,GenomicDB* targetdb ,GeneParser4Score * gp) 
{
#ifdef PTHREAD   
    int i;   
    int thr_no;  
    ...
    if( dbsi->trace_level > 5 )  
      warn("Asking for trace level of %d in database search for GeneLoop6, but it was compiled with a trace level of 416. Not all trace statements can be shown",dbsi->trace_level); 
    switch(dbsi->type)   { /*switch on implementation*/ 
      case DBSearchImpl_Serial : 
        return serial_search_GeneLoop6(out,query, targetdb ,gp); 
...
/* Function:  serial_search_GeneLoop6(out,query,targetdb,gp)
 *
 * Descrip:    This function makes a database search of GeneLoop6
 ...
 *
 */
Search_Return_Type serial_search_GeneLoop6(Hscore * out,GeneWiseScore * query,GenomicDB* targetdb ,GeneParser4Score * gp) 
{
...
int score_only_GeneLoop6(GeneWiseScore * query,ComplexSequence* target ,GeneParser4Score * gp) 
{
    ...
    for(j=0;j<mat->lenj;j++) { /*for all target positions*/ 
      ...
      for(i=0;i<mat->leni;i++)   { /*for all query positions*/ 
        ...
        temp = GeneLoop6_VSMALL_MATRIX(mat,i-1,j-6,INTRON_0) + (((mat->gp->transition[GP4_INTRON2CDS]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS]);  
        if( temp  > score )  {  
          score = temp;  
          }  
        ...
        temp = GeneLoop6_VSMALL_MATRIX(mat,i-0,j-6,INTRON_0) + (((mat->gp->transition[GP4_INTRON2CDS]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS]);  
        if( temp  > score )  {  
          score = temp;  
          }  
        ...
        /* For state INTRON_0 */ 
        /* setting first movement to score */ 
        score = GeneLoop6_VSMALL_MATRIX(mat,i-0,j-8,MATCH) + (((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);  
        /* From state INSERT to state INTRON_0 */ 
        temp = GeneLoop6_VSMALL_MATRIX(mat,i-0,j-8,INSERT) + (((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);  
        if( temp  > score )  {  
          score = temp;  
          }  
        ...
        /* For state INTRON_1 */ 
        /* setting first movement to score */ 
        score = GeneLoop6_VSMALL_MATRIX(mat,i-0,j-9,MATCH) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);     
        /* From state INSERT to state INTRON_1 */ 
        temp = GeneLoop6_VSMALL_MATRIX(mat,i-0,j-9,INSERT) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);     
        if( temp  > score )  {  
          score = temp;  
          }  
        ...
        /* For state INTRON_2 */ 
        /* setting first movement to score */ 
        score = GeneLoop6_VSMALL_MATRIX(mat,i-0,j-10,MATCH) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);    
        /* From state INSERT to state INTRON_2 */ 
        temp = GeneLoop6_VSMALL_MATRIX(mat,i-0,j-10,INSERT) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);    
        if( temp  > score )  {  
          score = temp;  
          }  
...
PackAln * PackAln_bestmemory_GeneLoop6(GeneWiseScore * query,ComplexSequence* target ,GeneParser4Score * gp,DPEnvelope * dpenv,DPRunImpl * dpri) 
{
    ...
    if( strategy == DPIM_Linear )    {  
      ...
      if( (mat=allocate_Expl_GeneLoop6(query, target , gp)) == NULL )    {  
...
/* Function:  allocate_Expl_GeneLoop6(query,target,gp)
 *
 ...
 *
 */
GeneLoop6 * allocate_Expl_GeneLoop6(GeneWiseScore * query,ComplexSequence* target ,GeneParser4Score * gp) 
{
    GeneLoop6 * out; 
      ...
      switch(pau->state) {  
        case MATCH :     
          ...
          if( offi == 1 && offj == 6 && prev->state == INTRON_0 )    {  
            pau->score = (((mat->gp->transition[GP4_INTRON2CDS]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS]) + (CSEQ_GENOMIC_CDSPOT(mat->target,j));     
            continue;    
            }  
        ...
        case INTRON_0 :  
          if( offi == 0 && offj == 8 && prev->state == MATCH )   {  
            pau->score = (((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]) + (0);    
            continue;    
            }  
...
PackAln * PackAln_calculate_Small_GeneLoop6(GeneLoop6 * mat,DPEnvelope * dpenv) 
{
    ...
    dc_start_end_calculate_GeneLoop6(mat,dpenv); 
...
AlnRangeSet * AlnRangeSet_calculate_Small_GeneLoop6(GeneLoop6 * mat) 
{
    ...
    start_reporting("Find start end points: ");  
    dc_start_end_calculate_GeneLoop6(mat,NULL);  
    ...
    *reti = (*retj) = (*retstate) = GeneLoop6_READ_OFF_ERROR;    
    ...
    switch(state)    { /*Switch state */ 
      case MATCH :   
        ...
        temp = cscore - ((((mat->gp->transition[GP4_INTRON2CDS]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));   
      ...
      case INTRON_0 :    
        ...
        temp = cscore - ((((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])) -  (0);  
...
void calculate_hidden_GeneLoop6(GeneLoop6 * mat,int starti,int startj,int startstate,int stopi,int stopj,int stopstate,DPEnvelope * dpenv) 
{
    ...
    for(j=startj;j<=stopj;j++)   {  
      for(i=starti;i<=stopi;i++) {  
        ...
        /* From state INTRON_0 to state MATCH */ 
        temp = GeneLoop6_HIDDEN_MATRIX(mat,i-1,j-6,INTRON_0) + (((mat->gp->transition[GP4_INTRON2CDS]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS]);  
        if( temp  > score )  {  
          score = temp;  
          }  
        /* From state INTRON_1 to state MATCH */ 
        ...
        /* From state INTRON_0 to state INSERT */ 
        temp = GeneLoop6_HIDDEN_MATRIX(mat,i-0,j-6,INTRON_0) + (((mat->gp->transition[GP4_INTRON2CDS]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS]);  
        if( temp  > score )  {  
          score = temp;  
          }  
        /* From state INTRON_1 to state INSERT */ 
        ...
        /* For state INTRON_0 */ 
        /* setting first movement to score */ 
        score = GeneLoop6_HIDDEN_MATRIX(mat,i-0,j-8,MATCH) + (((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);  
        /* From state INSERT to state INTRON_0 */ 
        temp = GeneLoop6_HIDDEN_MATRIX(mat,i-0,j-8,INSERT) + (((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);  
        if( temp  > score )  {  
          score = temp;  
          }  
        /* From state INTRON_0 to state INTRON_0 */ 
        ...
        /* For state INTRON_1 */ 
        /* setting first movement to score */ 
        score = GeneLoop6_HIDDEN_MATRIX(mat,i-0,j-9,MATCH) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);     
        /* From state INSERT to state INTRON_1 */ 
        temp = GeneLoop6_HIDDEN_MATRIX(mat,i-0,j-9,INSERT) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);     
        if( temp  > score )  {  
          score = temp;  
          }  
        /* From state INTRON_1 to state INTRON_1 */ 
        ...
        /* For state INTRON_2 */ 
        /* setting first movement to score */ 
        score = GeneLoop6_HIDDEN_MATRIX(mat,i-0,j-10,MATCH) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);    
        /* From state INSERT to state INTRON_2 */ 
        temp = GeneLoop6_HIDDEN_MATRIX(mat,i-0,j-10,INSERT) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);    
        if( temp  > score )  {  
          score = temp;  
          }  
        /* From state INTRON_2 to state INTRON_2 */ 
...
void push_dc_at_merge_GeneLoop6(GeneLoop6 * mat,int starti,int stopi,int startj,int * stopj,DPEnvelope * dpenv) 
{
    ...
    for(count=0,j=startj;count<10;count++,j++)   {  
      for(i=starti;i<=stopi;i++) {  
        ...
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-1,j-6,INTRON_0) + (((mat->gp->transition[GP4_INTRON2CDS]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS]);   
        if( temp > score)    {  
          score = temp;  
        ...
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-0,j-6,INTRON_0) + (((mat->gp->transition[GP4_INTRON2CDS]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS]);   
        if( temp > score)    {  
          score = temp;  
        ...
        score = GeneLoop6_DC_SHADOW_MATRIX(mat,i-0,j-8,MATCH) + (((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);   
        if( j - 8 <= mergej) {  
        ...
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-0,j-8,INSERT) + (((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);   
        if( temp > score)    {  
          score = temp;  
        ...
        score = GeneLoop6_DC_SHADOW_MATRIX(mat,i-0,j-9,MATCH) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);  
        if( j - 9 <= mergej) {  
        ...
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-0,j-9,INSERT) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);  
        if( temp > score)    {  
          score = temp;  
        ...
        score = GeneLoop6_DC_SHADOW_MATRIX(mat,i-0,j-10,MATCH) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);     
        if( j - 10 <= mergej)    {  
        ...
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-0,j-10,INSERT) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);     
        if( temp > score)    {  
          score = temp;  
...
/* Function:  dc_start_end_calculate_GeneLoop6(mat,dpenv)
 *
 ...
 *
 */
boolean dc_start_end_calculate_GeneLoop6(GeneLoop6 * mat,DPEnvelope * dpenv) 
{
    int i;   
    int j;   
    int k;   
    ...
    init_start_end_linear_GeneLoop6(mat);    
    ...
    for(j=0;j<lenj;j++)  { /*for each j strip*/ 
      for(i=0;i<leni;i++)    { /*for each i position in strip*/ 
        ...
        /* For state MATCH */ 
        /* setting first movement to score */ 
        score = GeneLoop6_DC_SHADOW_MATRIX(mat,i-1,j-3,MATCH) + (mat->query->seg[i]->transition[GW_MATCH2MATCH]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)]) + (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 3,MATCH,k);   
        /* From state INSERT to state MATCH */ 
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-1,j-3,INSERT) + (mat->query->seg[i]->transition[GW_INSERT2MATCH]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)]) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 3,INSERT,k);    
        ...
        /* From state DELETE to state MATCH */ 
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-1,j-3,DELETE) + (mat->query->seg[i]->transition[GW_DELETE2MATCH]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)]) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 3,DELETE,k);    
        ...
        /* From state LOOP to state MATCH */ 
        temp = GeneLoop6_DC_SHADOW_SPECIAL(mat,i-1,j-3,LOOP) + ((mat->query->seg[i]->transition[GW_START2MATCH]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+mat->gp->transition[GP4_LOOP2MODEL]) + (CSEQ_GENOMIC_CDSPOT(mat->target,j));   
        if( temp  > score )  {  
          score = temp;  
        ...
        /* From state INTRON_0 to state MATCH */ 
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-1,j-6,INTRON_0) + (((mat->gp->transition[GP4_INTRON2CDS]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS]) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 6,INTRON_0,k);  
        ...
        /* From state INTRON_1 to state MATCH */ 
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-1,j-5,INTRON_1) + (mat->gp->transition[GP4_INTRON2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-2))) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 5,INTRON_1,k);  
        ...
        /* From state INTRON_2 to state MATCH */ 
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-1,j-4,INTRON_2) + (mat->gp->transition[GP4_INTRON2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-1))) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 4,INTRON_2,k);  
        ...
        /* From state MATCH to state MATCH */ 
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-1,j-2,MATCH) + mat->gp->transition[GP4_DELETE_1_BASE] +(CSEQ_GENOMIC_CDSPOT(mat->target,j));     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 2,MATCH,k); 
        ...
        /* From state MATCH to state MATCH */ 
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-1,j-1,MATCH) + mat->gp->transition[GP4_DELETE_2_BASE] +(CSEQ_GENOMIC_CDSPOT(mat->target,j));     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 1,MATCH,k); 
        ...
        /* From state MATCH to state MATCH */ 
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-1,j-4,MATCH) + mat->gp->transition[GP4_INSERT_1_BASE] +(CSEQ_GENOMIC_CDSPOT(mat->target,j));     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 4,MATCH,k); 
        ...
        /* From state MATCH to state MATCH */ 
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-1,j-5,MATCH) + mat->gp->transition[GP4_INSERT_2_BASE] +(CSEQ_GENOMIC_CDSPOT(mat->target,j));     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 5,MATCH,k); 
        ...
        /* state MATCH is a source for special LOOP */ 
        temp = score + (mat->query->seg[i]->transition[GW_MATCH2END]) + (0) ;    
        if( temp > GeneLoop6_DC_SHADOW_SPECIAL(mat,i,j,LOOP) )   {  
          GeneLoop6_DC_SHADOW_SPECIAL(mat,i,j,LOOP) = temp;  
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            GeneLoop6_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,k) = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i,j,MATCH,k); 
          GeneLoop6_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,6) = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i,j,MATCH,6);   
          GeneLoop6_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,3) = i;    
          GeneLoop6_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,4) = j;    
          GeneLoop6_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,5) = MATCH;    
          }  
        ...
        /* For state INSERT */ 
        /* setting first movement to score */ 
        score = GeneLoop6_DC_SHADOW_MATRIX(mat,i-0,j-3,MATCH) + (mat->query->seg[i]->transition[GW_MATCH2INSERT]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)]) + (CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 3,MATCH,k);   
        /* From state INSERT to state INSERT */ 
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-0,j-3,INSERT) + (mat->query->seg[i]->transition[GW_INSERT2INSERT]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)]) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 3,INSERT,k);    
        ...
        /* From state DELETE to state INSERT */ 
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-0,j-3,DELETE) + (mat->query->seg[i]->transition[GW_DELETE2INSERT]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)]) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 3,DELETE,k);    
        ...
        /* From state LOOP to state INSERT */ 
        temp = GeneLoop6_DC_SHADOW_SPECIAL(mat,i-0,j-3,LOOP) + ((mat->query->seg[i]->transition[GW_START2INSERT]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)])+mat->gp->transition[GP4_LOOP2MODEL]) + (CSEQ_GENOMIC_CDSPOT(mat->target,j));     
        if( temp  > score )  {  
          score = temp;  
        ...
        /* From state INTRON_0 to state INSERT */ 
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-0,j-6,INTRON_0) + (((mat->gp->transition[GP4_INTRON2CDS]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS]) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 6,INTRON_0,k);  
        ...
        /* From state INTRON_1 to state INSERT */ 
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-0,j-5,INTRON_1) + (mat->gp->transition[GP4_INTRON2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-2))) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 5,INTRON_1,k);  
        ...
        /* From state INTRON_2 to state INSERT */ 
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-0,j-4,INTRON_2) + (mat->gp->transition[GP4_INTRON2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-1))) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 4,INTRON_2,k);  
        ...
        /* From state INSERT to state INSERT */ 
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-0,j-2,INSERT) + mat->gp->transition[GP4_DELETE_1_BASE] +(CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 2,INSERT,k);    
        ...
        /* From state INSERT to state INSERT */ 
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-0,j-1,INSERT) + mat->gp->transition[GP4_DELETE_2_BASE] +(CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,INSERT,k);    
        ...
        if( temp > GeneLoop6_DC_SHADOW_SPECIAL(mat,i,j,LOOP) )   {  
          ...
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            GeneLoop6_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,k) = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i,j,INSERT,k);    
          GeneLoop6_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,6) = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i,j,INSERT,6);  
          GeneLoop6_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,3) = i;    
          GeneLoop6_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,4) = j;    
          GeneLoop6_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,5) = INSERT;   
          }  
        ...
        /* For state DELETE */ 
        /* setting first movement to score */ 
        score = GeneLoop6_DC_SHADOW_MATRIX(mat,i-1,j-0,MATCH) + mat->query->seg[i]->transition[GW_MATCH2DELETE] + (0);   
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 0,MATCH,k);   
        /* From state INSERT to state DELETE */ 
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-1,j-0,INSERT) + mat->query->seg[i]->transition[GW_INSERT2DELETE] +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 0,INSERT,k);    
        ...
        /* From state DELETE to state DELETE */ 
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-1,j-0,DELETE) + mat->query->seg[i]->transition[GW_DELETE2DELETE] +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 0,DELETE,k);    
        ...
        if( temp > GeneLoop6_DC_SHADOW_SPECIAL(mat,i,j,LOOP) )   {  
          ...
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            GeneLoop6_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,k) = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i,j,DELETE,k);    
          GeneLoop6_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,6) = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i,j,DELETE,6);  
          GeneLoop6_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,3) = i;    
          GeneLoop6_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,4) = j;    
          GeneLoop6_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,5) = DELETE;   
          }  
        ...
        /* For state INTRON_0 */ 
        /* setting first movement to score */ 
        score = GeneLoop6_DC_SHADOW_MATRIX(mat,i-0,j-8,MATCH) + (((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]) + (0);     
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 8,MATCH,k);   
        /* From state INSERT to state INTRON_0 */ 
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-0,j-8,INSERT) + (((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]) +(0);  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 8,INSERT,k);    
        ...
        /* From state INTRON_0 to state INTRON_0 */ 
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-0,j-1,INTRON_0) + (mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp->transition[GP4_INTRON2INTRON]) +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,INTRON_0,k);  
        ...
        /* For state INTRON_1 */ 
        /* setting first movement to score */ 
        score = GeneLoop6_DC_SHADOW_MATRIX(mat,i-0,j-9,MATCH) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]) + (0);    
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 9,MATCH,k);   
        /* From state INSERT to state INTRON_1 */ 
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-0,j-9,INSERT) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]) +(0);     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 9,INSERT,k);    
        ...
        /* From state INTRON_1 to state INTRON_1 */ 
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-0,j-1,INTRON_1) + (mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp->transition[GP4_INTRON2INTRON]) +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,INTRON_1,k);  
        ...
        /* For state INTRON_2 */ 
        /* setting first movement to score */ 
        score = GeneLoop6_DC_SHADOW_MATRIX(mat,i-0,j-10,MATCH) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]) + (0);   
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 10,MATCH,k);  
        /* From state INSERT to state INTRON_2 */ 
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-0,j-10,INSERT) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]) +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 10,INSERT,k);   
        ...
        /* From state INTRON_2 to state INTRON_2 */ 
        temp = GeneLoop6_DC_SHADOW_MATRIX(mat,i-0,j-1,INTRON_2) + (mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp->transition[GP4_INTRON2INTRON]) +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneLoop6_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,INTRON_2,k);  
      ...
      /* Special state LOOP has special to speical */ 
      /* Set score to current score (remember, state probably updated during main loop */ 
      score = GeneLoop6_DC_SHADOW_SPECIAL(mat,0,j,LOOP); 


      /* Source START is a special source for LOOP */ 
      temp = GeneLoop6_DC_SHADOW_SPECIAL(mat,0,j - 1,START) + (0) + (0);     
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneLoop6_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,k) = GeneLoop6_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,START,k);  
        }  
      ...
      /* Source DELETE for state LOOP is not special... already calculated */ 
      /* Source LOOP is a special source for LOOP */ 
      temp = GeneLoop6_DC_SHADOW_SPECIAL(mat,0,j - 1,LOOP) + (mat->gp->transition[GP4_LOOP2LOOP]) + (0);     
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneLoop6_DC_SHADOW_SPECIAL_SP(mat,i,j,LOOP,k) = GeneLoop6_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,LOOP,k);   
        }  


      /* Put back score... (now updated!) */ 
      GeneLoop6_DC_SHADOW_SPECIAL(mat,0,j,LOOP) = score; 
      /* Finished updating state LOOP */ 
      ...
      /* Special state END has special to speical */ 
      /* Set score to current score (remember, state probably updated during main loop */ 
      score = GeneLoop6_DC_SHADOW_SPECIAL(mat,0,j,END);  


      /* Source LOOP is a special source for END */ 
      temp = GeneLoop6_DC_SHADOW_SPECIAL(mat,0,j - 1,LOOP) + (0) + (0);  
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneLoop6_DC_SHADOW_SPECIAL_SP(mat,i,j,END,k) = GeneLoop6_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,LOOP,k);    
        }  


      /* Put back score... (now updated!) */ 
      GeneLoop6_DC_SHADOW_SPECIAL(mat,0,j,END) = score;  
      /* Finished updating state END */ 
...
PackAln * PackAln_read_Expl_GeneLoop6(GeneLoop6 * mat) 
{
    ...
    if( mat->basematrix->type != BASEMATRIX_TYPE_EXPLICIT)   {  
      warn("In GeneLoop6_basic_read You have asked for an alignment from a non-explicit matrix: c'est impossible [current type is %d - %s]", mat->basematrix->type,basematrix_type_to_string(mat->basematrix->type));    
      return NULL;   
      }  
    ...
    out->score =  find_end_GeneLoop6(mat,&i,&j,&state,&isspecial);   
    ...
    while( state != START || isspecial != TRUE)  { /*while state != START*/ 
      ...
      if( isspecial == TRUE )    
        max_calc_special_GeneLoop6(mat,i,j,state,isspecial,&i,&j,&state,&isspecial,&cellscore);  
      else   
        max_calc_GeneLoop6(mat,i,j,state,isspecial,&i,&j,&state,&isspecial,&cellscore);  
...
/* Function:  find_end_GeneLoop6(mat,ri,rj,state,isspecial)
 *
 * Descrip: No Description
 *
 * Arg:              mat [UNKN ] Undocumented argument [GeneLoop6 *]
 ...
 *
 */
int find_end_GeneLoop6(GeneLoop6 * mat,int * ri,int * rj,int * state,boolean * isspecial) 
{
    register int j;  
    register int max;    
    register int maxj;   
    ...
    max = GeneLoop6_EXPL_SPECIAL(mat,0,mat->target->seq->len-1,END); 
    maxj = mat->target->seq->len-1;  
    for(j= mat->target->seq->len-2 ;j >= 0 ;j--) {  
      if( GeneLoop6_EXPL_SPECIAL(mat,0,j,END) > max )    {  
        max = GeneLoop6_EXPL_SPECIAL(mat,0,j,END);   
        maxj = j;    
        }  
      }  
...
/* Function:  max_calc_GeneLoop6(mat,i,j,state,isspecial,reti,retj,retstate,retspecial,cellscore)
 *
 * Descrip: No Description
 *
 * Arg:               mat [UNKN ] Undocumented argument [GeneLoop6 *]
 ...
 *
 */
int max_calc_GeneLoop6(GeneLoop6 * mat,int i,int j,int state,boolean isspecial,int * reti,int * retj,int * retstate,boolean * retspecial,int * cellscore) 
{
    register int temp;   
    register int cscore; 
    ...
    *reti = (*retj) = (*retstate) = GeneLoop6_READ_OFF_ERROR;    
    ...
    /* Then you have to select the correct switch statement to figure out the readoff      */ 
    /* Somewhat odd - reverse the order of calculation and return as soon as it is correct */ 
    cscore = GeneLoop6_EXPL_MATRIX(mat,i,j,state);   
    switch(state)    { /*Switch state */ 
      case MATCH :   
        temp = cscore - (mat->gp->transition[GP4_INSERT_2_BASE]) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 5,MATCH) )   {  
          *reti = i - 1; 
          *retj = j - 5; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-1,j-5,MATCH);  
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 5,MATCH);   
          }  
        temp = cscore - (mat->gp->transition[GP4_INSERT_1_BASE]) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 4,MATCH) )   {  
          *reti = i - 1; 
          *retj = j - 4; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-1,j-4,MATCH);  
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 4,MATCH);   
          }  
        temp = cscore - (mat->gp->transition[GP4_DELETE_2_BASE]) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 1,MATCH) )   {  
          *reti = i - 1; 
          *retj = j - 1; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-1,j-1,MATCH);  
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 1,MATCH);   
          }  
        temp = cscore - (mat->gp->transition[GP4_DELETE_1_BASE]) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 2,MATCH) )   {  
          *reti = i - 1; 
          *retj = j - 2; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-1,j-2,MATCH);  
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 2,MATCH);   
          }  
        temp = cscore - ((mat->gp->transition[GP4_INTRON2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-1)))) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j)); 
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 4,INTRON_2) )    {  
          *reti = i - 1; 
          *retj = j - 4; 
          *retstate = INTRON_2;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-1,j-4,INTRON_2);   
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 4,INTRON_2);    
          }  
        temp = cscore - ((mat->gp->transition[GP4_INTRON2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-2)))) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j)); 
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 5,INTRON_1) )    {  
          *reti = i - 1; 
          *retj = j - 5; 
          *retstate = INTRON_1;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-1,j-5,INTRON_1);   
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 5,INTRON_1);    
          }  
        temp = cscore - ((((mat->gp->transition[GP4_INTRON2CDS]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));   
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 6,INTRON_0) )    {  
          *reti = i - 1; 
          *retj = j - 6; 
          *retstate = INTRON_0;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-1,j-6,INTRON_0);   
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 6,INTRON_0);    
          }  
        temp = cscore - (((mat->query->seg[i]->transition[GW_START2MATCH]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+mat->gp->transition[GP4_LOOP2MODEL])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));   
        if( temp == GeneLoop6_EXPL_SPECIAL(mat,i - 1,j - 3,LOOP) )   {  
          *reti = i - 1; 
          *retj = j - 3; 
          *retstate = LOOP;  
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_SPECIAL(mat,i-1,j-3,LOOP);  
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 3,LOOP);    
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_DELETE2MATCH]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 3,DELETE) )  {  
          *reti = i - 1; 
          *retj = j - 3; 
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-1,j-3,DELETE); 
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 3,DELETE);  
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_INSERT2MATCH]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 3,INSERT) )  {  
          *reti = i - 1; 
          *retj = j - 3; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-1,j-3,INSERT); 
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 3,INSERT);  
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_MATCH2MATCH]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j)); 
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 3,MATCH) )   {  
          *reti = i - 1; 
          *retj = j - 3; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-1,j-3,MATCH);  
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 3,MATCH);   
          }  
        warn("Major problem (!) - in GeneLoop6 read off, position %d,%d state %d no source found!",i,j,state);   
        return (-1); 
      case INSERT :  
        temp = cscore - (mat->gp->transition[GP4_DELETE_2_BASE]) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 1,INSERT) )  {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-0,j-1,INSERT); 
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 1,INSERT);  
          }  
        temp = cscore - (mat->gp->transition[GP4_DELETE_1_BASE]) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 2,INSERT) )  {  
          *reti = i - 0; 
          *retj = j - 2; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-0,j-2,INSERT); 
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 2,INSERT);  
          }  
        temp = cscore - ((mat->gp->transition[GP4_INTRON2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-1)))) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j)); 
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 4,INTRON_2) )    {  
          *reti = i - 0; 
          *retj = j - 4; 
          *retstate = INTRON_2;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-0,j-4,INTRON_2);   
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 4,INTRON_2);    
          }  
        temp = cscore - ((mat->gp->transition[GP4_INTRON2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-2)))) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j)); 
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 5,INTRON_1) )    {  
          *reti = i - 0; 
          *retj = j - 5; 
          *retstate = INTRON_1;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-0,j-5,INTRON_1);   
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 5,INTRON_1);    
          }  
        temp = cscore - ((((mat->gp->transition[GP4_INTRON2CDS]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));   
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 6,INTRON_0) )    {  
          *reti = i - 0; 
          *retj = j - 6; 
          *retstate = INTRON_0;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-0,j-6,INTRON_0);   
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 6,INTRON_0);    
          }  
        temp = cscore - (((mat->query->seg[i]->transition[GW_START2INSERT]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)])+mat->gp->transition[GP4_LOOP2MODEL])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j)); 
        if( temp == GeneLoop6_EXPL_SPECIAL(mat,i - 0,j - 3,LOOP) )   {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = LOOP;  
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_SPECIAL(mat,i-0,j-3,LOOP);  
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 3,LOOP);    
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_DELETE2INSERT]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 3,DELETE) )  {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-0,j-3,DELETE); 
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 3,DELETE);  
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_INSERT2INSERT]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 3,INSERT) )  {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-0,j-3,INSERT); 
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 3,INSERT);  
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_MATCH2INSERT]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));   
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 3,MATCH) )   {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-0,j-3,MATCH);  
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 3,MATCH);   
          }  
        warn("Major problem (!) - in GeneLoop6 read off, position %d,%d state %d no source found!",i,j,state);   
        return (-1); 
      case DELETE :  
        temp = cscore - (mat->query->seg[i]->transition[GW_DELETE2DELETE]) -  (0);   
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 0,DELETE) )  {  
          *reti = i - 1; 
          *retj = j - 0; 
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-1,j-0,DELETE); 
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 0,DELETE);  
          }  
        temp = cscore - (mat->query->seg[i]->transition[GW_INSERT2DELETE]) -  (0);   
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 0,INSERT) )  {  
          *reti = i - 1; 
          *retj = j - 0; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-1,j-0,INSERT); 
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 0,INSERT);  
          }  
        temp = cscore - (mat->query->seg[i]->transition[GW_MATCH2DELETE]) -  (0);    
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 0,MATCH) )   {  
          *reti = i - 1; 
          *retj = j - 0; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-1,j-0,MATCH);  
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 1,j - 0,MATCH);   
          }  
        warn("Major problem (!) - in GeneLoop6 read off, position %d,%d state %d no source found!",i,j,state);   
        return (-1); 
      case INTRON_0 :    
        temp = cscore - ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp->transition[GP4_INTRON2INTRON])) -  (0); 
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 1,INTRON_0) )    {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = INTRON_0;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-0,j-1,INTRON_0);   
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 1,INTRON_0);    
          }  
        temp = cscore - ((((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])) -  (0);  
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 8,INSERT) )  {  
          *reti = i - 0; 
          *retj = j - 8; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-0,j-8,INSERT); 
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 8,INSERT);  
          }  
        temp = cscore - ((((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])) -  (0);  
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 8,MATCH) )   {  
          *reti = i - 0; 
          *retj = j - 8; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-0,j-8,MATCH);  
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 8,MATCH);   
          }  
        warn("Major problem (!) - in GeneLoop6 read off, position %d,%d state %d no source found!",i,j,state);   
        return (-1); 
      case INTRON_1 :    
        temp = cscore - ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp->transition[GP4_INTRON2INTRON])) -  (0); 
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 1,INTRON_1) )    {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = INTRON_1;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-0,j-1,INTRON_1);   
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 1,INTRON_1);    
          }  
        temp = cscore - (((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])) -  (0); 
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 9,INSERT) )  {  
          *reti = i - 0; 
          *retj = j - 9; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-0,j-9,INSERT); 
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 9,INSERT);  
          }  
        temp = cscore - (((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])) -  (0); 
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 9,MATCH) )   {  
          *reti = i - 0; 
          *retj = j - 9; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-0,j-9,MATCH);  
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 9,MATCH);   
          }  
        warn("Major problem (!) - in GeneLoop6 read off, position %d,%d state %d no source found!",i,j,state);   
        return (-1); 
      case INTRON_2 :    
        temp = cscore - ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp->transition[GP4_INTRON2INTRON])) -  (0); 
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 1,INTRON_2) )    {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = INTRON_2;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-0,j-1,INTRON_2);   
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 1,INTRON_2);    
          }  
        temp = cscore - (((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])) -  (0); 
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 10,INSERT) ) {  
          *reti = i - 0; 
          *retj = j - 10;    
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-0,j-10,INSERT);    
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 10,INSERT);     
          }  
        temp = cscore - (((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])) -  (0); 
        if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 10,MATCH) )  {  
          *reti = i - 0; 
          *retj = j - 10;    
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-0,j-10,MATCH); 
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 10,MATCH);  
          }  
        warn("Major problem (!) - in GeneLoop6 read off, position %d,%d state %d no source found!",i,j,state);   
        return (-1); 
      default:   
...
/* Function:  max_calc_special_GeneLoop6(mat,i,j,state,isspecial,reti,retj,retstate,retspecial,cellscore)
 *
 * Descrip: No Description
 *
 * Arg:               mat [UNKN ] Undocumented argument [GeneLoop6 *]
 ...
 *
 */
int max_calc_special_GeneLoop6(GeneLoop6 * mat,int i,int j,int state,boolean isspecial,int * reti,int * retj,int * retstate,boolean * retspecial,int * cellscore) 
{
    register int temp;   
    register int cscore; 
    ...
    *reti = (*retj) = (*retstate) = GeneLoop6_READ_OFF_ERROR;    
    ...
    cscore = GeneLoop6_EXPL_SPECIAL(mat,i,j,state);  
    switch(state)    { /*switch on special states*/ 
      case LOOP :    
        /* source LOOP is a special */ 
        temp = cscore - (mat->gp->transition[GP4_LOOP2LOOP]) - (0);  
        if( temp == GeneLoop6_EXPL_SPECIAL(mat,i - 0,j - 1,LOOP) )   {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = LOOP;  
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_SPECIAL(mat,i-0,j-1,LOOP);  
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 1,LOOP) ;   
          }  
        ...
        for(i= mat->query->len-1;i >= 0 ;i--)    { /*for i >= 0*/ 
          ...
          if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 0,DELETE) )    {  
            *reti = i - 0;   
            *retj = j - 0;   
            *retstate = DELETE;  
            *retspecial = FALSE; 
            if( cellscore != NULL)   {  
              *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-0,j-0,DELETE);   
              }  
            return GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 0,DELETE) ;   
            }  
          } /* end of for i >= 0 */ 
        /* source INSERT is from main matrix */ 
        for(i= mat->query->len-1;i >= 0 ;i--)    { /*for i >= 0*/ 
          temp = cscore - (mat->query->seg[i]->transition[GW_INSERT2END]) - (0);     
          if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 0,INSERT) )    {  
            *reti = i - 0;   
            *retj = j - 0;   
            *retstate = INSERT;  
            *retspecial = FALSE; 
            if( cellscore != NULL)   {  
              *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-0,j-0,INSERT);   
              }  
            return GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 0,INSERT) ;   
            }  
          } /* end of for i >= 0 */ 
        /* source MATCH is from main matrix */ 
        for(i= mat->query->len-1;i >= 0 ;i--)    { /*for i >= 0*/ 
          temp = cscore - (mat->query->seg[i]->transition[GW_MATCH2END]) - (0);  
          if( temp == GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 0,MATCH) ) {  
            *reti = i - 0;   
            *retj = j - 0;   
            *retstate = MATCH;   
            *retspecial = FALSE; 
            if( cellscore != NULL)   {  
              *cellscore = cscore - GeneLoop6_EXPL_MATRIX(mat,i-0,j-0,MATCH);    
              }  
            return GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 0,MATCH) ;    
            }  
          } /* end of for i >= 0 */ 
        /* source START is a special */ 
        temp = cscore - (0) - (0);   
        if( temp == GeneLoop6_EXPL_SPECIAL(mat,i - 0,j - 1,START) )  {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = START; 
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneLoop6_EXPL_SPECIAL(mat,i-0,j-1,START);     
            }  
          return GeneLoop6_EXPL_MATRIX(mat,i - 0,j - 1,START) ;  
          }  
      case START :   
      case END :     
        /* source LOOP is a special */ 
...
boolean calculate_GeneLoop6(GeneLoop6 * mat) 
{
    ...
    for(j=0;j<lenj;j++)  {  
      ...
      for(i=0;i<leni;i++)    {  
        ...
        /* From state INTRON_0 to state MATCH */ 
        temp = GeneLoop6_EXPL_MATRIX(mat,i-1,j-6,INTRON_0) + (((mat->gp->transition[GP4_INTRON2CDS]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS]);    
        if( temp  > score )  {  
          score = temp;  
          }  
        /* From state INTRON_1 to state MATCH */ 
        ...
        /* From state INTRON_0 to state INSERT */ 
        temp = GeneLoop6_EXPL_MATRIX(mat,i-0,j-6,INTRON_0) + (((mat->gp->transition[GP4_INTRON2CDS]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS]);    
        if( temp  > score )  {  
          score = temp;  
          }  
        /* From state INTRON_1 to state INSERT */ 
        ...
        /* For state INTRON_0 */ 
        /* setting first movement to score */ 
        score = GeneLoop6_EXPL_MATRIX(mat,i-0,j-8,MATCH) + (((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);    
        /* From state INSERT to state INTRON_0 */ 
        temp = GeneLoop6_EXPL_MATRIX(mat,i-0,j-8,INSERT) + (((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);    
        if( temp  > score )  {  
          score = temp;  
          }  
        /* From state INTRON_0 to state INTRON_0 */ 
        ...
        /* For state INTRON_1 */ 
        /* setting first movement to score */ 
        score = GeneLoop6_EXPL_MATRIX(mat,i-0,j-9,MATCH) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);   
        /* From state INSERT to state INTRON_1 */ 
        temp = GeneLoop6_EXPL_MATRIX(mat,i-0,j-9,INSERT) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);   
        if( temp  > score )  {  
          score = temp;  
          }  
        /* From state INTRON_1 to state INTRON_1 */ 
        ...
        /* For state INTRON_2 */ 
        /* setting first movement to score */ 
        score = GeneLoop6_EXPL_MATRIX(mat,i-0,j-10,MATCH) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);  
        /* From state INSERT to state INTRON_2 */ 
        temp = GeneLoop6_EXPL_MATRIX(mat,i-0,j-10,INSERT) + ((mat->gp->intron[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]);  
        if( temp  > score )  {  
          score = temp;  
          }  
        /* From state INTRON_2 to state INTRON_2 */ 
