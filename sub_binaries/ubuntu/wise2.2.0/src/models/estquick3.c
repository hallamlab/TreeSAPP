...
Search_Return_Type search_EstQuick3(DBSearchImpl * dbsi,Hscore * out,GeneWiseQuickDB* querydb,cDNADB* targetdb ,cDNAParserScore * cp) 
{
    ...
    if( dbsi->trace_level > 5 )  
      warn("Asking for trace level of %d in database search for EstQuick3, but it was compiled with a trace level of 416. Not all trace statements can be shown",dbsi->trace_level); 
...
PackAln * PackAln_bestmemory_EstQuick3(GeneWiseScoreFlat* query,ComplexSequence* target ,cDNAParserScore * cp,DPEnvelope * dpenv,DPRunImpl * dpri) 
{
    ...
    if( strategy == DPIM_Linear )    {  
      ...
      if( (mat=allocate_Expl_EstQuick3(query, target , cp)) == NULL )    {  
...
/* Function:  allocate_Expl_EstQuick3(query,target,cp)
 *
 ...
 *
 */
EstQuick3 * allocate_Expl_EstQuick3(GeneWiseScoreFlat* query,ComplexSequence* target ,cDNAParserScore * cp) 
{
    EstQuick3 * out; 
...
PackAln * PackAln_calculate_Small_EstQuick3(EstQuick3 * mat,DPEnvelope * dpenv) 
{
    ...
    dc_start_end_calculate_EstQuick3(mat,dpenv); 
...
AlnRangeSet * AlnRangeSet_calculate_Small_EstQuick3(EstQuick3 * mat) 
{
    ...
    start_reporting("Find start end points: ");  
    dc_start_end_calculate_EstQuick3(mat,NULL);  
...
/* Function:  dc_start_end_calculate_EstQuick3(mat,dpenv)
 *
 ...
 *
 */
boolean dc_start_end_calculate_EstQuick3(EstQuick3 * mat,DPEnvelope * dpenv) 
{
    int i;   
    int j;   
    int k;   
    ...
    init_start_end_linear_EstQuick3(mat);    
    ...
    for(j=0;j<lenj;j++)  { /*for each j strip*/ 
      for(i=0;i<leni;i++)    { /*for each i position in strip*/ 
        ...
        /* For state MATCH */ 
        /* setting first movement to score */ 
        score = EstQuick3_DC_SHADOW_MATRIX(mat,i-1,j-3,MATCH) + (mat->query->seg[i].transition[GW_MATCH2MATCH]+mat->query->seg[i].match[CSEQ_CDNA_CODON(mat->target,j)]) + (0);  
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = EstQuick3_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 3,MATCH,k);   
        /* From state INSERT to state MATCH */ 
        temp = EstQuick3_DC_SHADOW_MATRIX(mat,i-1,j-3,INSERT) + (mat->query->seg[i].transition[GW_INSERT2MATCH]+mat->query->seg[i].match[CSEQ_CDNA_CODON(mat->target,j)]) +(0);  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstQuick3_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 3,INSERT,k);    
        ...
        /* From state DELETE to state MATCH */ 
        temp = EstQuick3_DC_SHADOW_MATRIX(mat,i-1,j-3,DELETE) + (mat->query->seg[i].transition[GW_DELETE2MATCH]+mat->query->seg[i].match[CSEQ_CDNA_CODON(mat->target,j)]) +(0);  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstQuick3_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 3,DELETE,k);    
        ...
        /* From state START to state MATCH */ 
        temp = EstQuick3_DC_SHADOW_SPECIAL(mat,i-1,j-3,START) + (mat->query->seg[i].transition[GW_START2MATCH]+mat->query->seg[i].match[CSEQ_CDNA_CODON(mat->target,j)]) + (0);  
        if( temp  > score )  {  
          score = temp;  
        ...
        /* From state MATCH to state MATCH */ 
        temp = EstQuick3_DC_SHADOW_MATRIX(mat,i-1,j-2,MATCH) + mat->cp->trans[PCD_DELETE_1_BASE] +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstQuick3_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 2,MATCH,k); 
        ...
        /* From state MATCH to state MATCH */ 
        temp = EstQuick3_DC_SHADOW_MATRIX(mat,i-1,j-4,MATCH) + mat->cp->trans[PCD_INSERT_1_BASE] +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstQuick3_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 4,MATCH,k); 
        ...
        /* state MATCH is a source for special END */ 
        temp = score + (mat->query->seg[i].transition[GW_MATCH2END]) + (0) ;     
        if( temp > EstQuick3_DC_SHADOW_SPECIAL(mat,i,j,END) )    {  
          EstQuick3_DC_SHADOW_SPECIAL(mat,i,j,END) = temp;   
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            EstQuick3_DC_SHADOW_SPECIAL_SP(mat,i,j,END,k) = EstQuick3_DC_SHADOW_MATRIX_SP(mat,i,j,MATCH,k);  
          EstQuick3_DC_SHADOW_SPECIAL_SP(mat,i,j,END,6) = EstQuick3_DC_SHADOW_MATRIX_SP(mat,i,j,MATCH,6);    
          EstQuick3_DC_SHADOW_SPECIAL_SP(mat,i,j,END,3) = i; 
          EstQuick3_DC_SHADOW_SPECIAL_SP(mat,i,j,END,4) = j; 
          EstQuick3_DC_SHADOW_SPECIAL_SP(mat,i,j,END,5) = MATCH; 
          }  
        ...
        /* For state INSERT */ 
        /* setting first movement to score */ 
        score = EstQuick3_DC_SHADOW_MATRIX(mat,i-0,j-3,MATCH) + (mat->query->seg[i].transition[GW_MATCH2INSERT]+mat->query->seg[i].insert[CSEQ_CDNA_CODON(mat->target,j)]) + (0);    
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = EstQuick3_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 3,MATCH,k);   
        /* From state INSERT to state INSERT */ 
        temp = EstQuick3_DC_SHADOW_MATRIX(mat,i-0,j-3,INSERT) + (mat->query->seg[i].transition[GW_INSERT2INSERT]+mat->query->seg[i].insert[CSEQ_CDNA_CODON(mat->target,j)]) +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstQuick3_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 3,INSERT,k);    
        ...
        /* From state INSERT to state INSERT */ 
        temp = EstQuick3_DC_SHADOW_MATRIX(mat,i-0,j-2,INSERT) + mat->cp->trans[PCD_DELETE_1_BASE] +(0);  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstQuick3_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 2,INSERT,k);    
        ...
        /* From state INSERT to state INSERT */ 
        temp = EstQuick3_DC_SHADOW_MATRIX(mat,i-0,j-4,INSERT) + mat->cp->trans[PCD_INSERT_1_BASE] +(0);  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstQuick3_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 4,INSERT,k);    
        ...
        /* For state DELETE */ 
        /* setting first movement to score */ 
        score = EstQuick3_DC_SHADOW_MATRIX(mat,i-1,j-0,MATCH) + mat->query->seg[i].transition[GW_MATCH2DELETE] + (0);    
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = EstQuick3_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 0,MATCH,k);   
        /* From state DELETE to state DELETE */ 
        temp = EstQuick3_DC_SHADOW_MATRIX(mat,i-1,j-0,DELETE) + mat->query->seg[i].transition[GW_DELETE2DELETE] +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = EstQuick3_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 0,DELETE,k);    
...
PackAln * PackAln_read_Expl_EstQuick3(EstQuick3 * mat) 
{
    ...
    if( mat->basematrix->type != BASEMATRIX_TYPE_EXPLICIT)   {  
      warn("In EstQuick3_basic_read You have asked for an alignment from a non-explicit matrix: c'est impossible [current type is %d - %s]", mat->basematrix->type,basematrix_type_to_string(mat->basematrix->type));    
      return NULL;   
      }  
    ...
    out->score =  find_end_EstQuick3(mat,&i,&j,&state,&isspecial);   
    ...
    while( state != START || isspecial != TRUE)  { /*while state != START*/ 
      ...
      if( isspecial == TRUE )    
        max_calc_special_EstQuick3(mat,i,j,state,isspecial,&i,&j,&state,&isspecial,&cellscore);  
      else   
        max_calc_EstQuick3(mat,i,j,state,isspecial,&i,&j,&state,&isspecial,&cellscore);  
...
/* Function:  find_end_EstQuick3(mat,ri,rj,state,isspecial)
 *
 * Descrip: No Description
 *
 * Arg:              mat [UNKN ] Undocumented argument [EstQuick3 *]
 ...
 *
 */
int find_end_EstQuick3(EstQuick3 * mat,int * ri,int * rj,int * state,boolean * isspecial) 
{
    register int j;  
    register int max;    
    register int maxj;   
    ...
    max = EstQuick3_EXPL_SPECIAL(mat,0,mat->target->seq->len-1,END); 
    maxj = mat->target->seq->len-1;  
    for(j= mat->target->seq->len-2 ;j >= 0 ;j--) {  
      if( EstQuick3_EXPL_SPECIAL(mat,0,j,END) > max )    {  
        max = EstQuick3_EXPL_SPECIAL(mat,0,j,END);   
        maxj = j;    
        }  
      }  
...
/* Function:  max_calc_EstQuick3(mat,i,j,state,isspecial,reti,retj,retstate,retspecial,cellscore)
 *
 * Descrip: No Description
 *
 * Arg:               mat [UNKN ] Undocumented argument [EstQuick3 *]
 ...
 *
 */
int max_calc_EstQuick3(EstQuick3 * mat,int i,int j,int state,boolean isspecial,int * reti,int * retj,int * retstate,boolean * retspecial,int * cellscore) 
{
    register int temp;   
    register int cscore; 
    ...
    *reti = (*retj) = (*retstate) = EstQuick3_READ_OFF_ERROR;    
    ...
    /* Then you have to select the correct switch statement to figure out the readoff      */ 
    /* Somewhat odd - reverse the order of calculation and return as soon as it is correct */ 
    cscore = EstQuick3_EXPL_MATRIX(mat,i,j,state);   
    switch(state)    { /*Switch state */ 
      case MATCH :   
        temp = cscore - (mat->cp->trans[PCD_INSERT_1_BASE]) -  (0);  
        if( temp == EstQuick3_EXPL_MATRIX(mat,i - 1,j - 4,MATCH) )   {  
          *reti = i - 1; 
          *retj = j - 4; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstQuick3_EXPL_MATRIX(mat,i-1,j-4,MATCH);  
            }  
          return EstQuick3_EXPL_MATRIX(mat,i - 1,j - 4,MATCH);   
          }  
        temp = cscore - (mat->cp->trans[PCD_DELETE_1_BASE]) -  (0);  
        if( temp == EstQuick3_EXPL_MATRIX(mat,i - 1,j - 2,MATCH) )   {  
          *reti = i - 1; 
          *retj = j - 2; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstQuick3_EXPL_MATRIX(mat,i-1,j-2,MATCH);  
            }  
          return EstQuick3_EXPL_MATRIX(mat,i - 1,j - 2,MATCH);   
          }  
        temp = cscore - ((mat->query->seg[i].transition[GW_START2MATCH]+mat->query->seg[i].match[CSEQ_CDNA_CODON(mat->target,j)])) -  (0);   
        if( temp == EstQuick3_EXPL_SPECIAL(mat,i - 1,j - 3,START) )  {  
          *reti = i - 1; 
          *retj = j - 3; 
          *retstate = START; 
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - EstQuick3_EXPL_SPECIAL(mat,i-1,j-3,START); 
            }  
          return EstQuick3_EXPL_MATRIX(mat,i - 1,j - 3,START);   
          }  
        temp = cscore - ((mat->query->seg[i].transition[GW_DELETE2MATCH]+mat->query->seg[i].match[CSEQ_CDNA_CODON(mat->target,j)])) -  (0);  
        if( temp == EstQuick3_EXPL_MATRIX(mat,i - 1,j - 3,DELETE) )  {  
          *reti = i - 1; 
          *retj = j - 3; 
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstQuick3_EXPL_MATRIX(mat,i-1,j-3,DELETE); 
            }  
          return EstQuick3_EXPL_MATRIX(mat,i - 1,j - 3,DELETE);  
          }  
        temp = cscore - ((mat->query->seg[i].transition[GW_INSERT2MATCH]+mat->query->seg[i].match[CSEQ_CDNA_CODON(mat->target,j)])) -  (0);  
        if( temp == EstQuick3_EXPL_MATRIX(mat,i - 1,j - 3,INSERT) )  {  
          *reti = i - 1; 
          *retj = j - 3; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstQuick3_EXPL_MATRIX(mat,i-1,j-3,INSERT); 
            }  
          return EstQuick3_EXPL_MATRIX(mat,i - 1,j - 3,INSERT);  
          }  
        temp = cscore - ((mat->query->seg[i].transition[GW_MATCH2MATCH]+mat->query->seg[i].match[CSEQ_CDNA_CODON(mat->target,j)])) -  (0);   
        if( temp == EstQuick3_EXPL_MATRIX(mat,i - 1,j - 3,MATCH) )   {  
          *reti = i - 1; 
          *retj = j - 3; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstQuick3_EXPL_MATRIX(mat,i-1,j-3,MATCH);  
            }  
          return EstQuick3_EXPL_MATRIX(mat,i - 1,j - 3,MATCH);   
          }  
        warn("Major problem (!) - in EstQuick3 read off, position %d,%d state %d no source found!",i,j,state);   
        return (-1); 
      case INSERT :  
        temp = cscore - (mat->cp->trans[PCD_INSERT_1_BASE]) -  (0);  
        if( temp == EstQuick3_EXPL_MATRIX(mat,i - 0,j - 4,INSERT) )  {  
          *reti = i - 0; 
          *retj = j - 4; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstQuick3_EXPL_MATRIX(mat,i-0,j-4,INSERT); 
            }  
          return EstQuick3_EXPL_MATRIX(mat,i - 0,j - 4,INSERT);  
          }  
        temp = cscore - (mat->cp->trans[PCD_DELETE_1_BASE]) -  (0);  
        if( temp == EstQuick3_EXPL_MATRIX(mat,i - 0,j - 2,INSERT) )  {  
          *reti = i - 0; 
          *retj = j - 2; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstQuick3_EXPL_MATRIX(mat,i-0,j-2,INSERT); 
            }  
          return EstQuick3_EXPL_MATRIX(mat,i - 0,j - 2,INSERT);  
          }  
        temp = cscore - ((mat->query->seg[i].transition[GW_INSERT2INSERT]+mat->query->seg[i].insert[CSEQ_CDNA_CODON(mat->target,j)])) -  (0);    
        if( temp == EstQuick3_EXPL_MATRIX(mat,i - 0,j - 3,INSERT) )  {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstQuick3_EXPL_MATRIX(mat,i-0,j-3,INSERT); 
            }  
          return EstQuick3_EXPL_MATRIX(mat,i - 0,j - 3,INSERT);  
          }  
        temp = cscore - ((mat->query->seg[i].transition[GW_MATCH2INSERT]+mat->query->seg[i].insert[CSEQ_CDNA_CODON(mat->target,j)])) -  (0); 
        if( temp == EstQuick3_EXPL_MATRIX(mat,i - 0,j - 3,MATCH) )   {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstQuick3_EXPL_MATRIX(mat,i-0,j-3,MATCH);  
            }  
          return EstQuick3_EXPL_MATRIX(mat,i - 0,j - 3,MATCH);   
          }  
        warn("Major problem (!) - in EstQuick3 read off, position %d,%d state %d no source found!",i,j,state);   
        return (-1); 
      case DELETE :  
        temp = cscore - (mat->query->seg[i].transition[GW_DELETE2DELETE]) -  (0);    
        if( temp == EstQuick3_EXPL_MATRIX(mat,i - 1,j - 0,DELETE) )  {  
          *reti = i - 1; 
          *retj = j - 0; 
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstQuick3_EXPL_MATRIX(mat,i-1,j-0,DELETE); 
            }  
          return EstQuick3_EXPL_MATRIX(mat,i - 1,j - 0,DELETE);  
          }  
        temp = cscore - (mat->query->seg[i].transition[GW_MATCH2DELETE]) -  (0); 
        if( temp == EstQuick3_EXPL_MATRIX(mat,i - 1,j - 0,MATCH) )   {  
          *reti = i - 1; 
          *retj = j - 0; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - EstQuick3_EXPL_MATRIX(mat,i-1,j-0,MATCH);  
            }  
          return EstQuick3_EXPL_MATRIX(mat,i - 1,j - 0,MATCH);   
          }  
        warn("Major problem (!) - in EstQuick3 read off, position %d,%d state %d no source found!",i,j,state);   
        return (-1); 
      default:   
...
/* Function:  max_calc_special_EstQuick3(mat,i,j,state,isspecial,reti,retj,retstate,retspecial,cellscore)
 *
 * Descrip: No Description
 *
 * Arg:               mat [UNKN ] Undocumented argument [EstQuick3 *]
 ...
 *
 */
int max_calc_special_EstQuick3(EstQuick3 * mat,int i,int j,int state,boolean isspecial,int * reti,int * retj,int * retstate,boolean * retspecial,int * cellscore) 
{
    register int temp;   
    register int cscore; 
    ...
    *reti = (*retj) = (*retstate) = EstQuick3_READ_OFF_ERROR;    
    ...
    cscore = EstQuick3_EXPL_SPECIAL(mat,i,j,state);  
    switch(state)    { /*switch on special states*/ 
      case START :   
      case END :     
        ...
        for(i= mat->query->len-1;i >= 0 ;i--)    { /*for i >= 0*/ 
          ...
          if( temp == EstQuick3_EXPL_MATRIX(mat,i - 0,j - 0,MATCH) ) {  
            *reti = i - 0;   
            *retj = j - 0;   
            *retstate = MATCH;   
            *retspecial = FALSE; 
            if( cellscore != NULL)   {  
              *cellscore = cscore - EstQuick3_EXPL_MATRIX(mat,i-0,j-0,MATCH);    
              }  
            return EstQuick3_EXPL_MATRIX(mat,i - 0,j - 0,MATCH) ;    
            }  
          } /* end of for i >= 0 */ 
      default:   
