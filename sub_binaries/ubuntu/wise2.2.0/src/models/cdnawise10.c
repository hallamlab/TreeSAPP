...
Search_Return_Type search_CdnaWise10(DBSearchImpl * dbsi,Hscore * out,cDNADB* querydb,GenomicDB* targetdb ,GeneParser4Score * gp,CodonMatrixScore * sc,RandomCodonScore * rndcodon,DnaMatrix* utr,Score gap_open,Score gap_ext,Score utr_gap,Score intron_gap) 
{
    ...
    if( dbsi->trace_level > 5 )  
      warn("Asking for trace level of %d in database search for CdnaWise10, but it was compiled with a trace level of 416. Not all trace statements can be shown",dbsi->trace_level);    
...
PackAln * PackAln_bestmemory_CdnaWise10(ComplexSequence* query,ComplexSequence* target ,GeneParser4Score * gp,CodonMatrixScore * sc,RandomCodonScore * rndcodon,DnaMatrix* utr,Score gap_open,Score gap_ext,Score utr_gap,Score intron_gap,DPEnvelope * dpenv,DPRunImpl * dpri) 
{
    ...
    if( strategy == DPIM_Linear )    {  
      ...
      if( (mat=allocate_Expl_CdnaWise10(query, target , gp, sc, rndcodon, utr, gap_open, gap_ext, utr_gap, intron_gap)) == NULL )    {  
...
/* Function:  allocate_Expl_CdnaWise10(query,target,gp,sc,rndcodon,utr,gap_open,gap_ext,utr_gap,intron_gap)
 *
 ...
 *
 */
CdnaWise10 * allocate_Expl_CdnaWise10(ComplexSequence* query,ComplexSequence* target ,GeneParser4Score * gp,CodonMatrixScore * sc,RandomCodonScore * rndcodon,DnaMatrix* utr,Score gap_open,Score gap_ext,Score utr_gap,Score intron_gap) 
{
    CdnaWise10 * out;    
...
PackAln * PackAln_calculate_Small_CdnaWise10(CdnaWise10 * mat,DPEnvelope * dpenv) 
{
    ...
    dc_start_end_calculate_CdnaWise10(mat,dpenv);    
...
AlnRangeSet * AlnRangeSet_calculate_Small_CdnaWise10(CdnaWise10 * mat) 
{
    ...
    start_reporting("Find start end points: ");  
    dc_start_end_calculate_CdnaWise10(mat,NULL); 
...
/* Function:  dc_start_end_calculate_CdnaWise10(mat,dpenv)
 *
 ...
 *
 */
boolean dc_start_end_calculate_CdnaWise10(CdnaWise10 * mat,DPEnvelope * dpenv) 
{
    int i;   
    int j;   
    int k;   
    ...
    init_start_end_linear_CdnaWise10(mat);   
    ...
    for(j=0;j<lenj;j++)  { /*for each j strip*/ 
      for(i=0;i<leni;i++)    { /*for each i position in strip*/ 
        ...
        /* For state UTR5 */ 
        /* setting first movement to score */ 
        score = CdnaWise10_DC_SHADOW_MATRIX(mat,i-1,j-1,UTR5) + DnaMatrix_MATCH(mat->utr,CSEQ_CDNA_BASE(mat->query,i),CSEQ_GENOMIC_BASE(mat->target,j)) + (0);   
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 1,UTR5,k);   
        /* From state START to state UTR5 */ 
        temp = CdnaWise10_DC_SHADOW_SPECIAL(mat,i-1,j-1,START) + DnaMatrix_MATCH(mat->utr,CSEQ_CDNA_BASE(mat->query,i),CSEQ_GENOMIC_BASE(mat->target,j)) + (0);  
        if( temp  > score )  {  
          score = temp;  
        ...
        /* From state UTR5_INTRON to state UTR5 */ 
        temp = CdnaWise10_DC_SHADOW_MATRIX(mat,i-1,j-1,UTR5_INTRON) + DnaMatrix_MATCH(mat->utr,CSEQ_CDNA_BASE(mat->query,i),CSEQ_GENOMIC_BASE(mat->target,j)) +(0);  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 1,UTR5_INTRON,k);  
        ...
        /* From state UTR5 to state UTR5 */ 
        temp = CdnaWise10_DC_SHADOW_MATRIX(mat,i-0,j-1,UTR5) + mat->utr_gap +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,UTR5,k); 
        ...
        /* From state UTR5 to state UTR5 */ 
        temp = CdnaWise10_DC_SHADOW_MATRIX(mat,i-1,j-0,UTR5) + mat->utr_gap +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 0,UTR5,k); 
        ...
        /* For state UTR5_INTRON */ 
        /* setting first movement to score */ 
        score = CdnaWise10_DC_SHADOW_MATRIX(mat,i-0,j-1,UTR5_INTRON) + 0 + (0);  
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,UTR5_INTRON,k);    
        /* From state UTR5 to state UTR5_INTRON */ 
        temp = CdnaWise10_DC_SHADOW_MATRIX(mat,i-0,j-1,UTR5) + (CSEQ_GENOMIC_5SS(mat->target,j)+mat->intron_gap) +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,UTR5,k); 
        ...
        /* For state MATCH */ 
        /* setting first movement to score */ 
        score = CdnaWise10_DC_SHADOW_MATRIX(mat,i-3,j-3,MATCH) + mat->sc->score[CSEQ_CDNA_CODON(mat->query,i)][CSEQ_GENOMIC_CODON(mat->target,j)] + (0);     
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 3,j - 3,MATCH,k);  
        /* From state INSERT to state MATCH */ 
        temp = CdnaWise10_DC_SHADOW_MATRIX(mat,i-3,j-3,INSERT) + mat->sc->score[CSEQ_CDNA_CODON(mat->query,i)][CSEQ_GENOMIC_CODON(mat->target,j)] +(0);  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 3,j - 3,INSERT,k);   
        ...
        /* From state DELETE to state MATCH */ 
        temp = CdnaWise10_DC_SHADOW_MATRIX(mat,i-3,j-3,DELETE) + mat->sc->score[CSEQ_CDNA_CODON(mat->query,i)][CSEQ_GENOMIC_CODON(mat->target,j)] +(0);  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 3,j - 3,DELETE,k);   
        ...
        /* From state START to state MATCH */ 
        temp = CdnaWise10_DC_SHADOW_SPECIAL(mat,i-3,j-3,START) + mat->sc->score[CSEQ_CDNA_CODON(mat->query,i)][CSEQ_GENOMIC_CODON(mat->target,j)] + (0);     
        if( temp  > score )  {  
          score = temp;  
        ...
        /* From state UTR5 to state MATCH */ 
        temp = CdnaWise10_DC_SHADOW_MATRIX(mat,i-3,j-3,UTR5) + mat->sc->score[CSEQ_CDNA_CODON(mat->query,i)][CSEQ_GENOMIC_CODON(mat->target,j)] +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 3,j - 3,UTR5,k); 
        ...
        /* From state INTRON_0 to state MATCH */ 
        temp = CdnaWise10_DC_SHADOW_MATRIX(mat,i-0,j-6,INTRON_0) + CSEQ_GENOMIC_3SS(mat->target,(j-3)) +(0);     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 6,INTRON_0,k); 
        ...
        /* From state INTRON_1 to state MATCH */ 
        temp = CdnaWise10_DC_SHADOW_MATRIX(mat,i-0,j-5,INTRON_1) + CSEQ_GENOMIC_3SS(mat->target,(j-2)) +(0);     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 5,INTRON_1,k); 
        ...
        /* From state INTRON_2 to state MATCH */ 
        temp = CdnaWise10_DC_SHADOW_MATRIX(mat,i-0,j-4,INTRON_2) + CSEQ_GENOMIC_3SS(mat->target,(j-1)) +(0);     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 4,INTRON_2,k); 
        ...
        /* state MATCH is a source for special END */ 
        temp = score + (0) + (0) ;   
        if( temp > CdnaWise10_DC_SHADOW_SPECIAL(mat,i,j,END) )   {  
          CdnaWise10_DC_SHADOW_SPECIAL(mat,i,j,END) = temp;  
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            CdnaWise10_DC_SHADOW_SPECIAL_SP(mat,i,j,END,k) = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i,j,MATCH,k);    
          CdnaWise10_DC_SHADOW_SPECIAL_SP(mat,i,j,END,6) = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i,j,MATCH,6);  
          CdnaWise10_DC_SHADOW_SPECIAL_SP(mat,i,j,END,3) = i;    
          CdnaWise10_DC_SHADOW_SPECIAL_SP(mat,i,j,END,4) = j;    
          CdnaWise10_DC_SHADOW_SPECIAL_SP(mat,i,j,END,5) = MATCH;    
          }  
        ...
        /* For state INSERT */ 
        /* setting first movement to score */ 
        score = CdnaWise10_DC_SHADOW_MATRIX(mat,i-0,j-3,MATCH) + (mat->rndcodon->codon[CSEQ_GENOMIC_CODON(mat->target,j)]+mat->gap_open) + (0);  
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 3,MATCH,k);  
        /* From state INSERT to state INSERT */ 
        temp = CdnaWise10_DC_SHADOW_MATRIX(mat,i-0,j-3,INSERT) + (mat->rndcodon->codon[CSEQ_GENOMIC_CODON(mat->target,j)]+mat->gap_ext) +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 3,INSERT,k);   
        ...
        /* For state DELETE */ 
        /* setting first movement to score */ 
        score = CdnaWise10_DC_SHADOW_MATRIX(mat,i-1,j-0,MATCH) + (mat->rndcodon->codon[CSEQ_CDNA_CODON(mat->query,i)]+mat->gap_open) + (0);  
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 0,MATCH,k);  
        /* From state INSERT to state DELETE */ 
        temp = CdnaWise10_DC_SHADOW_MATRIX(mat,i-1,j-0,INSERT) + (mat->rndcodon->codon[CSEQ_CDNA_CODON(mat->query,i)]+mat->gap_ext) +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 0,INSERT,k);   
        ...
        /* For state INTRON_0 */ 
        /* setting first movement to score */ 
        score = CdnaWise10_DC_SHADOW_MATRIX(mat,i-3,j-8,MATCH) + (CSEQ_GENOMIC_5SS(mat->target,(j-7))+mat->intron_gap) + (0);    
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 3,j - 8,MATCH,k);  
        /* From state INSERT to state INTRON_0 */ 
        temp = CdnaWise10_DC_SHADOW_MATRIX(mat,i-0,j-8,INSERT) + (CSEQ_GENOMIC_5SS(mat->target,(j-7))+mat->intron_gap) +(0);     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 8,INSERT,k);   
        ...
        /* From state INTRON_0 to state INTRON_0 */ 
        temp = CdnaWise10_DC_SHADOW_MATRIX(mat,i-0,j-1,INTRON_0) + 0 +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,INTRON_0,k); 
        ...
        /* For state INTRON_1 */ 
        /* setting first movement to score */ 
        score = CdnaWise10_DC_SHADOW_MATRIX(mat,i-3,j-9,MATCH) + (CSEQ_GENOMIC_5SS(mat->target,(j-7))+mat->intron_gap) + (0);    
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 3,j - 9,MATCH,k);  
        /* From state INSERT to state INTRON_1 */ 
        temp = CdnaWise10_DC_SHADOW_MATRIX(mat,i-0,j-9,INSERT) + (CSEQ_GENOMIC_5SS(mat->target,(j-7))+mat->intron_gap) +(0);     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 9,INSERT,k);   
        ...
        /* From state INTRON_1 to state INTRON_1 */ 
        temp = CdnaWise10_DC_SHADOW_MATRIX(mat,i-0,j-1,INTRON_1) + 0 +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,INTRON_1,k); 
        ...
        /* For state INTRON_2 */ 
        /* setting first movement to score */ 
        score = CdnaWise10_DC_SHADOW_MATRIX(mat,i-3,j-10,MATCH) + (CSEQ_GENOMIC_5SS(mat->target,(j-7))+mat->intron_gap) + (0);   
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 3,j - 10,MATCH,k); 
        /* From state INSERT to state INTRON_2 */ 
        temp = CdnaWise10_DC_SHADOW_MATRIX(mat,i-0,j-10,INSERT) + (CSEQ_GENOMIC_5SS(mat->target,(j-7))+mat->intron_gap) +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 10,INSERT,k);  
        ...
        /* From state INTRON_2 to state INTRON_2 */ 
        temp = CdnaWise10_DC_SHADOW_MATRIX(mat,i-0,j-1,INTRON_2) + 0 +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,INTRON_2,k); 
        ...
        /* For state UTR3 */ 
        /* setting first movement to score */ 
        score = CdnaWise10_DC_SHADOW_MATRIX(mat,i-1,j-1,UTR3) + DnaMatrix_MATCH(mat->utr,CSEQ_CDNA_BASE(mat->query,i),CSEQ_GENOMIC_BASE(mat->target,j)) + (0);   
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 1,UTR3,k);   
        /* From state UTR3_INTRON to state UTR3 */ 
        temp = CdnaWise10_DC_SHADOW_MATRIX(mat,i-1,j-1,UTR3_INTRON) + DnaMatrix_MATCH(mat->utr,CSEQ_CDNA_BASE(mat->query,i),CSEQ_GENOMIC_BASE(mat->target,j)) +(0);  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 1,UTR3_INTRON,k);  
        ...
        /* From state UTR3 to state UTR3 */ 
        temp = CdnaWise10_DC_SHADOW_MATRIX(mat,i-0,j-1,UTR3) + mat->utr_gap +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,UTR3,k); 
        ...
        /* From state UTR3 to state UTR3 */ 
        temp = CdnaWise10_DC_SHADOW_MATRIX(mat,i-1,j-0,UTR3) + mat->utr_gap +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 0,UTR3,k); 
        ...
        if( temp > CdnaWise10_DC_SHADOW_SPECIAL(mat,i,j,END) )   {  
          ...
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            CdnaWise10_DC_SHADOW_SPECIAL_SP(mat,i,j,END,k) = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i,j,UTR3,k); 
          CdnaWise10_DC_SHADOW_SPECIAL_SP(mat,i,j,END,6) = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i,j,UTR3,6);   
          CdnaWise10_DC_SHADOW_SPECIAL_SP(mat,i,j,END,3) = i;    
          CdnaWise10_DC_SHADOW_SPECIAL_SP(mat,i,j,END,4) = j;    
          CdnaWise10_DC_SHADOW_SPECIAL_SP(mat,i,j,END,5) = UTR3; 
          }  
        ...
        /* For state UTR3_INTRON */ 
        /* setting first movement to score */ 
        score = CdnaWise10_DC_SHADOW_MATRIX(mat,i-0,j-1,UTR3_INTRON) + 0 + (0);  
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,UTR3_INTRON,k);    
        /* From state UTR3 to state UTR3_INTRON */ 
        temp = CdnaWise10_DC_SHADOW_MATRIX(mat,i-0,j-1,UTR3) + (CSEQ_GENOMIC_5SS(mat->target,j)+mat->intron_gap) +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CdnaWise10_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,UTR3,k); 
...
PackAln * PackAln_read_Expl_CdnaWise10(CdnaWise10 * mat) 
{
    ...
    if( mat->basematrix->type != BASEMATRIX_TYPE_EXPLICIT)   {  
      warn("In CdnaWise10_basic_read You have asked for an alignment from a non-explicit matrix: c'est impossible [current type is %d - %s]", mat->basematrix->type,basematrix_type_to_string(mat->basematrix->type));   
      return NULL;   
      }  
    ...
    out->score =  find_end_CdnaWise10(mat,&i,&j,&state,&isspecial);  
    ...
    while( state != START || isspecial != TRUE)  { /*while state != START*/ 
      ...
      if( isspecial == TRUE )    
        max_calc_special_CdnaWise10(mat,i,j,state,isspecial,&i,&j,&state,&isspecial,&cellscore);     
      else   
        max_calc_CdnaWise10(mat,i,j,state,isspecial,&i,&j,&state,&isspecial,&cellscore);     
...
/* Function:  find_end_CdnaWise10(mat,ri,rj,state,isspecial)
 *
 * Descrip: No Description
 *
 * Arg:              mat [UNKN ] Undocumented argument [CdnaWise10 *]
 ...
 *
 */
int find_end_CdnaWise10(CdnaWise10 * mat,int * ri,int * rj,int * state,boolean * isspecial) 
{
    register int j;  
    register int max;    
    register int maxj;   
    ...
    max = CdnaWise10_EXPL_SPECIAL(mat,0,mat->target->seq->len-1,END);    
    maxj = mat->target->seq->len-1;  
    for(j= mat->target->seq->len-2 ;j >= 0 ;j--) {  
      if( CdnaWise10_EXPL_SPECIAL(mat,0,j,END) > max )   {  
        max = CdnaWise10_EXPL_SPECIAL(mat,0,j,END);  
        maxj = j;    
        }  
      }  
...
/* Function:  max_calc_CdnaWise10(mat,i,j,state,isspecial,reti,retj,retstate,retspecial,cellscore)
 *
 * Descrip: No Description
 *
 * Arg:               mat [UNKN ] Undocumented argument [CdnaWise10 *]
 ...
 *
 */
int max_calc_CdnaWise10(CdnaWise10 * mat,int i,int j,int state,boolean isspecial,int * reti,int * retj,int * retstate,boolean * retspecial,int * cellscore) 
{
    register int temp;   
    register int cscore; 
    ...
    *reti = (*retj) = (*retstate) = CdnaWise10_READ_OFF_ERROR;   
    ...
    /* Then you have to select the correct switch statement to figure out the readoff      */ 
    /* Somewhat odd - reverse the order of calculation and return as soon as it is correct */ 
    cscore = CdnaWise10_EXPL_MATRIX(mat,i,j,state);  
    switch(state)    { /*Switch state */ 
      case UTR5 :    
        temp = cscore - (mat->utr_gap) -  (0);   
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 1,j - 0,UTR5) )   {  
          *reti = i - 1; 
          *retj = j - 0; 
          *retstate = UTR5;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-1,j-0,UTR5);  
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 1,j - 0,UTR5);   
          }  
        temp = cscore - (mat->utr_gap) -  (0);   
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 1,UTR5) )   {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = UTR5;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-0,j-1,UTR5);  
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 1,UTR5);   
          }  
        temp = cscore - (DnaMatrix_MATCH(mat->utr,CSEQ_CDNA_BASE(mat->query,i),CSEQ_GENOMIC_BASE(mat->target,j))) -  (0);    
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 1,j - 1,UTR5_INTRON) )    {  
          *reti = i - 1; 
          *retj = j - 1; 
          *retstate = UTR5_INTRON;   
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-1,j-1,UTR5_INTRON);   
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 1,j - 1,UTR5_INTRON);    
          }  
        temp = cscore - (DnaMatrix_MATCH(mat->utr,CSEQ_CDNA_BASE(mat->query,i),CSEQ_GENOMIC_BASE(mat->target,j))) -  (0);    
        if( temp == CdnaWise10_EXPL_SPECIAL(mat,i - 1,j - 1,START) ) {  
          *reti = i - 1; 
          *retj = j - 1; 
          *retstate = START; 
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_SPECIAL(mat,i-1,j-1,START);    
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 1,j - 1,START);  
          }  
        temp = cscore - (DnaMatrix_MATCH(mat->utr,CSEQ_CDNA_BASE(mat->query,i),CSEQ_GENOMIC_BASE(mat->target,j))) -  (0);    
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 1,j - 1,UTR5) )   {  
          *reti = i - 1; 
          *retj = j - 1; 
          *retstate = UTR5;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-1,j-1,UTR5);  
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 1,j - 1,UTR5);   
          }  
        warn("Major problem (!) - in CdnaWise10 read off, position %d,%d state %d no source found!",i,j,state);  
        return (-1); 
      case UTR5_INTRON :     
        ...
        temp = cscore - (0) -  (0);  
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 1,UTR5_INTRON) )    {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = UTR5_INTRON;   
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-0,j-1,UTR5_INTRON);   
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 1,UTR5_INTRON);    
          }  
        warn("Major problem (!) - in CdnaWise10 read off, position %d,%d state %d no source found!",i,j,state);  
        return (-1); 
      case MATCH :   
        temp = cscore - (CSEQ_GENOMIC_3SS(mat->target,(j-1))) -  (0);    
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 4,INTRON_2) )   {  
          *reti = i - 0; 
          *retj = j - 4; 
          *retstate = INTRON_2;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-0,j-4,INTRON_2);  
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 4,INTRON_2);   
          }  
        temp = cscore - (CSEQ_GENOMIC_3SS(mat->target,(j-2))) -  (0);    
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 5,INTRON_1) )   {  
          *reti = i - 0; 
          *retj = j - 5; 
          *retstate = INTRON_1;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-0,j-5,INTRON_1);  
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 5,INTRON_1);   
          }  
        temp = cscore - (CSEQ_GENOMIC_3SS(mat->target,(j-3))) -  (0);    
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 6,INTRON_0) )   {  
          *reti = i - 0; 
          *retj = j - 6; 
          *retstate = INTRON_0;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-0,j-6,INTRON_0);  
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 6,INTRON_0);   
          }  
        temp = cscore - (mat->sc->score[CSEQ_CDNA_CODON(mat->query,i)][CSEQ_GENOMIC_CODON(mat->target,j)]) -  (0);   
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 3,j - 3,UTR5) )   {  
          *reti = i - 3; 
          *retj = j - 3; 
          *retstate = UTR5;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-3,j-3,UTR5);  
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 3,j - 3,UTR5);   
          }  
        temp = cscore - (mat->sc->score[CSEQ_CDNA_CODON(mat->query,i)][CSEQ_GENOMIC_CODON(mat->target,j)]) -  (0);   
        if( temp == CdnaWise10_EXPL_SPECIAL(mat,i - 3,j - 3,START) ) {  
          *reti = i - 3; 
          *retj = j - 3; 
          *retstate = START; 
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_SPECIAL(mat,i-3,j-3,START);    
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 3,j - 3,START);  
          }  
        temp = cscore - (mat->sc->score[CSEQ_CDNA_CODON(mat->query,i)][CSEQ_GENOMIC_CODON(mat->target,j)]) -  (0);   
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 3,j - 3,DELETE) ) {  
          *reti = i - 3; 
          *retj = j - 3; 
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-3,j-3,DELETE);    
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 3,j - 3,DELETE);     
          }  
        temp = cscore - (mat->sc->score[CSEQ_CDNA_CODON(mat->query,i)][CSEQ_GENOMIC_CODON(mat->target,j)]) -  (0);   
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 3,j - 3,INSERT) ) {  
          *reti = i - 3; 
          *retj = j - 3; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-3,j-3,INSERT);    
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 3,j - 3,INSERT);     
          }  
        temp = cscore - (mat->sc->score[CSEQ_CDNA_CODON(mat->query,i)][CSEQ_GENOMIC_CODON(mat->target,j)]) -  (0);   
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 3,j - 3,MATCH) )  {  
          *reti = i - 3; 
          *retj = j - 3; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-3,j-3,MATCH); 
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 3,j - 3,MATCH);  
          }  
        warn("Major problem (!) - in CdnaWise10 read off, position %d,%d state %d no source found!",i,j,state);  
        return (-1); 
      case INSERT :  
        ...
        temp = cscore - ((mat->rndcodon->codon[CSEQ_GENOMIC_CODON(mat->target,j)]+mat->gap_ext)) -  (0); 
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 3,INSERT) ) {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-0,j-3,INSERT);    
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 3,INSERT);     
          }  
        temp = cscore - ((mat->rndcodon->codon[CSEQ_GENOMIC_CODON(mat->target,j)]+mat->gap_open)) -  (0);    
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 3,MATCH) )  {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-0,j-3,MATCH); 
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 3,MATCH);  
          }  
        warn("Major problem (!) - in CdnaWise10 read off, position %d,%d state %d no source found!",i,j,state);  
        return (-1); 
      case DELETE :  
        temp = cscore - ((mat->rndcodon->codon[CSEQ_CDNA_CODON(mat->query,i)]+mat->gap_ext)) -  (0); 
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 1,j - 0,INSERT) ) {  
          *reti = i - 1; 
          *retj = j - 0; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-1,j-0,INSERT);    
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 1,j - 0,INSERT);     
          }  
        temp = cscore - ((mat->rndcodon->codon[CSEQ_CDNA_CODON(mat->query,i)]+mat->gap_open)) -  (0);    
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 1,j - 0,MATCH) )  {  
          *reti = i - 1; 
          *retj = j - 0; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-1,j-0,MATCH); 
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 1,j - 0,MATCH);  
          }  
        warn("Major problem (!) - in CdnaWise10 read off, position %d,%d state %d no source found!",i,j,state);  
        return (-1); 
      case INTRON_0 :    
        temp = cscore - (0) -  (0);  
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 1,INTRON_0) )   {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = INTRON_0;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-0,j-1,INTRON_0);  
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 1,INTRON_0);   
          }  
        temp = cscore - ((CSEQ_GENOMIC_5SS(mat->target,(j-7))+mat->intron_gap)) -  (0);  
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 8,INSERT) ) {  
          *reti = i - 0; 
          *retj = j - 8; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-0,j-8,INSERT);    
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 8,INSERT);     
          }  
        temp = cscore - ((CSEQ_GENOMIC_5SS(mat->target,(j-7))+mat->intron_gap)) -  (0);  
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 3,j - 8,MATCH) )  {  
          *reti = i - 3; 
          *retj = j - 8; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-3,j-8,MATCH); 
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 3,j - 8,MATCH);  
          }  
        warn("Major problem (!) - in CdnaWise10 read off, position %d,%d state %d no source found!",i,j,state);  
        return (-1); 
      case INTRON_1 :    
        temp = cscore - (0) -  (0);  
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 1,INTRON_1) )   {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = INTRON_1;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-0,j-1,INTRON_1);  
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 1,INTRON_1);   
          }  
        temp = cscore - ((CSEQ_GENOMIC_5SS(mat->target,(j-7))+mat->intron_gap)) -  (0);  
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 9,INSERT) ) {  
          *reti = i - 0; 
          *retj = j - 9; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-0,j-9,INSERT);    
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 9,INSERT);     
          }  
        temp = cscore - ((CSEQ_GENOMIC_5SS(mat->target,(j-7))+mat->intron_gap)) -  (0);  
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 3,j - 9,MATCH) )  {  
          *reti = i - 3; 
          *retj = j - 9; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-3,j-9,MATCH); 
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 3,j - 9,MATCH);  
          }  
        warn("Major problem (!) - in CdnaWise10 read off, position %d,%d state %d no source found!",i,j,state);  
        return (-1); 
      case INTRON_2 :    
        temp = cscore - (0) -  (0);  
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 1,INTRON_2) )   {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = INTRON_2;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-0,j-1,INTRON_2);  
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 1,INTRON_2);   
          }  
        temp = cscore - ((CSEQ_GENOMIC_5SS(mat->target,(j-7))+mat->intron_gap)) -  (0);  
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 10,INSERT) )    {  
          *reti = i - 0; 
          *retj = j - 10;    
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-0,j-10,INSERT);   
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 10,INSERT);    
          }  
        temp = cscore - ((CSEQ_GENOMIC_5SS(mat->target,(j-7))+mat->intron_gap)) -  (0);  
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 3,j - 10,MATCH) ) {  
          *reti = i - 3; 
          *retj = j - 10;    
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-3,j-10,MATCH);    
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 3,j - 10,MATCH);     
          }  
        warn("Major problem (!) - in CdnaWise10 read off, position %d,%d state %d no source found!",i,j,state);  
        return (-1); 
      case UTR3 :    
        temp = cscore - (mat->utr_gap) -  (0);   
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 1,j - 0,UTR3) )   {  
          *reti = i - 1; 
          *retj = j - 0; 
          *retstate = UTR3;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-1,j-0,UTR3);  
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 1,j - 0,UTR3);   
          }  
        temp = cscore - (mat->utr_gap) -  (0);   
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 1,UTR3) )   {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = UTR3;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-0,j-1,UTR3);  
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 1,UTR3);   
          }  
        temp = cscore - (DnaMatrix_MATCH(mat->utr,CSEQ_CDNA_BASE(mat->query,i),CSEQ_GENOMIC_BASE(mat->target,j))) -  (0);    
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 1,j - 1,UTR3_INTRON) )    {  
          *reti = i - 1; 
          *retj = j - 1; 
          *retstate = UTR3_INTRON;   
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-1,j-1,UTR3_INTRON);   
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 1,j - 1,UTR3_INTRON);    
          }  
        temp = cscore - (DnaMatrix_MATCH(mat->utr,CSEQ_CDNA_BASE(mat->query,i),CSEQ_GENOMIC_BASE(mat->target,j))) -  (0);    
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 1,j - 1,UTR3) )   {  
          *reti = i - 1; 
          *retj = j - 1; 
          *retstate = UTR3;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-1,j-1,UTR3);  
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 1,j - 1,UTR3);   
          }  
        warn("Major problem (!) - in CdnaWise10 read off, position %d,%d state %d no source found!",i,j,state);  
        return (-1); 
      case UTR3_INTRON :     
        ...
        temp = cscore - (0) -  (0);  
        if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 1,UTR3_INTRON) )    {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = UTR3_INTRON;   
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-0,j-1,UTR3_INTRON);   
            }  
          return CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 1,UTR3_INTRON);    
          }  
        warn("Major problem (!) - in CdnaWise10 read off, position %d,%d state %d no source found!",i,j,state);  
        return (-1); 
      default:   
...
/* Function:  max_calc_special_CdnaWise10(mat,i,j,state,isspecial,reti,retj,retstate,retspecial,cellscore)
 *
 * Descrip: No Description
 *
 * Arg:               mat [UNKN ] Undocumented argument [CdnaWise10 *]
 ...
 *
 */
int max_calc_special_CdnaWise10(CdnaWise10 * mat,int i,int j,int state,boolean isspecial,int * reti,int * retj,int * retstate,boolean * retspecial,int * cellscore) 
{
    register int temp;   
    register int cscore; 
    ...
    *reti = (*retj) = (*retstate) = CdnaWise10_READ_OFF_ERROR;   
    ...
    cscore = CdnaWise10_EXPL_SPECIAL(mat,i,j,state); 
    switch(state)    { /*switch on special states*/ 
      case START :   
      case END :     
        ...
        for(i= mat->query->seq->len-1;i >= 0 ;i--)   { /*for i >= 0*/ 
          temp = cscore - (0) - (0);     
          if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 0,UTR3) ) {  
            *reti = i - 0;   
            *retj = j - 0;   
            *retstate = UTR3;    
            *retspecial = FALSE; 
            if( cellscore != NULL)   {  
              *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-0,j-0,UTR3);    
              }  
            return CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 0,UTR3) ;    
            }  
          } /* end of for i >= 0 */ 
        /* source MATCH is from main matrix */ 
        for(i= mat->query->seq->len-1;i >= 0 ;i--)   { /*for i >= 0*/ 
          temp = cscore - (0) - (0);     
          if( temp == CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 0,MATCH) )    {  
            *reti = i - 0;   
            *retj = j - 0;   
            *retstate = MATCH;   
            *retspecial = FALSE; 
            if( cellscore != NULL)   {  
              *cellscore = cscore - CdnaWise10_EXPL_MATRIX(mat,i-0,j-0,MATCH);   
              }  
            return CdnaWise10_EXPL_MATRIX(mat,i - 0,j - 0,MATCH) ;   
            }  
          } /* end of for i >= 0 */ 
      default:   
