 ...
 * Arg:                 mode [UNKN ] Undocumented argument [int]
 * Arg:          wing_length [UNKN ] Undocumented argument [int]
 * Arg:        internal_bias [UNKN ] Undocumented argument [Probability]
 *
 */
# line 138 "threestatemodel.dy"
void set_startend_policy_ThreeStateModel(ThreeStateModel * tsm,int mode,int wing_length,Probability internal_bias)
{
...
ThreeStateUnit *  ThreeStateUnit_from_half_bit_aminoacid(char aa,CompMat * mat,RandomModel * rm,int gap,int ext)
{
  ...
  error = renormalise_Probability_array(tsu->match_emission,26);

  error = renormalise_Probability_array(tsu->insert_emission,26);
  if( fabs(error) > 0.001 ) {
    warn("Very bad problem - got a stupid error %f after renormalisation from the null model\n",error);
  }
