...
Search_Return_Type search_GeneWise21(DBSearchImpl * dbsi,Hscore * out,GeneWiseDB* querydb,GenomicDB* targetdb ,GeneParser21Score * gp21,RandomCodonScore * rndcodon,RandomModelDNAScore * rndbase) 
{
    ...
    if( dbsi->trace_level > 5 )  
      warn("Asking for trace level of %d in database search for GeneWise21, but it was compiled with a trace level of 416. Not all trace statements can be shown",dbsi->trace_level);    
...
PackAln * PackAln_bestmemory_GeneWise21(GeneWiseScore* query,ComplexSequence* target ,GeneParser21Score * gp21,RandomCodonScore * rndcodon,RandomModelDNAScore * rndbase,DPEnvelope * dpenv,DPRunImpl * dpri) 
{
    ...
    if( strategy == DPIM_Linear )    {  
      ...
      if( (mat=allocate_Expl_GeneWise21(query, target , gp21, rndcodon, rndbase)) == NULL )  {  
...
/* Function:  allocate_Expl_GeneWise21(query,target,gp21,rndcodon,rndbase)
 *
 ...
 *
 */
GeneWise21 * allocate_Expl_GeneWise21(GeneWiseScore* query,ComplexSequence* target ,GeneParser21Score * gp21,RandomCodonScore * rndcodon,RandomModelDNAScore * rndbase) 
{
    GeneWise21 * out;    
...
PackAln * PackAln_calculate_Small_GeneWise21(GeneWise21 * mat,DPEnvelope * dpenv) 
{
    ...
    dc_start_end_calculate_GeneWise21(mat,dpenv);    
...
AlnRangeSet * AlnRangeSet_calculate_Small_GeneWise21(GeneWise21 * mat) 
{
    ...
    start_reporting("Find start end points: ");  
    dc_start_end_calculate_GeneWise21(mat,NULL); 
...
/* Function:  dc_start_end_calculate_GeneWise21(mat,dpenv)
 *
 ...
 *
 */
boolean dc_start_end_calculate_GeneWise21(GeneWise21 * mat,DPEnvelope * dpenv) 
{
    int i;   
    int j;   
    int k;   
    ...
    init_start_end_linear_GeneWise21(mat);   
    ...
    for(j=0;j<lenj;j++)  { /*for each j strip*/ 
      for(i=0;i<leni;i++)    { /*for each i position in strip*/ 
        ...
        /* For state MATCH */ 
        /* setting first movement to score */ 
        score = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-3,MATCH) + (mat->query->seg[i]->transition[GW_MATCH2MATCH]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)]) + (CSEQ_GENOMIC_CDSPOT(mat->target,j));   
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 3,MATCH,k);  
        /* From state INSERT to state MATCH */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-3,INSERT) + (mat->query->seg[i]->transition[GW_INSERT2MATCH]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)]) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 3,INSERT,k);   
        ...
        /* From state DELETE to state MATCH */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-3,DELETE) + (mat->query->seg[i]->transition[GW_DELETE2MATCH]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)]) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 3,DELETE,k);   
        ...
        /* From state BEFORE_CODON to state MATCH */ 
        temp = GeneWise21_DC_SHADOW_SPECIAL(mat,i-1,j-3,BEFORE_CODON) + (mat->query->seg[i]->transition[GW_START2MATCH]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)]) + (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp  > score )  {  
          score = temp;  
        ...
        /* From state SPACER_0M to state MATCH */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-6,SPACER_0M) + (((mat->gp21->transition[GP21_SPACER2CDS]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS]) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 6,SPACER_0M,k);    
        ...
        /* From state PY_0M to state MATCH */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-6,PY_0M) + (((mat->gp21->transition[GP21_PY2CDS]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS]) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 6,PY_0M,k);    
        ...
        /* From state SPACER_1M to state MATCH */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-5,SPACER_1M) + (mat->gp21->transition[GP21_SPACER2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-2))) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 5,SPACER_1M,k);    
        ...
        /* From state PY_1M to state MATCH */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-5,PY_1M) + (mat->gp21->transition[GP21_PY2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-2))) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 5,PY_1M,k);    
        ...
        /* From state SPACER_2M to state MATCH */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-4,SPACER_2M) + (mat->gp21->transition[GP21_SPACER2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-1))) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 4,SPACER_2M,k);    
        ...
        /* From state PY_2M to state MATCH */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-4,PY_2M) + (mat->gp21->transition[GP21_PY2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-1))) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 4,PY_2M,k);    
        ...
        /* From state MATCH to state MATCH */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-2,MATCH) + mat->gp21->transition[GP21_DELETE_1_BASE] +(CSEQ_GENOMIC_CDSPOT(mat->target,j));     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 2,MATCH,k);    
        ...
        /* From state MATCH to state MATCH */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-1,MATCH) + mat->gp21->transition[GP21_DELETE_2_BASE] +(CSEQ_GENOMIC_CDSPOT(mat->target,j));     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 1,MATCH,k);    
        ...
        /* From state MATCH to state MATCH */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-4,MATCH) + mat->gp21->transition[GP21_INSERT_1_BASE] +(CSEQ_GENOMIC_CDSPOT(mat->target,j));     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 4,MATCH,k);    
        ...
        /* From state MATCH to state MATCH */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-5,MATCH) + mat->gp21->transition[GP21_INSERT_2_BASE] +(CSEQ_GENOMIC_CDSPOT(mat->target,j));     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 5,MATCH,k);    
        ...
        /* state MATCH is a source for special AFTER_CODON */ 
        temp = score + (mat->query->seg[i]->transition[GW_MATCH2END]) + (0) ;    
        if( temp > GeneWise21_DC_SHADOW_SPECIAL(mat,i,j,AFTER_CODON) )   {  
          GeneWise21_DC_SHADOW_SPECIAL(mat,i,j,AFTER_CODON) = temp;  
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_CODON,k) = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i,j,MATCH,k);    
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_CODON,6) = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i,j,MATCH,6);  
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_CODON,3) = i;    
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_CODON,4) = j;    
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_CODON,5) = MATCH;    
          }  
        ...
        /* For state INSERT */ 
        /* setting first movement to score */ 
        score = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-3,MATCH) + (mat->query->seg[i]->transition[GW_MATCH2INSERT]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)]) + (CSEQ_GENOMIC_CDSPOT(mat->target,j));     
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 3,MATCH,k);  
        /* From state INSERT to state INSERT */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-3,INSERT) + (mat->query->seg[i]->transition[GW_INSERT2INSERT]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)]) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 3,INSERT,k);   
        ...
        /* From state DELETE to state INSERT */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-3,DELETE) + (mat->query->seg[i]->transition[GW_DELETE2INSERT]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)]) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 3,DELETE,k);   
        ...
        /* From state BEFORE_CODON to state INSERT */ 
        temp = GeneWise21_DC_SHADOW_SPECIAL(mat,i-0,j-3,BEFORE_CODON) + (mat->query->seg[i]->transition[GW_START2INSERT]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)]) + (CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp  > score )  {  
          score = temp;  
        ...
        /* From state SPACER_0I to state INSERT */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-6,SPACER_0I) + (((mat->gp21->transition[GP21_SPACER2CDS]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS]) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 6,SPACER_0I,k);    
        ...
        /* From state SPACER_1I to state INSERT */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-5,SPACER_1I) + (mat->gp21->transition[GP21_SPACER2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-2))) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 5,SPACER_1I,k);    
        ...
        /* From state SPACER_2I to state INSERT */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-4,SPACER_2I) + (mat->gp21->transition[GP21_SPACER2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-1))) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 4,SPACER_2I,k);    
        ...
        /* From state PY_0I to state INSERT */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-6,PY_0I) + (((mat->gp21->transition[GP21_PY2CDS]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS]) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 6,PY_0I,k);    
        ...
        /* From state PY_1I to state INSERT */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-5,PY_1I) + (mat->gp21->transition[GP21_PY2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-2))) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 5,PY_1I,k);    
        ...
        /* From state PY_2I to state INSERT */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-4,PY_2I) + (mat->gp21->transition[GP21_PY2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-1))) +(CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 4,PY_2I,k);    
        ...
        /* From state INSERT to state INSERT */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-2,INSERT) + mat->gp21->transition[GP21_DELETE_1_BASE] +(CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 2,INSERT,k);   
        ...
        /* From state INSERT to state INSERT */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-1,INSERT) + mat->gp21->transition[GP21_DELETE_2_BASE] +(CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 1,INSERT,k);   
        ...
        /* From state INSERT to state INSERT */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-4,INSERT) + mat->gp21->transition[GP21_INSERT_1_BASE] +(CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 4,INSERT,k);   
        ...
        /* From state INSERT to state INSERT */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-5,INSERT) + mat->gp21->transition[GP21_INSERT_2_BASE] +(CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 5,INSERT,k);   
        ...
        if( temp > GeneWise21_DC_SHADOW_SPECIAL(mat,i,j,AFTER_CODON) )   {  
          ...
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_CODON,k) = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i,j,INSERT,k);   
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_CODON,6) = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i,j,INSERT,6); 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_CODON,3) = i;    
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_CODON,4) = j;    
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_CODON,5) = INSERT;   
          }  
        ...
        /* For state DELETE */ 
        /* setting first movement to score */ 
        score = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-0,MATCH) + mat->query->seg[i]->transition[GW_MATCH2DELETE] + (0);  
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 0,MATCH,k);  
        /* From state INSERT to state DELETE */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-0,INSERT) + mat->query->seg[i]->transition[GW_INSERT2DELETE] +(0);  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 0,INSERT,k);   
        ...
        /* From state DELETE to state DELETE */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-1,j-0,DELETE) + mat->query->seg[i]->transition[GW_DELETE2DELETE] +(0);  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 0,DELETE,k);   
        ...
        /* From state BEFORE_CODON to state DELETE */ 
        temp = GeneWise21_DC_SHADOW_SPECIAL(mat,i-1,j-0,BEFORE_CODON) + mat->query->seg[i]->transition[GW_START2DELETE] + (0);   
        if( temp  > score )  {  
          score = temp;  
        ...
        if( temp > GeneWise21_DC_SHADOW_SPECIAL(mat,i,j,AFTER_CODON) )   {  
          ...
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_CODON,k) = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i,j,DELETE,k);   
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_CODON,6) = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i,j,DELETE,6); 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_CODON,3) = i;    
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_CODON,4) = j;    
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_CODON,5) = DELETE;   
          }  
        ...
        /* For state CENTRAL_0M */ 
        /* setting first movement to score */ 
        score = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-8,MATCH) + (((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH2MATCH])+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]) + (0);   
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 8,MATCH,k);  
        /* From state INSERT to state CENTRAL_0M */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-8,INSERT) + (((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_INSERT2MATCH])+mat->query->seg[i]->transition[GW_INSERT_BALANCE_5SS]) +(0);  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 8,INSERT,k);   
        ...
        /* From state DELETE to state CENTRAL_0M */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-8,DELETE) + ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_DELETE2MATCH]) +(0);  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 8,DELETE,k);   
        ...
        /* From state CENTRAL_0M to state CENTRAL_0M */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,CENTRAL_0M) + (mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp21->transition[GP21_CENTRAL2CENTRAL]) +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,CENTRAL_0M,k);   
        ...
        /* For state PY_0M */ 
        /* setting first movement to score */ 
        score = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,CENTRAL_0M) + mat->gp21->transition[GP21_CENTRAL2PY] + (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,CENTRAL_0M,k); 
        /* From state PY_0M to state PY_0M */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,PY_0M) + mat->gp21->transition[GP21_PY2PY] +(mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,PY_0M,k);    
        ...
        /* For state SPACER_0M */ 
        /* setting first movement to score */ 
        score = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,PY_0M) + mat->gp21->transition[GP21_PY2SPACER] + (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);  
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,PY_0M,k);  
        /* From state SPACER_0M to state SPACER_0M */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,SPACER_0M) + mat->gp21->transition[GP21_SPACER2SPACER] +(mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,SPACER_0M,k);    
        ...
        /* For state CENTRAL_1M */ 
        /* setting first movement to score */ 
        score = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-9,MATCH) + (((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH2MATCH])+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]) + (0);   
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 9,MATCH,k);  
        /* From state INSERT to state CENTRAL_1M */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-9,INSERT) + (((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_INSERT2MATCH])+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]) +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 9,INSERT,k);   
        ...
        /* From state DELETE to state CENTRAL_1M */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-9,DELETE) + ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_DELETE2MATCH]) +(0);  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 9,DELETE,k);   
        ...
        /* From state CENTRAL_1M to state CENTRAL_1M */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,CENTRAL_1M) + (mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp21->transition[GP21_CENTRAL2CENTRAL]) +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,CENTRAL_1M,k);   
        ...
        /* For state PY_1M */ 
        /* setting first movement to score */ 
        score = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,CENTRAL_1M) + mat->gp21->transition[GP21_CENTRAL2PY] + (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,CENTRAL_1M,k); 
        /* From state PY_1M to state PY_1M */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,PY_1M) + mat->gp21->transition[GP21_PY2PY] +(mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,PY_1M,k);    
        ...
        /* For state SPACER_1M */ 
        /* setting first movement to score */ 
        score = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,PY_1M) + mat->gp21->transition[GP21_PY2SPACER] + (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);  
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,PY_1M,k);  
        /* From state SPACER_1M to state SPACER_1M */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,SPACER_1M) + mat->gp21->transition[GP21_SPACER2SPACER] +(mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,SPACER_1M,k);    
        ...
        /* For state CENTRAL_2M */ 
        /* setting first movement to score */ 
        score = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-10,MATCH) + (((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH2MATCH])+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]) + (0);  
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 10,MATCH,k); 
        /* From state INSERT to state CENTRAL_2M */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-10,INSERT) + (((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_INSERT2MATCH])+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]) +(0);  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 10,INSERT,k);  
        ...
        /* From state DELETE to state CENTRAL_2M */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-10,DELETE) + ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_DELETE2MATCH]) +(0);     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 10,DELETE,k);  
        ...
        /* From state CENTRAL_2M to state CENTRAL_2M */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,CENTRAL_2M) + (mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp21->transition[GP21_CENTRAL2CENTRAL]) +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,CENTRAL_2M,k);   
        ...
        /* For state PY_2M */ 
        /* setting first movement to score */ 
        score = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,CENTRAL_2M) + mat->gp21->transition[GP21_CENTRAL2PY] + (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,CENTRAL_2M,k); 
        /* From state PY_2M to state PY_2M */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,PY_2M) + mat->gp21->transition[GP21_PY2PY] +(mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,PY_2M,k);    
        ...
        /* For state SPACER_2M */ 
        /* setting first movement to score */ 
        score = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,PY_2M) + mat->gp21->transition[GP21_PY2SPACER] + (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);  
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,PY_2M,k);  
        /* From state SPACER_2M to state SPACER_2M */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,SPACER_2M) + mat->gp21->transition[GP21_SPACER2SPACER] +(mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,SPACER_2M,k);    
        ...
        /* For state CENTRAL_0I */ 
        /* setting first movement to score */ 
        score = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-8,MATCH) + (((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH2INSERT])+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS]) + (0);  
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 8,MATCH,k);  
        /* From state INSERT to state CENTRAL_0I */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-8,INSERT) + (((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_INSERT2INSERT])+mat->query->seg[i]->transition[GW_INSERT_BALANCE_5SS]) +(0);     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 8,INSERT,k);   
        ...
        /* From state DELETE to state CENTRAL_0I */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-8,DELETE) + ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_DELETE2INSERT]) +(0);     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 8,DELETE,k);   
        ...
        /* From state CENTRAL_0I to state CENTRAL_0I */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,CENTRAL_0I) + (mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp21->transition[GP21_CENTRAL2CENTRAL]) +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,CENTRAL_0I,k);   
        ...
        /* For state PY_0I */ 
        /* setting first movement to score */ 
        score = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,CENTRAL_0I) + mat->gp21->transition[GP21_CENTRAL2PY] + (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,CENTRAL_0I,k); 
        /* From state PY_0I to state PY_0I */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,PY_0I) + mat->gp21->transition[GP21_PY2PY] +(mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,PY_0I,k);    
        ...
        /* For state SPACER_0I */ 
        /* setting first movement to score */ 
        score = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,PY_0I) + mat->gp21->transition[GP21_PY2SPACER] + (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);  
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,PY_0I,k);  
        /* From state SPACER_0I to state SPACER_0I */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,SPACER_0I) + mat->gp21->transition[GP21_SPACER2SPACER] +(mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,SPACER_0I,k);    
        ...
        /* For state CENTRAL_1I */ 
        /* setting first movement to score */ 
        score = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-9,MATCH) + ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH2INSERT]) + (0);     
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 9,MATCH,k);  
        /* From state INSERT to state CENTRAL_1I */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-9,INSERT) + ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_INSERT2INSERT]) +(0);     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 9,INSERT,k);   
        ...
        /* From state DELETE to state CENTRAL_1I */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-9,DELETE) + ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_DELETE2INSERT]) +(0);     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 9,DELETE,k);   
        ...
        /* From state CENTRAL_1I to state CENTRAL_1I */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,CENTRAL_1I) + (mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp21->transition[GP21_CENTRAL2CENTRAL]) +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,CENTRAL_1I,k);   
        ...
        /* For state PY_1I */ 
        /* setting first movement to score */ 
        score = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,CENTRAL_1I) + mat->gp21->transition[GP21_CENTRAL2PY] + (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,CENTRAL_1I,k); 
        /* From state PY_1I to state PY_1I */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,PY_1I) + mat->gp21->transition[GP21_PY2PY] +(mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,PY_1I,k);    
        ...
        /* For state SPACER_1I */ 
        /* setting first movement to score */ 
        score = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,PY_1I) + mat->gp21->transition[GP21_PY2SPACER] + (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);  
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,PY_1I,k);  
        /* From state SPACER_1I to state SPACER_1I */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,SPACER_1I) + mat->gp21->transition[GP21_SPACER2SPACER] +(mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,SPACER_1I,k);    
        ...
        /* For state CENTRAL_2I */ 
        /* setting first movement to score */ 
        score = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-10,MATCH) + ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH2INSERT]) + (0);    
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 10,MATCH,k); 
        /* From state INSERT to state CENTRAL_2I */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-10,INSERT) + ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_INSERT2INSERT]) +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 10,INSERT,k);  
        ...
        /* From state INSERT to state CENTRAL_2I */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-10,INSERT) + ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_DELETE2INSERT]) +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 10,INSERT,k);  
        ...
        /* From state CENTRAL_2I to state CENTRAL_2I */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,CENTRAL_2I) + (mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp21->transition[GP21_CENTRAL2CENTRAL]) +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,CENTRAL_2I,k);   
        ...
        /* For state PY_2I */ 
        /* setting first movement to score */ 
        score = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,CENTRAL_2I) + mat->gp21->transition[GP21_CENTRAL2PY] + (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,CENTRAL_2I,k); 
        /* From state PY_2I to state PY_2I */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,PY_2I) + mat->gp21->transition[GP21_PY2PY] +(mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,PY_2I,k);    
        ...
        /* For state SPACER_2I */ 
        /* setting first movement to score */ 
        score = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,PY_2I) + mat->gp21->transition[GP21_PY2SPACER] + (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);  
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,PY_2I,k);  
        /* From state SPACER_2I to state SPACER_2I */ 
        temp = GeneWise21_DC_SHADOW_MATRIX(mat,i-0,j-1,SPACER_2I) + mat->gp21->transition[GP21_SPACER2SPACER] +(mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GeneWise21_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,SPACER_2I,k);    
      ...
      /* Special state END has special to speical */ 
      /* Set score to current score (remember, state probably updated during main loop */ 
      score = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j,END); 


      /* Source AFTER_RND is a special source for END */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 1,AFTER_RND) + (0) + (0);    
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,END,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,AFTER_RND,k); 
        }  


      /* Put back score... (now updated!) */ 
      GeneWise21_DC_SHADOW_SPECIAL(mat,0,j,END) = score; 
      /* Finished updating state END */ 
      ...
      /* Special state BEFORE_RND has special to speical */ 
      /* Set score to current score (remember, state probably updated during main loop */ 
      score = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j,BEFORE_RND);  


      /* Source START is a special source for BEFORE_RND */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 1,START) + (mat->rndbase->base[CSEQ_GENOMIC_BASE(mat->target,j)]) + (0);     
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,BEFORE_RND,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,START,k);  
        }  


      /* Source BEFORE_RND is a special source for BEFORE_RND */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 1,BEFORE_RND) + ((mat->rndbase->base[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp21->transition[GP21_RND2RND])) + (0);  
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,BEFORE_RND,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,BEFORE_RND,k); 
        }  


      /* Source BEFORE_CODON is a special source for BEFORE_RND */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 1,BEFORE_CODON) + ((mat->rndbase->base[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp21->transition[GP21_CDS2RND])) + (0);    
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,BEFORE_RND,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,BEFORE_CODON,k);   
        }  


      /* Put back score... (now updated!) */ 
      GeneWise21_DC_SHADOW_SPECIAL(mat,0,j,BEFORE_RND) = score;  
      /* Finished updating state BEFORE_RND */ 
      ...
      /* Special state BEFORE_CODON has special to speical */ 
      /* Set score to current score (remember, state probably updated during main loop */ 
      score = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j,BEFORE_CODON);    


      /* Source BEFORE_RND is a special source for BEFORE_CODON */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 3,BEFORE_RND) + ((mat->rndcodon->codon[CSEQ_GENOMIC_CODON(mat->target,j)]+mat->gp21->transition[GP21_RND2CDS])) + (0);   
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,BEFORE_CODON,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 3,BEFORE_RND,k);   
        }  


      /* Source BEFORE_CODON is a special source for BEFORE_CODON */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 3,BEFORE_CODON) + ((mat->rndcodon->codon[CSEQ_GENOMIC_CODON(mat->target,j)]+mat->gp21->transition[GP21_CDS2CDS])) + (0);     
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,BEFORE_CODON,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 3,BEFORE_CODON,k); 
        }  


      /* Source BEFORE_SPACER is a special source for BEFORE_CODON */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 5,BEFORE_SPACER) + (((mat->gp21->transition[GP21_SPACER2CDS]+mat->rndcodon->codon[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))) + (0);   
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,BEFORE_CODON,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 5,BEFORE_SPACER,k);    
        }  


      /* Source BEFORE_SPACER is a special source for BEFORE_CODON */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 4,BEFORE_SPACER) + ((mat->gp21->transition[GP21_SPACER2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-2)))) + (0);     
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,BEFORE_CODON,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 4,BEFORE_SPACER,k);    
        }  


      /* Source BEFORE_SPACER is a special source for BEFORE_CODON */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 3,BEFORE_SPACER) + ((mat->gp21->transition[GP21_SPACER2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-1)))) + (0);     
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,BEFORE_CODON,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 3,BEFORE_SPACER,k);    
        }  


      /* Put back score... (now updated!) */ 
      GeneWise21_DC_SHADOW_SPECIAL(mat,0,j,BEFORE_CODON) = score;    
      /* Finished updating state BEFORE_CODON */ 
      ...
      /* Special state BEFORE_CENTRAL has special to speical */ 
      /* Set score to current score (remember, state probably updated during main loop */ 
      score = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j,BEFORE_CENTRAL);  


      /* Source BEFORE_CENTRAL is a special source for BEFORE_CENTRAL */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 1,BEFORE_CENTRAL) + ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp21->transition[GP21_CENTRAL2CENTRAL])) + (0);  
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,BEFORE_CENTRAL,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,BEFORE_CENTRAL,k); 
        }  


      /* Source BEFORE_CODON is a special source for BEFORE_CENTRAL */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 8,BEFORE_CODON) + ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))) + (0);    
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,BEFORE_CENTRAL,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 8,BEFORE_CODON,k);   
        }  


      /* Source BEFORE_CODON is a special source for BEFORE_CENTRAL */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 9,BEFORE_CODON) + ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))) + (0);    
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,BEFORE_CENTRAL,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 9,BEFORE_CODON,k);   
        }  


      /* Source BEFORE_CODON is a special source for BEFORE_CENTRAL */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 10,BEFORE_CODON) + ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))) + (0);   
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,BEFORE_CENTRAL,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 10,BEFORE_CODON,k);  
        }  


      /* Put back score... (now updated!) */ 
      GeneWise21_DC_SHADOW_SPECIAL(mat,0,j,BEFORE_CENTRAL) = score;  
      /* Finished updating state BEFORE_CENTRAL */ 
      ...
      /* Special state BEFORE_PY_TRACT has special to speical */ 
      /* Set score to current score (remember, state probably updated during main loop */ 
      score = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j,BEFORE_PY_TRACT); 


      /* Source BEFORE_CENTRAL is a special source for BEFORE_PY_TRACT */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 1,BEFORE_CENTRAL) + (mat->gp21->transition[GP21_CENTRAL2PY]) + (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);    
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,BEFORE_PY_TRACT,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,BEFORE_CENTRAL,k);    
        }  


      /* Source BEFORE_PY_TRACT is a special source for BEFORE_PY_TRACT */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 1,BEFORE_PY_TRACT) + (mat->gp21->transition[GP21_PY2PY]) + (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);    
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,BEFORE_PY_TRACT,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,BEFORE_PY_TRACT,k);   
        }  


      /* Put back score... (now updated!) */ 
      GeneWise21_DC_SHADOW_SPECIAL(mat,0,j,BEFORE_PY_TRACT) = score; 
      /* Finished updating state BEFORE_PY_TRACT */ 
      ...
      /* Special state BEFORE_SPACER has special to speical */ 
      /* Set score to current score (remember, state probably updated during main loop */ 
      score = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j,BEFORE_SPACER);   


      /* Source BEFORE_PY_TRACT is a special source for BEFORE_SPACER */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 1,BEFORE_PY_TRACT) + (mat->gp21->transition[GP21_PY2SPACER]) + (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);    
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,BEFORE_SPACER,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,BEFORE_PY_TRACT,k); 
        }  


      /* Source BEFORE_SPACER is a special source for BEFORE_SPACER */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 1,BEFORE_SPACER) + (mat->gp21->transition[GP21_SPACER2SPACER]) + (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);  
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,BEFORE_SPACER,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,BEFORE_SPACER,k);   
        }  


      /* Put back score... (now updated!) */ 
      GeneWise21_DC_SHADOW_SPECIAL(mat,0,j,BEFORE_SPACER) = score;   
      /* Finished updating state BEFORE_SPACER */ 
      ...
      /* Special state AFTER_RND has special to speical */ 
      /* Set score to current score (remember, state probably updated during main loop */ 
      score = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j,AFTER_RND);   


      /* Source AFTER_RND is a special source for AFTER_RND */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 1,AFTER_RND) + ((mat->rndbase->base[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp21->transition[GP21_RND2RND])) + (0);   
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_RND,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,AFTER_RND,k);   
        }  


      /* Source AFTER_CODON is a special source for AFTER_RND */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 1,AFTER_CODON) + ((mat->rndbase->base[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp21->transition[GP21_CDS2RND])) + (0);     
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_RND,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,AFTER_CODON,k); 
        }  


      /* Put back score... (now updated!) */ 
      GeneWise21_DC_SHADOW_SPECIAL(mat,0,j,AFTER_RND) = score;   
      /* Finished updating state AFTER_RND */ 
      ...
      /* Special state AFTER_CODON has special to speical */ 
      /* Set score to current score (remember, state probably updated during main loop */ 
      score = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j,AFTER_CODON); 


      /* Source AFTER_RND is a special source for AFTER_CODON */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 3,AFTER_RND) + ((mat->rndcodon->codon[CSEQ_GENOMIC_CODON(mat->target,j)]+mat->gp21->transition[GP21_RND2CDS])) + (0);    
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_CODON,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 3,AFTER_RND,k); 
        }  
      ...
      /* Source DELETE for state AFTER_CODON is not special... already calculated */ 
      /* Source AFTER_CODON is a special source for AFTER_CODON */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 3,AFTER_CODON) + ((mat->rndcodon->codon[CSEQ_GENOMIC_CODON(mat->target,j)]+mat->gp21->transition[GP21_CDS2CDS])) + (0);  
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_CODON,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 3,AFTER_CODON,k);   
        }  


      /* Source AFTER_SPACER is a special source for AFTER_CODON */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 5,AFTER_SPACER) + (((mat->gp21->transition[GP21_SPACER2CDS]+mat->rndcodon->codon[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))) + (0);    
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_CODON,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 5,AFTER_SPACER,k);  
        }  


      /* Source AFTER_SPACER is a special source for AFTER_CODON */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 4,AFTER_SPACER) + ((mat->gp21->transition[GP21_SPACER2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-2)))) + (0);  
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_CODON,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 4,AFTER_SPACER,k);  
        }  


      /* Source AFTER_SPACER is a special source for AFTER_CODON */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 3,AFTER_SPACER) + ((mat->gp21->transition[GP21_SPACER2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-1)))) + (0);  
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_CODON,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 3,AFTER_SPACER,k);  
        }  


      /* Put back score... (now updated!) */ 
      GeneWise21_DC_SHADOW_SPECIAL(mat,0,j,AFTER_CODON) = score; 
      /* Finished updating state AFTER_CODON */ 
      ...
      /* Special state AFTER_CENTRAL has special to speical */ 
      /* Set score to current score (remember, state probably updated during main loop */ 
      score = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j,AFTER_CENTRAL);   


      /* Source AFTER_CENTRAL is a special source for AFTER_CENTRAL */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 1,AFTER_CENTRAL) + ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp21->transition[GP21_CENTRAL2CENTRAL])) + (0);   
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_CENTRAL,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,AFTER_CENTRAL,k);   
        }  


      /* Source AFTER_CODON is a special source for AFTER_CENTRAL */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 8,AFTER_CODON) + ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))) + (0);     
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_CENTRAL,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 8,AFTER_CODON,k); 
        }  


      /* Source AFTER_CODON is a special source for AFTER_CENTRAL */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 9,AFTER_CODON) + ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))) + (0);     
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_CENTRAL,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 9,AFTER_CODON,k); 
        }  


      /* Source AFTER_CODON is a special source for AFTER_CENTRAL */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 10,AFTER_CODON) + ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))) + (0);    
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_CENTRAL,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 10,AFTER_CODON,k);    
        }  


      /* Put back score... (now updated!) */ 
      GeneWise21_DC_SHADOW_SPECIAL(mat,0,j,AFTER_CENTRAL) = score;   
      /* Finished updating state AFTER_CENTRAL */ 
      ...
      /* Special state AFTER_PY_TRACT has special to speical */ 
      /* Set score to current score (remember, state probably updated during main loop */ 
      score = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j,AFTER_PY_TRACT);  


      /* Source AFTER_CENTRAL is a special source for AFTER_PY_TRACT */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 1,AFTER_CENTRAL) + (mat->gp21->transition[GP21_CENTRAL2PY]) + (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);     
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_PY_TRACT,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,AFTER_CENTRAL,k);  
        }  


      /* Source AFTER_PY_TRACT is a special source for AFTER_PY_TRACT */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 1,AFTER_PY_TRACT) + (mat->gp21->transition[GP21_PY2PY]) + (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);     
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_PY_TRACT,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,AFTER_PY_TRACT,k); 
        }  


      /* Put back score... (now updated!) */ 
      GeneWise21_DC_SHADOW_SPECIAL(mat,0,j,AFTER_PY_TRACT) = score;  
      /* Finished updating state AFTER_PY_TRACT */ 
      ...
      /* Special state AFTER_SPACER has special to speical */ 
      /* Set score to current score (remember, state probably updated during main loop */ 
      score = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j,AFTER_SPACER);    


      /* Source AFTER_PY_TRACT is a special source for AFTER_SPACER */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 1,AFTER_PY_TRACT) + (mat->gp21->transition[GP21_PY2SPACER]) + (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);     
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_SPACER,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,AFTER_PY_TRACT,k);   
        }  


      /* Source AFTER_SPACER is a special source for AFTER_SPACER */ 
      temp = GeneWise21_DC_SHADOW_SPECIAL(mat,0,j - 1,AFTER_SPACER) + (mat->gp21->transition[GP21_SPACER2SPACER]) + (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);   
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i,j,AFTER_SPACER,k) = GeneWise21_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,AFTER_SPACER,k); 
        }  


      /* Put back score... (now updated!) */ 
      GeneWise21_DC_SHADOW_SPECIAL(mat,0,j,AFTER_SPACER) = score;    
      /* Finished updating state AFTER_SPACER */ 
...
PackAln * PackAln_read_Expl_GeneWise21(GeneWise21 * mat) 
{
    ...
    if( mat->basematrix->type != BASEMATRIX_TYPE_EXPLICIT)   {  
      warn("In GeneWise21_basic_read You have asked for an alignment from a non-explicit matrix: c'est impossible [current type is %d - %s]", mat->basematrix->type,basematrix_type_to_string(mat->basematrix->type));   
      return NULL;   
      }  
    ...
    out->score =  find_end_GeneWise21(mat,&i,&j,&state,&isspecial);  
    ...
    while( state != START || isspecial != TRUE)  { /*while state != START*/ 
      ...
      if( isspecial == TRUE )    
        max_calc_special_GeneWise21(mat,i,j,state,isspecial,&i,&j,&state,&isspecial,&cellscore);     
      else   
        max_calc_GeneWise21(mat,i,j,state,isspecial,&i,&j,&state,&isspecial,&cellscore);     
...
/* Function:  find_end_GeneWise21(mat,ri,rj,state,isspecial)
 *
 * Descrip: No Description
 *
 * Arg:              mat [UNKN ] Undocumented argument [GeneWise21 *]
 ...
 *
 */
int find_end_GeneWise21(GeneWise21 * mat,int * ri,int * rj,int * state,boolean * isspecial) 
{
    register int j;  
    register int max;    
    register int maxj;   
    ...
    max = GeneWise21_EXPL_SPECIAL(mat,0,mat->target->seq->len-1,END);    
    maxj = mat->target->seq->len-1;  
    for(j= mat->target->seq->len-2 ;j >= 0 ;j--) {  
      if( GeneWise21_EXPL_SPECIAL(mat,0,j,END) > max )   {  
        max = GeneWise21_EXPL_SPECIAL(mat,0,j,END);  
        maxj = j;    
        }  
      }  
...
/* Function:  max_calc_GeneWise21(mat,i,j,state,isspecial,reti,retj,retstate,retspecial,cellscore)
 *
 * Descrip: No Description
 *
 * Arg:               mat [UNKN ] Undocumented argument [GeneWise21 *]
 ...
 *
 */
int max_calc_GeneWise21(GeneWise21 * mat,int i,int j,int state,boolean isspecial,int * reti,int * retj,int * retstate,boolean * retspecial,int * cellscore) 
{
    register int temp;   
    register int cscore; 
    ...
    *reti = (*retj) = (*retstate) = GeneWise21_READ_OFF_ERROR;   
    ...
    /* Then you have to select the correct switch statement to figure out the readoff      */ 
    /* Somewhat odd - reverse the order of calculation and return as soon as it is correct */ 
    cscore = GeneWise21_EXPL_MATRIX(mat,i,j,state);  
    switch(state)    { /*Switch state */ 
      case MATCH :   
        temp = cscore - (mat->gp21->transition[GP21_INSERT_2_BASE]) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j)); 
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 5,MATCH) )  {  
          *reti = i - 1; 
          *retj = j - 5; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-5,MATCH); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 5,MATCH);  
          }  
        temp = cscore - (mat->gp21->transition[GP21_INSERT_1_BASE]) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j)); 
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 4,MATCH) )  {  
          *reti = i - 1; 
          *retj = j - 4; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-4,MATCH); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 4,MATCH);  
          }  
        temp = cscore - (mat->gp21->transition[GP21_DELETE_2_BASE]) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j)); 
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 1,MATCH) )  {  
          *reti = i - 1; 
          *retj = j - 1; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-1,MATCH); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 1,MATCH);  
          }  
        temp = cscore - (mat->gp21->transition[GP21_DELETE_1_BASE]) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j)); 
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 2,MATCH) )  {  
          *reti = i - 1; 
          *retj = j - 2; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-2,MATCH); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 2,MATCH);  
          }  
        temp = cscore - ((mat->gp21->transition[GP21_PY2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-1)))) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 4,PY_2M) )  {  
          *reti = i - 1; 
          *retj = j - 4; 
          *retstate = PY_2M; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-4,PY_2M); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 4,PY_2M);  
          }  
        temp = cscore - ((mat->gp21->transition[GP21_SPACER2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-1)))) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 4,SPACER_2M) )  {  
          *reti = i - 1; 
          *retj = j - 4; 
          *retstate = SPACER_2M; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-4,SPACER_2M); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 4,SPACER_2M);  
          }  
        temp = cscore - ((mat->gp21->transition[GP21_PY2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-2)))) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 5,PY_1M) )  {  
          *reti = i - 1; 
          *retj = j - 5; 
          *retstate = PY_1M; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-5,PY_1M); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 5,PY_1M);  
          }  
        temp = cscore - ((mat->gp21->transition[GP21_SPACER2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-2)))) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 5,SPACER_1M) )  {  
          *reti = i - 1; 
          *retj = j - 5; 
          *retstate = SPACER_1M; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-5,SPACER_1M); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 5,SPACER_1M);  
          }  
        temp = cscore - ((((mat->gp21->transition[GP21_PY2CDS]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 6,PY_0M) )  {  
          *reti = i - 1; 
          *retj = j - 6; 
          *retstate = PY_0M; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-6,PY_0M); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 6,PY_0M);  
          }  
        temp = cscore - ((((mat->gp21->transition[GP21_SPACER2CDS]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 6,SPACER_0M) )  {  
          *reti = i - 1; 
          *retj = j - 6; 
          *retstate = SPACER_0M; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-6,SPACER_0M); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 6,SPACER_0M);  
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_START2MATCH]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j)); 
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 1,j - 3,BEFORE_CODON) )  {  
          *reti = i - 1; 
          *retj = j - 3; 
          *retstate = BEFORE_CODON;  
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-1,j-3,BEFORE_CODON); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 3,BEFORE_CODON);   
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_DELETE2MATCH]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 3,DELETE) ) {  
          *reti = i - 1; 
          *retj = j - 3; 
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-3,DELETE);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 3,DELETE);     
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_INSERT2MATCH]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));    
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 3,INSERT) ) {  
          *reti = i - 1; 
          *retj = j - 3; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-3,INSERT);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 3,INSERT);     
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_MATCH2MATCH]+mat->query->seg[i]->match[CSEQ_GENOMIC_CODON(mat->target,j)])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j)); 
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 3,MATCH) )  {  
          *reti = i - 1; 
          *retj = j - 3; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-3,MATCH); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 3,MATCH);  
          }  
        warn("Major problem (!) - in GeneWise21 read off, position %d,%d state %d no source found!",i,j,state);  
        return (-1); 
      case INSERT :  
        temp = cscore - (mat->gp21->transition[GP21_INSERT_2_BASE]) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j)); 
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 5,INSERT) ) {  
          *reti = i - 1; 
          *retj = j - 5; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-5,INSERT);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 5,INSERT);     
          }  
        temp = cscore - (mat->gp21->transition[GP21_INSERT_1_BASE]) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j)); 
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 4,INSERT) ) {  
          *reti = i - 1; 
          *retj = j - 4; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-4,INSERT);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 4,INSERT);     
          }  
        temp = cscore - (mat->gp21->transition[GP21_DELETE_2_BASE]) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j)); 
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 1,INSERT) ) {  
          *reti = i - 1; 
          *retj = j - 1; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-1,INSERT);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 1,INSERT);     
          }  
        temp = cscore - (mat->gp21->transition[GP21_DELETE_1_BASE]) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j)); 
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 2,INSERT) ) {  
          *reti = i - 1; 
          *retj = j - 2; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-2,INSERT);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 2,INSERT);     
          }  
        temp = cscore - ((mat->gp21->transition[GP21_PY2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-1)))) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 4,PY_2I) )  {  
          *reti = i - 1; 
          *retj = j - 4; 
          *retstate = PY_2I; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-4,PY_2I); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 4,PY_2I);  
          }  
        temp = cscore - ((mat->gp21->transition[GP21_PY2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-2)))) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 5,PY_1I) )  {  
          *reti = i - 1; 
          *retj = j - 5; 
          *retstate = PY_1I; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-5,PY_1I); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 5,PY_1I);  
          }  
        temp = cscore - ((((mat->gp21->transition[GP21_PY2CDS]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));   
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 6,PY_0I) )  {  
          *reti = i - 1; 
          *retj = j - 6; 
          *retstate = PY_0I; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-6,PY_0I); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 6,PY_0I);  
          }  
        temp = cscore - ((mat->gp21->transition[GP21_SPACER2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-1)))) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 4,SPACER_2I) )  {  
          *reti = i - 1; 
          *retj = j - 4; 
          *retstate = SPACER_2I; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-4,SPACER_2I); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 4,SPACER_2I);  
          }  
        temp = cscore - ((mat->gp21->transition[GP21_SPACER2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-2)))) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 5,SPACER_1I) )  {  
          *reti = i - 1; 
          *retj = j - 5; 
          *retstate = SPACER_1I; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-5,SPACER_1I); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 5,SPACER_1I);  
          }  
        temp = cscore - ((((mat->gp21->transition[GP21_SPACER2CDS]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))+mat->query->seg[i]->transition[GW_MATCH_BALANCE_3SS])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));   
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 6,SPACER_0I) )  {  
          *reti = i - 1; 
          *retj = j - 6; 
          *retstate = SPACER_0I; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-6,SPACER_0I); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 6,SPACER_0I);  
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_START2INSERT]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));   
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 3,BEFORE_CODON) )  {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = BEFORE_CODON;  
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-3,BEFORE_CODON); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 3,BEFORE_CODON);   
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_DELETE2INSERT]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 3,DELETE) ) {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-3,DELETE);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 3,DELETE);     
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_INSERT2INSERT]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));  
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 3,INSERT) ) {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-3,INSERT);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 3,INSERT);     
          }  
        temp = cscore - ((mat->query->seg[i]->transition[GW_MATCH2INSERT]+mat->query->seg[i]->insert[CSEQ_GENOMIC_CODON(mat->target,j)])) -  (CSEQ_GENOMIC_CDSPOT(mat->target,j));   
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 3,MATCH) )  {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-3,MATCH); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 3,MATCH);  
          }  
        warn("Major problem (!) - in GeneWise21 read off, position %d,%d state %d no source found!",i,j,state);  
        return (-1); 
      case DELETE :  
        temp = cscore - (mat->query->seg[i]->transition[GW_START2DELETE]) -  (0);    
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 1,j - 0,BEFORE_CODON) )  {  
          *reti = i - 1; 
          *retj = j - 0; 
          *retstate = BEFORE_CODON;  
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-1,j-0,BEFORE_CODON); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 0,BEFORE_CODON);   
          }  
        temp = cscore - (mat->query->seg[i]->transition[GW_DELETE2DELETE]) -  (0);   
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 0,DELETE) ) {  
          *reti = i - 1; 
          *retj = j - 0; 
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-0,DELETE);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 0,DELETE);     
          }  
        temp = cscore - (mat->query->seg[i]->transition[GW_INSERT2DELETE]) -  (0);   
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 0,INSERT) ) {  
          *reti = i - 1; 
          *retj = j - 0; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-0,INSERT);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 0,INSERT);     
          }  
        temp = cscore - (mat->query->seg[i]->transition[GW_MATCH2DELETE]) -  (0);    
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 1,j - 0,MATCH) )  {  
          *reti = i - 1; 
          *retj = j - 0; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-1,j-0,MATCH); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 1,j - 0,MATCH);  
          }  
        warn("Major problem (!) - in GeneWise21 read off, position %d,%d state %d no source found!",i,j,state);  
        return (-1); 
      case CENTRAL_0M :  
        temp = cscore - ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp21->transition[GP21_CENTRAL2CENTRAL])) -  (0); 
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,CENTRAL_0M) ) {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = CENTRAL_0M;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-1,CENTRAL_0M);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,CENTRAL_0M);     
          }  
        temp = cscore - (((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_DELETE2MATCH])) -  (0);   
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 8,DELETE) ) {  
          *reti = i - 0; 
          *retj = j - 8; 
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-8,DELETE);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 8,DELETE);     
          }  
        temp = cscore - ((((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_INSERT2MATCH])+mat->query->seg[i]->transition[GW_INSERT_BALANCE_5SS])) -  (0);   
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 8,INSERT) ) {  
          *reti = i - 0; 
          *retj = j - 8; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-8,INSERT);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 8,INSERT);     
          }  
        temp = cscore - ((((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH2MATCH])+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])) -  (0); 
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 8,MATCH) )  {  
          *reti = i - 0; 
          *retj = j - 8; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-8,MATCH); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 8,MATCH);  
          }  
        warn("Major problem (!) - in GeneWise21 read off, position %d,%d state %d no source found!",i,j,state);  
        return (-1); 
      case PY_0M :   
        temp = cscore - (mat->gp21->transition[GP21_PY2PY]) -  (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,PY_0M) )  {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = PY_0M; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-1,PY_0M); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,PY_0M);  
          }  
        temp = cscore - (mat->gp21->transition[GP21_CENTRAL2PY]) -  (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);   
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,CENTRAL_0M) ) {  
          *reti = i - 0; 
      ...
      case SPACER_0M :   
        temp = cscore - (mat->gp21->transition[GP21_SPACER2SPACER]) -  (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,SPACER_0M) )  {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = SPACER_0M; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-1,SPACER_0M); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,SPACER_0M);  
          }  
        temp = cscore - (mat->gp21->transition[GP21_PY2SPACER]) -  (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,PY_0M) )  {  
          *reti = i - 0; 
      ...
      case CENTRAL_1M :  
        temp = cscore - ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp21->transition[GP21_CENTRAL2CENTRAL])) -  (0); 
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,CENTRAL_1M) ) {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = CENTRAL_1M;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-1,CENTRAL_1M);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,CENTRAL_1M);     
          }  
        temp = cscore - (((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_DELETE2MATCH])) -  (0);   
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 9,DELETE) ) {  
          *reti = i - 0; 
          *retj = j - 9; 
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-9,DELETE);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 9,DELETE);     
          }  
        temp = cscore - ((((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_INSERT2MATCH])+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])) -  (0);    
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 9,INSERT) ) {  
          *reti = i - 0; 
          *retj = j - 9; 
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-9,INSERT);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 9,INSERT);     
          }  
        temp = cscore - ((((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH2MATCH])+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])) -  (0); 
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 9,MATCH) )  {  
          *reti = i - 0; 
          *retj = j - 9; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-9,MATCH); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 9,MATCH);  
          }  
        warn("Major problem (!) - in GeneWise21 read off, position %d,%d state %d no source found!",i,j,state);  
        return (-1); 
      case PY_1M :   
        temp = cscore - (mat->gp21->transition[GP21_PY2PY]) -  (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,PY_1M) )  {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = PY_1M; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-1,PY_1M); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,PY_1M);  
          }  
        temp = cscore - (mat->gp21->transition[GP21_CENTRAL2PY]) -  (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);   
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,CENTRAL_1M) ) {  
          *reti = i - 0; 
      ...
      case SPACER_1M :   
        temp = cscore - (mat->gp21->transition[GP21_SPACER2SPACER]) -  (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,SPACER_1M) )  {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = SPACER_1M; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-1,SPACER_1M); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,SPACER_1M);  
          }  
        temp = cscore - (mat->gp21->transition[GP21_PY2SPACER]) -  (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,PY_1M) )  {  
          *reti = i - 0; 
      ...
      case CENTRAL_2M :  
        temp = cscore - ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp21->transition[GP21_CENTRAL2CENTRAL])) -  (0); 
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,CENTRAL_2M) ) {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = CENTRAL_2M;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-1,CENTRAL_2M);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,CENTRAL_2M);     
          }  
        temp = cscore - (((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_DELETE2MATCH])) -  (0);   
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 10,DELETE) )    {  
          *reti = i - 0; 
          *retj = j - 10;    
          *retstate = DELETE;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-10,DELETE);   
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 10,DELETE);    
          }  
        temp = cscore - ((((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_INSERT2MATCH])+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])) -  (0);    
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 10,INSERT) )    {  
          *reti = i - 0; 
          *retj = j - 10;    
          *retstate = INSERT;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-10,INSERT);   
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 10,INSERT);    
          }  
        temp = cscore - ((((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_MATCH2MATCH])+mat->query->seg[i]->transition[GW_MATCH_BALANCE_5SS])) -  (0); 
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 10,MATCH) ) {  
          *reti = i - 0; 
          *retj = j - 10;    
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-10,MATCH);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 10,MATCH);     
          }  
        warn("Major problem (!) - in GeneWise21 read off, position %d,%d state %d no source found!",i,j,state);  
        return (-1); 
      case PY_2M :   
        temp = cscore - (mat->gp21->transition[GP21_PY2PY]) -  (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,PY_2M) )  {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = PY_2M; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-1,PY_2M); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,PY_2M);  
          }  
        temp = cscore - (mat->gp21->transition[GP21_CENTRAL2PY]) -  (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);   
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,CENTRAL_2M) ) {  
          *reti = i - 0; 
      ...
      case SPACER_2M :   
        temp = cscore - (mat->gp21->transition[GP21_SPACER2SPACER]) -  (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,SPACER_2M) )  {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = SPACER_2M; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-1,SPACER_2M); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,SPACER_2M);  
          }  
        temp = cscore - (mat->gp21->transition[GP21_PY2SPACER]) -  (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,PY_2M) )  {  
          *reti = i - 0; 
      ...
      case CENTRAL_0I :  
        temp = cscore - ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp21->transition[GP21_CENTRAL2CENTRAL])) -  (0); 
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,CENTRAL_0I) ) {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = CENTRAL_0I;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-1,CENTRAL_0I);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,CENTRAL_0I);     
          }  
        temp = cscore - (((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_DELETE2INSERT])) -  (0);  
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 8,DELETE) ) {  
          *reti = i - 0; 
      ...
      case PY_0I :   
        temp = cscore - (mat->gp21->transition[GP21_PY2PY]) -  (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,PY_0I) )  {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = PY_0I; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-1,PY_0I); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,PY_0I);  
          }  
        temp = cscore - (mat->gp21->transition[GP21_CENTRAL2PY]) -  (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);   
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,CENTRAL_0I) ) {  
          *reti = i - 0; 
      ...
      case SPACER_0I :   
        temp = cscore - (mat->gp21->transition[GP21_SPACER2SPACER]) -  (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,SPACER_0I) )  {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = SPACER_0I; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-1,SPACER_0I); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,SPACER_0I);  
          }  
        temp = cscore - (mat->gp21->transition[GP21_PY2SPACER]) -  (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,PY_0I) )  {  
          *reti = i - 0; 
      ...
      case CENTRAL_1I :  
        temp = cscore - ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp21->transition[GP21_CENTRAL2CENTRAL])) -  (0); 
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,CENTRAL_1I) ) {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = CENTRAL_1I;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-1,CENTRAL_1I);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,CENTRAL_1I);     
          }  
        temp = cscore - (((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_DELETE2INSERT])) -  (0);  
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 9,DELETE) ) {  
          *reti = i - 0; 
      ...
      case PY_1I :   
        temp = cscore - (mat->gp21->transition[GP21_PY2PY]) -  (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,PY_1I) )  {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = PY_1I; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-1,PY_1I); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,PY_1I);  
          }  
        temp = cscore - (mat->gp21->transition[GP21_CENTRAL2PY]) -  (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);   
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,CENTRAL_1I) ) {  
          *reti = i - 0; 
      ...
      case SPACER_1I :   
        temp = cscore - (mat->gp21->transition[GP21_SPACER2SPACER]) -  (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,SPACER_1I) )  {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = SPACER_1I; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-1,SPACER_1I); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,SPACER_1I);  
          }  
        temp = cscore - (mat->gp21->transition[GP21_PY2SPACER]) -  (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,PY_1I) )  {  
          *reti = i - 0; 
      ...
      case CENTRAL_2I :  
        temp = cscore - ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp21->transition[GP21_CENTRAL2CENTRAL])) -  (0); 
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,CENTRAL_2I) ) {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = CENTRAL_2I;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-1,CENTRAL_2I);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,CENTRAL_2I);     
          }  
        temp = cscore - (((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))+mat->query->seg[i]->transition[GW_DELETE2INSERT])) -  (0);  
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 10,INSERT) )    {  
          *reti = i - 0; 
      ...
      case PY_2I :   
        temp = cscore - (mat->gp21->transition[GP21_PY2PY]) -  (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,PY_2I) )  {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = PY_2I; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-1,PY_2I); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,PY_2I);  
          }  
        temp = cscore - (mat->gp21->transition[GP21_CENTRAL2PY]) -  (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);   
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,CENTRAL_2I) ) {  
          *reti = i - 0; 
      ...
      case SPACER_2I :   
        temp = cscore - (mat->gp21->transition[GP21_SPACER2SPACER]) -  (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,SPACER_2I) )  {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = SPACER_2I; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-1,SPACER_2I); 
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,SPACER_2I);  
          }  
        temp = cscore - (mat->gp21->transition[GP21_PY2SPACER]) -  (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,PY_2I) )  {  
          *reti = i - 0; 
...
/* Function:  max_calc_special_GeneWise21(mat,i,j,state,isspecial,reti,retj,retstate,retspecial,cellscore)
 *
 * Descrip: No Description
 *
 * Arg:               mat [UNKN ] Undocumented argument [GeneWise21 *]
 ...
 *
 */
int max_calc_special_GeneWise21(GeneWise21 * mat,int i,int j,int state,boolean isspecial,int * reti,int * retj,int * retstate,boolean * retspecial,int * cellscore) 
{
    register int temp;   
    register int cscore; 
    ...
    *reti = (*retj) = (*retstate) = GeneWise21_READ_OFF_ERROR;   
    ...
    cscore = GeneWise21_EXPL_SPECIAL(mat,i,j,state); 
    switch(state)    { /*switch on special states*/ 
      case START :   
      case END :     
        /* source AFTER_RND is a special */ 
        temp = cscore - (0) - (0);   
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 1,AFTER_RND) ) {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = AFTER_RND; 
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-1,AFTER_RND);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,AFTER_RND) ;     
          }  
      case BEFORE_RND :  
        /* source BEFORE_CODON is a special */ 
        temp = cscore - ((mat->rndbase->base[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp21->transition[GP21_CDS2RND])) - (0);  
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 1,BEFORE_CODON) )  {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = BEFORE_CODON;  
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-1,BEFORE_CODON);     
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,BEFORE_CODON) ;  
          }  
        /* source BEFORE_RND is a special */ 
        temp = cscore - ((mat->rndbase->base[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp21->transition[GP21_RND2RND])) - (0);  
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 1,BEFORE_RND) )    {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = BEFORE_RND;    
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-1,BEFORE_RND);   
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,BEFORE_RND) ;    
          }  
        /* source START is a special */ 
        temp = cscore - (mat->rndbase->base[CSEQ_GENOMIC_BASE(mat->target,j)]) - (0);    
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 1,START) ) {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = START; 
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-1,START);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,START) ;     
          }  
      case BEFORE_CODON :    
        /* source BEFORE_SPACER is a special */ 
        temp = cscore - ((mat->gp21->transition[GP21_SPACER2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-1)))) - (0);    
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 3,BEFORE_SPACER) ) {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = BEFORE_SPACER; 
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-3,BEFORE_SPACER);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 3,BEFORE_SPACER) ;     
          }  
        /* source BEFORE_SPACER is a special */ 
        temp = cscore - ((mat->gp21->transition[GP21_SPACER2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-2)))) - (0);    
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 4,BEFORE_SPACER) ) {  
          *reti = i - 0; 
          *retj = j - 4; 
          *retstate = BEFORE_SPACER; 
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-4,BEFORE_SPACER);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 4,BEFORE_SPACER) ;     
          }  
        /* source BEFORE_SPACER is a special */ 
        temp = cscore - (((mat->gp21->transition[GP21_SPACER2CDS]+mat->rndcodon->codon[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))) - (0);  
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 5,BEFORE_SPACER) ) {  
          *reti = i - 0; 
          *retj = j - 5; 
          *retstate = BEFORE_SPACER; 
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-5,BEFORE_SPACER);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 5,BEFORE_SPACER) ;     
          }  
        /* source BEFORE_CODON is a special */ 
        temp = cscore - ((mat->rndcodon->codon[CSEQ_GENOMIC_CODON(mat->target,j)]+mat->gp21->transition[GP21_CDS2CDS])) - (0);   
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 3,BEFORE_CODON) )  {  
        ...
        /* source BEFORE_RND is a special */ 
        temp = cscore - ((mat->rndcodon->codon[CSEQ_GENOMIC_CODON(mat->target,j)]+mat->gp21->transition[GP21_RND2CDS])) - (0);   
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 3,BEFORE_RND) )    {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = BEFORE_RND;    
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-3,BEFORE_RND);   
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 3,BEFORE_RND) ;    
          }  
      case BEFORE_CENTRAL :  
        /* source BEFORE_CODON is a special */ 
        temp = cscore - ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))) - (0);  
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 10,BEFORE_CODON) ) {  
          *reti = i - 0; 
          *retj = j - 10;    
          *retstate = BEFORE_CODON;  
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-10,BEFORE_CODON);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 10,BEFORE_CODON) ;     
          }  
        /* source BEFORE_CODON is a special */ 
        temp = cscore - ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))) - (0);  
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 9,BEFORE_CODON) )  {  
          *reti = i - 0; 
          *retj = j - 9; 
          *retstate = BEFORE_CODON;  
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-9,BEFORE_CODON);     
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 9,BEFORE_CODON) ;  
          }  
        /* source BEFORE_CODON is a special */ 
        temp = cscore - ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))) - (0);  
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 8,BEFORE_CODON) )  {  
          *reti = i - 0; 
          *retj = j - 8; 
          *retstate = BEFORE_CODON;  
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-8,BEFORE_CODON);     
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 8,BEFORE_CODON) ;  
          }  
        /* source BEFORE_CENTRAL is a special */ 
        temp = cscore - ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp21->transition[GP21_CENTRAL2CENTRAL])) - (0);  
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 1,BEFORE_CENTRAL) )    {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = BEFORE_CENTRAL;    
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-1,BEFORE_CENTRAL);   
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,BEFORE_CENTRAL) ;    
          }  
      case BEFORE_PY_TRACT :     
        /* source BEFORE_PY_TRACT is a special */ 
        temp = cscore - (mat->gp21->transition[GP21_PY2PY]) - (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);     
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 1,BEFORE_PY_TRACT) )   {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = BEFORE_PY_TRACT;   
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-1,BEFORE_PY_TRACT);  
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,BEFORE_PY_TRACT) ;   
          }  
        /* source BEFORE_CENTRAL is a special */ 
        temp = cscore - (mat->gp21->transition[GP21_CENTRAL2PY]) - (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 1,BEFORE_CENTRAL) )    {  
      ...
      case BEFORE_SPACER :   
        /* source BEFORE_SPACER is a special */ 
        temp = cscore - (mat->gp21->transition[GP21_SPACER2SPACER]) - (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);     
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 1,BEFORE_SPACER) ) {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = BEFORE_SPACER; 
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-1,BEFORE_SPACER);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,BEFORE_SPACER) ;     
          }  
        /* source BEFORE_PY_TRACT is a special */ 
        temp = cscore - (mat->gp21->transition[GP21_PY2SPACER]) - (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);     
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 1,BEFORE_PY_TRACT) )   {  
      ...
      case AFTER_RND :   
        /* source AFTER_CODON is a special */ 
        temp = cscore - ((mat->rndbase->base[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp21->transition[GP21_CDS2RND])) - (0);  
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 1,AFTER_CODON) )   {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = AFTER_CODON;   
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-1,AFTER_CODON);  
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,AFTER_CODON) ;   
          }  
        /* source AFTER_RND is a special */ 
        temp = cscore - ((mat->rndbase->base[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp21->transition[GP21_RND2RND])) - (0);  
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 1,AFTER_RND) ) {  
      ...
      case AFTER_CODON :     
        /* source AFTER_SPACER is a special */ 
        temp = cscore - ((mat->gp21->transition[GP21_SPACER2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-1)))) - (0);    
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 3,AFTER_SPACER) )  {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = AFTER_SPACER;  
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-3,AFTER_SPACER);     
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 3,AFTER_SPACER) ;  
          }  
        /* source AFTER_SPACER is a special */ 
        temp = cscore - ((mat->gp21->transition[GP21_SPACER2CDS]+CSEQ_GENOMIC_3SS(mat->target,(j-2)))) - (0);    
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 4,AFTER_SPACER) )  {  
          *reti = i - 0; 
          *retj = j - 4; 
          *retstate = AFTER_SPACER;  
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-4,AFTER_SPACER);     
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 4,AFTER_SPACER) ;  
          }  
        /* source AFTER_SPACER is a special */ 
        temp = cscore - (((mat->gp21->transition[GP21_SPACER2CDS]+mat->rndcodon->codon[CSEQ_GENOMIC_CODON(mat->target,j)])+CSEQ_GENOMIC_3SS(mat->target,(j-3)))) - (0);  
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 5,AFTER_SPACER) )  {  
          *reti = i - 0; 
          *retj = j - 5; 
          *retstate = AFTER_SPACER;  
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-5,AFTER_SPACER);     
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 5,AFTER_SPACER) ;  
          }  
        /* source AFTER_CODON is a special */ 
        temp = cscore - ((mat->rndcodon->codon[CSEQ_GENOMIC_CODON(mat->target,j)]+mat->gp21->transition[GP21_CDS2CDS])) - (0);   
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 3,AFTER_CODON) )   {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = AFTER_CODON;   
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-3,AFTER_CODON);  
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 3,AFTER_CODON) ;   
          }  
        ...
        for(i= mat->query->len-1;i >= 0 ;i--)    { /*for i >= 0*/ 
          ...
          if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 0,DELETE) )   {  
            *reti = i - 0;   
            *retj = j - 0;   
            *retstate = DELETE;  
            *retspecial = FALSE; 
            if( cellscore != NULL)   {  
              *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-0,DELETE);  
              }  
            return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 0,DELETE) ;  
            }  
          } /* end of for i >= 0 */ 
        /* source INSERT is from main matrix */ 
        for(i= mat->query->len-1;i >= 0 ;i--)    { /*for i >= 0*/ 
          temp = cscore - (mat->query->seg[i]->transition[GW_INSERT2END]) - (0);     
          if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 0,INSERT) )   {  
            *reti = i - 0;   
            *retj = j - 0;   
            *retstate = INSERT;  
            *retspecial = FALSE; 
            if( cellscore != NULL)   {  
              *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-0,INSERT);  
              }  
            return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 0,INSERT) ;  
            }  
          } /* end of for i >= 0 */ 
        /* source MATCH is from main matrix */ 
        for(i= mat->query->len-1;i >= 0 ;i--)    { /*for i >= 0*/ 
          temp = cscore - (mat->query->seg[i]->transition[GW_MATCH2END]) - (0);  
          if( temp == GeneWise21_EXPL_MATRIX(mat,i - 0,j - 0,MATCH) )    {  
            *reti = i - 0;   
            *retj = j - 0;   
            *retstate = MATCH;   
            *retspecial = FALSE; 
            if( cellscore != NULL)   {  
              *cellscore = cscore - GeneWise21_EXPL_MATRIX(mat,i-0,j-0,MATCH);   
              }  
            return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 0,MATCH) ;   
            }  
          } /* end of for i >= 0 */ 
        /* source AFTER_RND is a special */ 
        temp = cscore - ((mat->rndcodon->codon[CSEQ_GENOMIC_CODON(mat->target,j)]+mat->gp21->transition[GP21_RND2CDS])) - (0);   
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 3,AFTER_RND) ) {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = AFTER_RND; 
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-3,AFTER_RND);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 3,AFTER_RND) ;     
          }  
      case AFTER_CENTRAL :   
        /* source AFTER_CODON is a special */ 
        temp = cscore - ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))) - (0);  
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 10,AFTER_CODON) )  {  
          *reti = i - 0; 
          *retj = j - 10;    
          *retstate = AFTER_CODON;   
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-10,AFTER_CODON);     
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 10,AFTER_CODON) ;  
          }  
        /* source AFTER_CODON is a special */ 
        temp = cscore - ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))) - (0);  
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 9,AFTER_CODON) )   {  
          *reti = i - 0; 
          *retj = j - 9; 
          *retstate = AFTER_CODON;   
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-9,AFTER_CODON);  
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 9,AFTER_CODON) ;   
          }  
        /* source AFTER_CODON is a special */ 
        temp = cscore - ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+CSEQ_GENOMIC_5SS(mat->target,(j-7)))) - (0);  
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 8,AFTER_CODON) )   {  
          *reti = i - 0; 
          *retj = j - 8; 
          *retstate = AFTER_CODON;   
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-8,AFTER_CODON);  
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 8,AFTER_CODON) ;   
          }  
        /* source AFTER_CENTRAL is a special */ 
        temp = cscore - ((mat->gp21->central[CSEQ_GENOMIC_BASE(mat->target,j)]+mat->gp21->transition[GP21_CENTRAL2CENTRAL])) - (0);  
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 1,AFTER_CENTRAL) ) {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = AFTER_CENTRAL; 
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-1,AFTER_CENTRAL);    
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,AFTER_CENTRAL) ;     
          }  
      case AFTER_PY_TRACT :  
        /* source AFTER_PY_TRACT is a special */ 
        temp = cscore - (mat->gp21->transition[GP21_PY2PY]) - (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);     
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 1,AFTER_PY_TRACT) )    {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = AFTER_PY_TRACT;    
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-1,AFTER_PY_TRACT);   
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,AFTER_PY_TRACT) ;    
          }  
        /* source AFTER_CENTRAL is a special */ 
        temp = cscore - (mat->gp21->transition[GP21_CENTRAL2PY]) - (mat->gp21->py[CSEQ_GENOMIC_BASE(mat->target,j)]);    
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 1,AFTER_CENTRAL) ) {  
      ...
      case AFTER_SPACER :    
        /* source AFTER_SPACER is a special */ 
        temp = cscore - (mat->gp21->transition[GP21_SPACER2SPACER]) - (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);     
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 1,AFTER_SPACER) )  {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = AFTER_SPACER;  
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GeneWise21_EXPL_SPECIAL(mat,i-0,j-1,AFTER_SPACER);     
            }  
          return GeneWise21_EXPL_MATRIX(mat,i - 0,j - 1,AFTER_SPACER) ;  
          }  
        /* source AFTER_PY_TRACT is a special */ 
        temp = cscore - (mat->gp21->transition[GP21_PY2SPACER]) - (mat->gp21->spacer[CSEQ_GENOMIC_BASE(mat->target,j)]);     
        if( temp == GeneWise21_EXPL_SPECIAL(mat,i - 0,j - 1,AFTER_PY_TRACT) )    {  
