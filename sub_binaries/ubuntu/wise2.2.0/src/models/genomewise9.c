...
Search_Return_Type search_GenomeWise9(DBSearchImpl * dbsi,Hscore * out,GenomeEvidenceSet* evi,GenomicDB* targetdb ,int switchcost,int newgenecost,int non_start_codon,int non_stop_codon,RandomCodonScore * rndcodon) 
{
    ...
    if( dbsi->trace_level > 5 )  
      warn("Asking for trace level of %d in database search for GenomeWise9, but it was compiled with a trace level of 416. Not all trace statements can be shown",dbsi->trace_level);   
...
PackAln * PackAln_bestmemory_GenomeWise9(GenomeEvidenceSet* evi,ComplexSequence* gen ,int switchcost,int newgenecost,int non_start_codon,int non_stop_codon,RandomCodonScore * rndcodon,DPEnvelope * dpenv,DPRunImpl * dpri) 
{
    ...
    if( strategy == DPIM_Linear )    {  
      ...
      if( (mat=allocate_Expl_GenomeWise9(evi, gen , switchcost, newgenecost, non_start_codon, non_stop_codon, rndcodon)) == NULL )   {  
...
/* Function:  allocate_Expl_GenomeWise9(evi,gen,switchcost,newgenecost,non_start_codon,non_stop_codon,rndcodon)
 *
 ...
 *
 */
GenomeWise9 * allocate_Expl_GenomeWise9(GenomeEvidenceSet* evi,ComplexSequence* gen ,int switchcost,int newgenecost,int non_start_codon,int non_stop_codon,RandomCodonScore * rndcodon) 
{
    GenomeWise9 * out;   
...
PackAln * PackAln_calculate_Small_GenomeWise9(GenomeWise9 * mat,DPEnvelope * dpenv) 
{
    ...
    dc_start_end_calculate_GenomeWise9(mat,dpenv);   
...
AlnRangeSet * AlnRangeSet_calculate_Small_GenomeWise9(GenomeWise9 * mat) 
{
    ...
    start_reporting("Find start end points: ");  
    dc_start_end_calculate_GenomeWise9(mat,NULL);    
...
/* Function:  dc_start_end_calculate_GenomeWise9(mat,dpenv)
 *
 ...
 *
 */
boolean dc_start_end_calculate_GenomeWise9(GenomeWise9 * mat,DPEnvelope * dpenv) 
{
    int i;   
    int j;   
    int k;   
    ...
    init_start_end_linear_GenomeWise9(mat);  
    ...
    for(j=0;j<lenj;j++)  { /*for each j strip*/ 
      for(i=0;i<leni;i++)    { /*for each i position in strip*/ 
        ...
        /* For state UTR5 */ 
        /* setting first movement to score */ 
        score = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-1,UTR5) + GNE_UTR(mat->evi,i,mat->gen,j) + (0);   
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,UTR5,k);  
        /* From state UTR5_INTRON to state UTR5 */ 
        temp = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-1,UTR5_INTRON) + GNE_UTR_3SS(mat->evi,i,mat->gen,j) +(0);  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,UTR5_INTRON,k); 
        ...
        /* From state SPECIAL_UTR5 to state UTR5 */ 
        temp = GenomeWise9_DC_SHADOW_SPECIAL(mat,i-0,j-1,SPECIAL_UTR5) + GNE_UTR(mat->evi,i,mat->gen,j) + (0);   
        if( temp  > score )  {  
          score = temp;  
        ...
        /* From state START to state UTR5 */ 
        temp = GenomeWise9_DC_SHADOW_SPECIAL(mat,i-0,j-1,START) + GNE_UTR5_START(mat->evi,i,mat->gen,j) + (0);   
        if( temp  > score )  {  
          score = temp;  
        ...
        /* From state INTERGENIC to state UTR5 */ 
        temp = GenomeWise9_DC_SHADOW_SPECIAL(mat,i-0,j-1,INTERGENIC) + (GNE_UTR(mat->evi,i,mat->gen,j)+GNE_UTR5_START(mat->evi,i,mat->gen,j)) + (0);     
        if( temp  > score )  {  
          score = temp;  
        ...
        /* From state PREGENE_INTERGENIC to state UTR5 */ 
        temp = GenomeWise9_DC_SHADOW_SPECIAL(mat,i-0,j-1,PREGENE_INTERGENIC) + GNE_UTR(mat->evi,i,mat->gen,j) + (0);     
        if( temp  > score )  {  
          score = temp;  
        ...
        /* state UTR5 is a source for special SPECIAL_UTR5 */ 
        temp = score + (mat->switchcost) + (0) ;     
        if( temp > GenomeWise9_DC_SHADOW_SPECIAL(mat,i,j,SPECIAL_UTR5) )     {  
          GenomeWise9_DC_SHADOW_SPECIAL(mat,i,j,SPECIAL_UTR5) = temp;    
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,SPECIAL_UTR5,k) = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i,j,UTR5,k);  
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,SPECIAL_UTR5,6) = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i,j,UTR5,6);    
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,SPECIAL_UTR5,3) = i;  
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,SPECIAL_UTR5,4) = j;  
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,SPECIAL_UTR5,5) = UTR5;   
          }  
        ...
        /* For state UTR5_INTRON */ 
        /* setting first movement to score */ 
        score = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-1,UTR5_INTRON) + GNE_UTR_INTRON(mat->evi,i,mat->gen,j) + (0);     
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,UTR5_INTRON,k);   
        /* From state UTR5 to state UTR5_INTRON */ 
        temp = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-1,UTR5) + GNE_UTR_5SS(mat->evi,i,mat->gen,j) +(0);     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,UTR5,k);    
        ...
        /* For state START_CODON */ 
        /* setting first movement to score */ 
        score = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-3,UTR5_INTRON) + GNE_START_CODON(mat->evi,i,mat->gen,j) + (0);    
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 3,UTR5_INTRON,k);   
        /* From state UTR5 to state START_CODON */ 
        temp = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-3,UTR5) + GNE_START_CODON(mat->evi,i,mat->gen,j) +(0);     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 3,UTR5,k);    
        ...
        /* For state CDS */ 
        /* setting first movement to score */ 
        score = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-3,CDS) + GNE_CDS(mat->evi,i,mat->gen,j) + (0);    
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 3,CDS,k);   
        /* From state CDS_INTRON_0 to state CDS */ 
        temp = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-6,CDS_INTRON_0) + GNE_CDS_3SS(mat->evi,i,mat->gen,(j-3),0) +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 6,CDS_INTRON_0,k);    
        ...
        /* From state CDS_INTRON_1 to state CDS */ 
        temp = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-5,CDS_INTRON_1) + GNE_CDS_3SS(mat->evi,i,mat->gen,(j-2),1) +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 5,CDS_INTRON_1,k);    
        ...
        /* From state CDS_INTRON_2 to state CDS */ 
        temp = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-4,CDS_INTRON_2) + GNE_CDS_3SS(mat->evi,i,mat->gen,(j-1),2) +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 4,CDS_INTRON_2,k);    
        ...
        /* From state CDS to state CDS */ 
        temp = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-2,CDS) + GNE_CDS_FRAMESHIFT(mat->evi,i,mat->gen,j,2) +(0);     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 2,CDS,k); 
        ...
        /* From state CDS to state CDS */ 
        temp = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-4,CDS) + GNE_CDS_FRAMESHIFT(mat->evi,i,mat->gen,j,4) +(0);     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 4,CDS,k); 
        ...
        /* From state UTR5 to state CDS */ 
        temp = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-3,UTR5) + (GNE_CDS(mat->evi,i,mat->gen,j)+mat->non_start_codon) +(0);  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 3,UTR5,k);    
        ...
        /* From state START_CODON to state CDS */ 
        temp = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-3,START_CODON) + GNE_CDS(mat->evi,i,mat->gen,j) +(0);  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 3,START_CODON,k); 
        ...
        /* From state SPECIAL_CDS to state CDS */ 
        temp = GenomeWise9_DC_SHADOW_SPECIAL(mat,i-0,j-3,SPECIAL_CDS) + GNE_CDS(mat->evi,i,mat->gen,j) + (0);    
        if( temp  > score )  {  
          score = temp;  
        ...
        /* From state INTERGENIC to state CDS */ 
        temp = GenomeWise9_DC_SHADOW_SPECIAL(mat,i-0,j-3,INTERGENIC) + (GNE_CDS(mat->evi,i,mat->gen,j)+GNE_UTR5_START(mat->evi,i,mat->gen,j)) + (0);     
        if( temp  > score )  {  
          score = temp;  
        ...
        /* state CDS is a source for special INTERGENIC */ 
        temp = score + ((mat->newgenecost+GNE_UTR3_END(mat->evi,i,mat->gen,j))) + (0) ;  
        if( temp > GenomeWise9_DC_SHADOW_SPECIAL(mat,i,j,INTERGENIC) )   {  
          GenomeWise9_DC_SHADOW_SPECIAL(mat,i,j,INTERGENIC) = temp;  
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,INTERGENIC,k) = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i,j,CDS,k); 
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,INTERGENIC,6) = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i,j,CDS,6);   
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,INTERGENIC,3) = i;    
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,INTERGENIC,4) = j;    
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,INTERGENIC,5) = CDS;  
          }  
        ...
        /* state CDS is a source for special SPECIAL_CDS */ 
        temp = score + ((mat->switchcost+mat->rndcodon->codon[CSEQ_GENOMIC_CODON(mat->gen,j)])) + (0) ;  
        if( temp > GenomeWise9_DC_SHADOW_SPECIAL(mat,i,j,SPECIAL_CDS) )  {  
          GenomeWise9_DC_SHADOW_SPECIAL(mat,i,j,SPECIAL_CDS) = temp;     
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,SPECIAL_CDS,k) = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i,j,CDS,k);    
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,SPECIAL_CDS,6) = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i,j,CDS,6);  
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,SPECIAL_CDS,3) = i;   
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,SPECIAL_CDS,4) = j;   
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,SPECIAL_CDS,5) = CDS; 
          }  
        ...
        /* For state CDS_INTRON_0 */ 
        /* setting first movement to score */ 
        score = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-1,CDS_INTRON_0) + GNE_CDS_INTRON(mat->evi,i,mat->gen,j) + (0);    
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,CDS_INTRON_0,k);  
        /* From state CDS to state CDS_INTRON_0 */ 
        temp = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-8,CDS) + GNE_CDS_5SS(mat->evi,i,mat->gen,(j-7),0) +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 8,CDS,k); 
        ...
        /* For state CDS_INTRON_1 */ 
        /* setting first movement to score */ 
        score = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-1,CDS_INTRON_1) + GNE_CDS_INTRON(mat->evi,i,mat->gen,j) + (0);    
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,CDS_INTRON_1,k);  
        /* From state CDS to state CDS_INTRON_1 */ 
        temp = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-9,CDS) + GNE_CDS_5SS(mat->evi,i,mat->gen,(j-7),1) +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 9,CDS,k); 
        ...
        /* For state CDS_INTRON_2 */ 
        /* setting first movement to score */ 
        score = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-1,CDS_INTRON_2) + GNE_CDS_INTRON(mat->evi,i,mat->gen,j) + (0);    
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,CDS_INTRON_2,k);  
        /* From state CDS to state CDS_INTRON_2 */ 
        temp = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-10,CDS) + GNE_CDS_5SS(mat->evi,i,mat->gen,(j-7),2) +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 10,CDS,k);    
        ...
        /* For state STOP_CODON */ 
        /* setting first movement to score */ 
        score = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-3,CDS) + GNE_STOP_CODON(mat->evi,i,mat->gen,j) + (0);     
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 3,CDS,k);   
        ...
        /* For state UTR3 */ 
        /* setting first movement to score */ 
        score = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-1,UTR3) + GNE_UTR(mat->evi,i,mat->gen,j) + (0);   
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,UTR3,k);  
        /* From state CDS to state UTR3 */ 
        temp = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-1,CDS) + (GNE_UTR(mat->evi,i,mat->gen,j)+mat->non_stop_codon) +(0);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,CDS,k); 
        ...
        /* From state STOP_CODON to state UTR3 */ 
        temp = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-1,STOP_CODON) + GNE_UTR(mat->evi,i,mat->gen,j) +(0);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,STOP_CODON,k);  
        ...
        /* From state UTR3_INTRON to state UTR3 */ 
        temp = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-1,UTR3_INTRON) + GNE_UTR_3SS(mat->evi,i,mat->gen,j) +(0);  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,UTR3_INTRON,k); 
        ...
        /* From state INTERGENIC to state UTR3 */ 
        temp = GenomeWise9_DC_SHADOW_SPECIAL(mat,i-0,j-1,INTERGENIC) + GNE_UTR(mat->evi,i,mat->gen,j) + (0);     
        if( temp  > score )  {  
          score = temp;  
        ...
        /* state UTR3 is a source for special POSTGENE_INTERGENIC */ 
        temp = score + ((mat->newgenecost+GNE_UTR3_END(mat->evi,i,mat->gen,j))) + (0) ;  
        if( temp > GenomeWise9_DC_SHADOW_SPECIAL(mat,i,j,POSTGENE_INTERGENIC) )  {  
          GenomeWise9_DC_SHADOW_SPECIAL(mat,i,j,POSTGENE_INTERGENIC) = temp;     
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,POSTGENE_INTERGENIC,k) = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i,j,UTR3,k);   
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,POSTGENE_INTERGENIC,6) = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i,j,UTR3,6); 
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,POSTGENE_INTERGENIC,3) = i;   
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,POSTGENE_INTERGENIC,4) = j;   
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,POSTGENE_INTERGENIC,5) = UTR3;    
          }  
        ...
        if( temp > GenomeWise9_DC_SHADOW_SPECIAL(mat,i,j,INTERGENIC) )   {  
          ...
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,INTERGENIC,k) = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i,j,UTR3,k);    
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,INTERGENIC,6) = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i,j,UTR3,6);  
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,INTERGENIC,3) = i;    
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,INTERGENIC,4) = j;    
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,INTERGENIC,5) = UTR3; 
          }  
        ...
        /* state UTR3 is a source for special SPECIAL_UTR3 */ 
        temp = score + (mat->switchcost) + (0) ;     
        if( temp > GenomeWise9_DC_SHADOW_SPECIAL(mat,i,j,SPECIAL_UTR3) )     {  
          GenomeWise9_DC_SHADOW_SPECIAL(mat,i,j,SPECIAL_UTR3) = temp;    
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,SPECIAL_UTR3,k) = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i,j,UTR3,k);  
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,SPECIAL_UTR3,6) = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i,j,UTR3,6);    
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,SPECIAL_UTR3,3) = i;  
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,SPECIAL_UTR3,4) = j;  
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,SPECIAL_UTR3,5) = UTR3;   
          }  
        ...
        /* For state UTR3_INTRON */ 
        /* setting first movement to score */ 
        score = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-1,UTR3_INTRON) + GNE_UTR_INTRON(mat->evi,i,mat->gen,j) + (0);     
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,UTR3_INTRON,k);   
        /* From state UTR3 to state UTR3_INTRON */ 
        temp = GenomeWise9_DC_SHADOW_MATRIX(mat,i-0,j-1,UTR3) + GNE_UTR_5SS(mat->evi,i,mat->gen,j) +(0);     
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = GenomeWise9_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,UTR3,k);    
        ...
        /* From state SPECIAL_UTR3 to state UTR3_INTRON */ 
        temp = GenomeWise9_DC_SHADOW_SPECIAL(mat,i-0,j-1,SPECIAL_UTR3) + GNE_UTR(mat->evi,i,mat->gen,j) + (0);   
        if( temp  > score )  {  
          score = temp;  
      ...
      /* Special state PREGENE_INTERGENIC has special to speical */ 
      /* Set score to current score (remember, state probably updated during main loop */ 
      score = GenomeWise9_DC_SHADOW_SPECIAL(mat,0,j,PREGENE_INTERGENIC); 


      /* Source START is a special source for PREGENE_INTERGENIC */ 
      temp = GenomeWise9_DC_SHADOW_SPECIAL(mat,0,j - 1,START) + (0) + (0);   
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,PREGENE_INTERGENIC,k) = GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,START,k);    
        }  


      /* Put back score... (now updated!) */ 
      GenomeWise9_DC_SHADOW_SPECIAL(mat,0,j,PREGENE_INTERGENIC) = score; 
      /* Finished updating state PREGENE_INTERGENIC */ 
      ...
      /* Special state INTERGENIC has special to speical */ 
      /* Set score to current score (remember, state probably updated during main loop */ 
      score = GenomeWise9_DC_SHADOW_SPECIAL(mat,0,j,INTERGENIC); 


      /* Source INTERGENIC is a special source for INTERGENIC */ 
      temp = GenomeWise9_DC_SHADOW_SPECIAL(mat,0,j - 1,INTERGENIC) + (0) + (0);  
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,INTERGENIC,k) = GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,INTERGENIC,k);   
        }  
      ...
      /* Source UTR3 for state INTERGENIC is not special... already calculated */ 
      /* Put back score... (now updated!) */ 
      GenomeWise9_DC_SHADOW_SPECIAL(mat,0,j,INTERGENIC) = score; 
      /* Finished updating state INTERGENIC */ 
      ...
      /* Special state SPECIAL_CDS has special to speical */ 
      /* Set score to current score (remember, state probably updated during main loop */ 
      score = GenomeWise9_DC_SHADOW_SPECIAL(mat,0,j,SPECIAL_CDS);    


      /* Source CDS for state SPECIAL_CDS is not special... already calculated */ 
      /* Source SPECIAL_CDS is a special source for SPECIAL_CDS */ 
      temp = GenomeWise9_DC_SHADOW_SPECIAL(mat,0,j - 3,SPECIAL_CDS) + (mat->rndcodon->codon[CSEQ_GENOMIC_CODON(mat->gen,j)]) + (0);  
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,SPECIAL_CDS,k) = GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 3,SPECIAL_CDS,k); 
        }  


      /* Put back score... (now updated!) */ 
      GenomeWise9_DC_SHADOW_SPECIAL(mat,0,j,SPECIAL_CDS) = score;    
      /* Finished updating state SPECIAL_CDS */ 
      ...
      /* Special state END has special to speical */ 
      /* Set score to current score (remember, state probably updated during main loop */ 
      score = GenomeWise9_DC_SHADOW_SPECIAL(mat,0,j,END);    
      ...
      if( temp > score ) {  
        ...
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i,j,END,k) = GenomeWise9_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,INTERGENIC,k);  
        }  


      /* Put back score... (now updated!) */ 
      GenomeWise9_DC_SHADOW_SPECIAL(mat,0,j,END) = score;    
      /* Finished updating state END */ 
...
PackAln * PackAln_read_Expl_GenomeWise9(GenomeWise9 * mat) 
{
    ...
    if( mat->basematrix->type != BASEMATRIX_TYPE_EXPLICIT)   {  
      warn("In GenomeWise9_basic_read You have asked for an alignment from a non-explicit matrix: c'est impossible [current type is %d - %s]", mat->basematrix->type,basematrix_type_to_string(mat->basematrix->type));  
      return NULL;   
      }  
    ...
    out->score =  find_end_GenomeWise9(mat,&i,&j,&state,&isspecial); 
    ...
    while( state != START || isspecial != TRUE)  { /*while state != START*/ 
      ...
      if( isspecial == TRUE )    
        max_calc_special_GenomeWise9(mat,i,j,state,isspecial,&i,&j,&state,&isspecial,&cellscore);    
      else   
        max_calc_GenomeWise9(mat,i,j,state,isspecial,&i,&j,&state,&isspecial,&cellscore);    
...
/* Function:  find_end_GenomeWise9(mat,ri,rj,state,isspecial)
 *
 * Descrip: No Description
 *
 * Arg:              mat [UNKN ] Undocumented argument [GenomeWise9 *]
 ...
 *
 */
int find_end_GenomeWise9(GenomeWise9 * mat,int * ri,int * rj,int * state,boolean * isspecial) 
{
    register int j;  
    register int max;    
    register int maxj;   
    ...
    max = GenomeWise9_EXPL_SPECIAL(mat,0,mat->gen->seq->len-1,END);  
    maxj = mat->gen->seq->len-1;     
    for(j= mat->gen->seq->len-2 ;j >= 0 ;j--)    {  
      if( GenomeWise9_EXPL_SPECIAL(mat,0,j,END) > max )  {  
        max = GenomeWise9_EXPL_SPECIAL(mat,0,j,END); 
        maxj = j;    
        }  
      }  
...
/* Function:  max_calc_GenomeWise9(mat,i,j,state,isspecial,reti,retj,retstate,retspecial,cellscore)
 *
 * Descrip: No Description
 *
 * Arg:               mat [UNKN ] Undocumented argument [GenomeWise9 *]
 ...
 *
 */
int max_calc_GenomeWise9(GenomeWise9 * mat,int i,int j,int state,boolean isspecial,int * reti,int * retj,int * retstate,boolean * retspecial,int * cellscore) 
{
    register int temp;   
    register int cscore; 
    ...
    *reti = (*retj) = (*retstate) = GenomeWise9_READ_OFF_ERROR;  
    ...
    /* Then you have to select the correct switch statement to figure out the readoff      */ 
    /* Somewhat odd - reverse the order of calculation and return as soon as it is correct */ 
    cscore = GenomeWise9_EXPL_MATRIX(mat,i,j,state); 
    switch(state)    { /*Switch state */ 
      case UTR5 :    
        temp = cscore - (GNE_UTR(mat->evi,i,mat->gen,j)) -  (0); 
        if( temp == GenomeWise9_EXPL_SPECIAL(mat,i - 0,j - 1,PREGENE_INTERGENIC) )   {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = PREGENE_INTERGENIC;    
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_SPECIAL(mat,i-0,j-1,PREGENE_INTERGENIC);  
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 1,PREGENE_INTERGENIC);    
          }  
        temp = cscore - ((GNE_UTR(mat->evi,i,mat->gen,j)+GNE_UTR5_START(mat->evi,i,mat->gen,j))) -  (0); 
        if( temp == GenomeWise9_EXPL_SPECIAL(mat,i - 0,j - 1,INTERGENIC) )   {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = INTERGENIC;    
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_SPECIAL(mat,i-0,j-1,INTERGENIC);  
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 1,INTERGENIC);    
          }  
        temp = cscore - (GNE_UTR5_START(mat->evi,i,mat->gen,j)) -  (0);  
        if( temp == GenomeWise9_EXPL_SPECIAL(mat,i - 0,j - 1,START) )    {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = START; 
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_SPECIAL(mat,i-0,j-1,START);   
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 1,START);     
          }  
        temp = cscore - (GNE_UTR(mat->evi,i,mat->gen,j)) -  (0); 
        if( temp == GenomeWise9_EXPL_SPECIAL(mat,i - 0,j - 1,SPECIAL_UTR5) ) {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = SPECIAL_UTR5;  
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_SPECIAL(mat,i-0,j-1,SPECIAL_UTR5);    
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 1,SPECIAL_UTR5);  
          }  
        temp = cscore - (GNE_UTR_3SS(mat->evi,i,mat->gen,j)) -  (0); 
        if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 1,UTR5_INTRON) )   {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = UTR5_INTRON;   
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_MATRIX(mat,i-0,j-1,UTR5_INTRON);  
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 1,UTR5_INTRON);   
          }  
        temp = cscore - (GNE_UTR(mat->evi,i,mat->gen,j)) -  (0); 
        if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 1,UTR5) )  {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = UTR5;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_MATRIX(mat,i-0,j-1,UTR5); 
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 1,UTR5);  
          }  
        warn("Major problem (!) - in GenomeWise9 read off, position %d,%d state %d no source found!",i,j,state); 
        return (-1); 
      case UTR5_INTRON :     
      ...
      case START_CODON :     
        temp = cscore - (GNE_START_CODON(mat->evi,i,mat->gen,j)) -  (0); 
        if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 3,UTR5) )  {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = UTR5;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_MATRIX(mat,i-0,j-3,UTR5); 
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 3,UTR5);  
          }  
        temp = cscore - (GNE_START_CODON(mat->evi,i,mat->gen,j)) -  (0); 
        if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 3,UTR5_INTRON) )   {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = UTR5_INTRON;   
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_MATRIX(mat,i-0,j-3,UTR5_INTRON);  
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 3,UTR5_INTRON);   
          }  
        warn("Major problem (!) - in GenomeWise9 read off, position %d,%d state %d no source found!",i,j,state); 
        return (-1); 
      case CDS :     
        temp = cscore - ((GNE_CDS(mat->evi,i,mat->gen,j)+GNE_UTR5_START(mat->evi,i,mat->gen,j))) -  (0); 
        if( temp == GenomeWise9_EXPL_SPECIAL(mat,i - 0,j - 3,INTERGENIC) )   {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = INTERGENIC;    
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_SPECIAL(mat,i-0,j-3,INTERGENIC);  
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 3,INTERGENIC);    
          }  
        temp = cscore - (GNE_CDS(mat->evi,i,mat->gen,j)) -  (0); 
        if( temp == GenomeWise9_EXPL_SPECIAL(mat,i - 0,j - 3,SPECIAL_CDS) )  {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = SPECIAL_CDS;   
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_SPECIAL(mat,i-0,j-3,SPECIAL_CDS); 
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 3,SPECIAL_CDS);   
          }  
        temp = cscore - (GNE_CDS(mat->evi,i,mat->gen,j)) -  (0); 
        if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 3,START_CODON) )   {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = START_CODON;   
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_MATRIX(mat,i-0,j-3,START_CODON);  
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 3,START_CODON);   
          }  
        temp = cscore - ((GNE_CDS(mat->evi,i,mat->gen,j)+mat->non_start_codon)) -  (0);  
        if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 3,UTR5) )  {  
          *reti = i - 0; 
        ...
        temp = cscore - (GNE_CDS_FRAMESHIFT(mat->evi,i,mat->gen,j,4)) -  (0);    
        if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 4,CDS) )   {  
          *reti = i - 0; 
          *retj = j - 4; 
          *retstate = CDS;   
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_MATRIX(mat,i-0,j-4,CDS);  
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 4,CDS);   
          }  
        temp = cscore - (GNE_CDS_FRAMESHIFT(mat->evi,i,mat->gen,j,2)) -  (0);    
        if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 2,CDS) )   {  
          *reti = i - 0; 
          *retj = j - 2; 
          *retstate = CDS;   
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_MATRIX(mat,i-0,j-2,CDS);  
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 2,CDS);   
          }  
        temp = cscore - (GNE_CDS_3SS(mat->evi,i,mat->gen,(j-1),2)) -  (0);   
        if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 4,CDS_INTRON_2) )  {  
          *reti = i - 0; 
          *retj = j - 4; 
          *retstate = CDS_INTRON_2;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_MATRIX(mat,i-0,j-4,CDS_INTRON_2); 
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 4,CDS_INTRON_2);  
          }  
        temp = cscore - (GNE_CDS_3SS(mat->evi,i,mat->gen,(j-2),1)) -  (0);   
        if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 5,CDS_INTRON_1) )  {  
          *reti = i - 0; 
          *retj = j - 5; 
          *retstate = CDS_INTRON_1;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_MATRIX(mat,i-0,j-5,CDS_INTRON_1); 
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 5,CDS_INTRON_1);  
          }  
        temp = cscore - (GNE_CDS_3SS(mat->evi,i,mat->gen,(j-3),0)) -  (0);   
        if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 6,CDS_INTRON_0) )  {  
          *reti = i - 0; 
          *retj = j - 6; 
          *retstate = CDS_INTRON_0;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_MATRIX(mat,i-0,j-6,CDS_INTRON_0); 
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 6,CDS_INTRON_0);  
          }  
        temp = cscore - (GNE_CDS(mat->evi,i,mat->gen,j)) -  (0); 
        if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 3,CDS) )   {  
          *reti = i - 0; 
          *retj = j - 3; 
          *retstate = CDS;   
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_MATRIX(mat,i-0,j-3,CDS);  
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 3,CDS);   
          }  
        warn("Major problem (!) - in GenomeWise9 read off, position %d,%d state %d no source found!",i,j,state); 
        return (-1); 
      case CDS_INTRON_0 :    
        temp = cscore - (GNE_CDS_5SS(mat->evi,i,mat->gen,(j-7),0)) -  (0);   
        if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 8,CDS) )   {  
          *reti = i - 0; 
          *retj = j - 8; 
          *retstate = CDS;   
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_MATRIX(mat,i-0,j-8,CDS);  
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 8,CDS);   
          }  
        temp = cscore - (GNE_CDS_INTRON(mat->evi,i,mat->gen,j)) -  (0);  
        if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 1,CDS_INTRON_0) )  {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = CDS_INTRON_0;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_MATRIX(mat,i-0,j-1,CDS_INTRON_0); 
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 1,CDS_INTRON_0);  
          }  
        warn("Major problem (!) - in GenomeWise9 read off, position %d,%d state %d no source found!",i,j,state); 
        return (-1); 
      case CDS_INTRON_1 :    
        temp = cscore - (GNE_CDS_5SS(mat->evi,i,mat->gen,(j-7),1)) -  (0);   
        if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 9,CDS) )   {  
          *reti = i - 0; 
          *retj = j - 9; 
          *retstate = CDS;   
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_MATRIX(mat,i-0,j-9,CDS);  
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 9,CDS);   
          }  
        temp = cscore - (GNE_CDS_INTRON(mat->evi,i,mat->gen,j)) -  (0);  
        if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 1,CDS_INTRON_1) )  {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = CDS_INTRON_1;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_MATRIX(mat,i-0,j-1,CDS_INTRON_1); 
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 1,CDS_INTRON_1);  
          }  
        warn("Major problem (!) - in GenomeWise9 read off, position %d,%d state %d no source found!",i,j,state); 
        return (-1); 
      case CDS_INTRON_2 :    
        temp = cscore - (GNE_CDS_5SS(mat->evi,i,mat->gen,(j-7),2)) -  (0);   
        if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 10,CDS) )  {  
          *reti = i - 0; 
          *retj = j - 10;    
          *retstate = CDS;   
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_MATRIX(mat,i-0,j-10,CDS); 
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 10,CDS);  
          }  
        temp = cscore - (GNE_CDS_INTRON(mat->evi,i,mat->gen,j)) -  (0);  
        if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 1,CDS_INTRON_2) )  {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = CDS_INTRON_2;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_MATRIX(mat,i-0,j-1,CDS_INTRON_2); 
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 1,CDS_INTRON_2);  
          }  
        warn("Major problem (!) - in GenomeWise9 read off, position %d,%d state %d no source found!",i,j,state); 
        return (-1); 
      case STOP_CODON :  
      ...
      case UTR3 :    
        ...
        temp = cscore - (GNE_UTR_3SS(mat->evi,i,mat->gen,j)) -  (0); 
        if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 1,UTR3_INTRON) )   {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = UTR3_INTRON;   
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_MATRIX(mat,i-0,j-1,UTR3_INTRON);  
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 1,UTR3_INTRON);   
          }  
        temp = cscore - (GNE_UTR(mat->evi,i,mat->gen,j)) -  (0); 
        if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 1,STOP_CODON) )    {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = STOP_CODON;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_MATRIX(mat,i-0,j-1,STOP_CODON);   
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 1,STOP_CODON);    
          }  
        temp = cscore - ((GNE_UTR(mat->evi,i,mat->gen,j)+mat->non_stop_codon)) -  (0);   
        if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 1,CDS) )   {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = CDS;   
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_MATRIX(mat,i-0,j-1,CDS);  
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 1,CDS);   
          }  
        temp = cscore - (GNE_UTR(mat->evi,i,mat->gen,j)) -  (0); 
        if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 1,UTR3) )  {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = UTR3;  
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_MATRIX(mat,i-0,j-1,UTR3); 
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 1,UTR3);  
          }  
        warn("Major problem (!) - in GenomeWise9 read off, position %d,%d state %d no source found!",i,j,state); 
        return (-1); 
      case UTR3_INTRON :     
        temp = cscore - (GNE_UTR(mat->evi,i,mat->gen,j)) -  (0); 
        if( temp == GenomeWise9_EXPL_SPECIAL(mat,i - 0,j - 1,SPECIAL_UTR3) ) {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = SPECIAL_UTR3;  
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - GenomeWise9_EXPL_SPECIAL(mat,i-0,j-1,SPECIAL_UTR3);    
            }  
          return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 1,SPECIAL_UTR3);  
          }  
        temp = cscore - (GNE_UTR_5SS(mat->evi,i,mat->gen,j)) -  (0); 
        if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 1,UTR3) )  {  
          *reti = i - 0; 
...
/* Function:  max_calc_special_GenomeWise9(mat,i,j,state,isspecial,reti,retj,retstate,retspecial,cellscore)
 *
 * Descrip: No Description
 *
 * Arg:               mat [UNKN ] Undocumented argument [GenomeWise9 *]
 ...
 *
 */
int max_calc_special_GenomeWise9(GenomeWise9 * mat,int i,int j,int state,boolean isspecial,int * reti,int * retj,int * retstate,boolean * retspecial,int * cellscore) 
{
    register int temp;   
    register int cscore; 
    ...
    *reti = (*retj) = (*retstate) = GenomeWise9_READ_OFF_ERROR;  
    ...
    cscore = GenomeWise9_EXPL_SPECIAL(mat,i,j,state);    
    switch(state)    { /*switch on special states*/ 
      case PREGENE_INTERGENIC :  
        /* source START is a special */ 
        temp = cscore - (0) - (0);   
      ...
      case POSTGENE_INTERGENIC :     
        ...
        for(i= mat->evi->len-1;i >= 0 ;i--)  { /*for i >= 0*/ 
          ...
          if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 0,UTR3) )    {  
            *reti = i - 0;   
            *retj = j - 0;   
            *retstate = UTR3;    
            *retspecial = FALSE; 
            if( cellscore != NULL)   {  
              *cellscore = cscore - GenomeWise9_EXPL_MATRIX(mat,i-0,j-0,UTR3);   
              }  
            return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 0,UTR3) ;   
            }  
          } /* end of for i >= 0 */ 
      case INTERGENIC :  
        /* source UTR3 is from main matrix */ 
        ...
        for(i= mat->evi->len-1;i >= 0 ;i--)  { /*for i >= 0*/ 
          temp = cscore - ((mat->newgenecost+GNE_UTR3_END(mat->evi,i,mat->gen,j))) - (0);    
          if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 0,CDS) ) {  
            *reti = i - 0;   
            *retj = j - 0;   
            *retstate = CDS; 
            *retspecial = FALSE; 
            if( cellscore != NULL)   {  
              *cellscore = cscore - GenomeWise9_EXPL_MATRIX(mat,i-0,j-0,CDS);    
              }  
            return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 0,CDS) ;    
            }  
          } /* end of for i >= 0 */ 
        /* source INTERGENIC is a special */ 
        temp = cscore - (0) - (0);   
      ...
      case SPECIAL_UTR5 :    
        ...
        for(i= mat->evi->len-1;i >= 0 ;i--)  { /*for i >= 0*/ 
          ...
          if( temp == GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 0,UTR5) )    {  
            *reti = i - 0;   
            *retj = j - 0;   
            *retstate = UTR5;    
            *retspecial = FALSE; 
            if( cellscore != NULL)   {  
              *cellscore = cscore - GenomeWise9_EXPL_MATRIX(mat,i-0,j-0,UTR5);   
              }  
            return GenomeWise9_EXPL_MATRIX(mat,i - 0,j - 0,UTR5) ;   
            }  
          } /* end of for i >= 0 */ 
      case SPECIAL_UTR3 :    
        /* source UTR3 is from main matrix */ 
