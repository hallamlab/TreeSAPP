...
Search_Return_Type search_ProteinSW(DBSearchImpl * dbsi,Hscore * out,ProteinDB* querydb,ProteinDB* targetdb ,CompMat* comp,int gap,int ext) 
{
    ...
    if( dbsi->trace_level > 5 )  
      warn("Asking for trace level of %d in database search for ProteinSW, but it was compiled with a trace level of 416. Not all trace statements can be shown",dbsi->trace_level); 
