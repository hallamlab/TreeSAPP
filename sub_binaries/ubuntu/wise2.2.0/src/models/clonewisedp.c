...
Search_Return_Type search_CloneWise(DBSearchImpl * dbsi,Hscore * out,MappedCloneSet * q,MappedCloneSet * t ,MappedCloneMatch* match,Score target_skip_start,Score target_skip,Score query_skip_start,Score query_skip,int spread,int target_special_s) 
{
    ...
    if( dbsi->trace_level > 5 )  
      warn("Asking for trace level of %d in database search for CloneWise, but it was compiled with a trace level of 416. Not all trace statements can be shown",dbsi->trace_level); 
...
PackAln * PackAln_bestmemory_CloneWise(MappedCloneSet * q,MappedCloneSet * t ,MappedCloneMatch* match,Score target_skip_start,Score target_skip,Score query_skip_start,Score query_skip,int spread,int target_special_s,DPEnvelope * dpenv,DPRunImpl * dpri) 
{
    ...
    if( strategy == DPIM_Linear )    {  
      ...
      if( (mat=allocate_Expl_CloneWise(q, t , match, target_skip_start, target_skip, query_skip_start, query_skip, spread, target_special_s)) == NULL )  {  
...
/* Function:  allocate_Expl_CloneWise(q,t,match,target_skip_start,target_skip,query_skip_start,query_skip,spread,target_special_s)
 *
 ...
 *
 */
CloneWise * allocate_Expl_CloneWise(MappedCloneSet * q,MappedCloneSet * t ,MappedCloneMatch* match,Score target_skip_start,Score target_skip,Score query_skip_start,Score query_skip,int spread,int target_special_s) 
{
    CloneWise * out; 
...
PackAln * PackAln_calculate_Small_CloneWise(CloneWise * mat,DPEnvelope * dpenv) 
{
    ...
    dc_start_end_calculate_CloneWise(mat,dpenv); 
...
AlnRangeSet * AlnRangeSet_calculate_Small_CloneWise(CloneWise * mat) 
{
    ...
    start_reporting("Find start end points: ");  
    dc_start_end_calculate_CloneWise(mat,NULL);  
...
/* Function:  dc_start_end_calculate_CloneWise(mat,dpenv)
 *
 ...
 *
 */
boolean dc_start_end_calculate_CloneWise(CloneWise * mat,DPEnvelope * dpenv) 
{
    int i;   
    int j;   
    int k;   
    ...
    init_start_end_linear_CloneWise(mat);    
    ...
    for(j=0;j<lenj;j++)  { /*for each j strip*/ 
      for(i=0;i<leni;i++)    { /*for each i position in strip*/ 
        ...
        /* For state MATCH */ 
        /* setting first movement to score */ 
        score = CloneWise_DC_SHADOW_MATRIX(mat,i-1,j-1,MATCH) + (mat->match->matrix[i][j]+1) + (mat->match->matrix[i][j]);   
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = CloneWise_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 1,MATCH,k);   
        /* From state MATCH to state MATCH */ 
        temp = CloneWise_DC_SHADOW_MATRIX(mat,i-0,j-1,MATCH) + 0 +(mat->match->matrix[i][j]);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CloneWise_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,MATCH,k); 
        ...
        /* From state MATCH to state MATCH */ 
        temp = CloneWise_DC_SHADOW_MATRIX(mat,i-1,j-0,MATCH) + 0 +(mat->match->matrix[i][j]);    
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CloneWise_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 0,MATCH,k); 
        ...
        /* From state SKIP_QUERY to state MATCH */ 
        temp = CloneWise_DC_SHADOW_MATRIX(mat,i-1,j-1,SKIP_QUERY) + 0 +(mat->match->matrix[i][j]);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CloneWise_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 1,SKIP_QUERY,k);    
        ...
        /* From state SKIP_TARGET to state MATCH */ 
        temp = CloneWise_DC_SHADOW_MATRIX(mat,i-1,j-1,SKIP_TARGET) + 0 +(mat->match->matrix[i][j]);  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CloneWise_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 1,SKIP_TARGET,k);   
        ...
        /* From state START to state MATCH */ 
        temp = CloneWise_DC_SHADOW_SPECIAL(mat,i-1,j-1,START) + 0 + (mat->match->matrix[i][j]);  
        if( temp  > score )  {  
          score = temp;  
        ...
        /* state MATCH is a source for special TRUSTED_SPECIAL */ 
        temp = score + (mat->target_special_s) + (0) ;   
        if( temp > CloneWise_DC_SHADOW_SPECIAL(mat,i,j,TRUSTED_SPECIAL) )    {  
          CloneWise_DC_SHADOW_SPECIAL(mat,i,j,TRUSTED_SPECIAL) = temp;   
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            CloneWise_DC_SHADOW_SPECIAL_SP(mat,i,j,TRUSTED_SPECIAL,k) = CloneWise_DC_SHADOW_MATRIX_SP(mat,i,j,MATCH,k);  
          CloneWise_DC_SHADOW_SPECIAL_SP(mat,i,j,TRUSTED_SPECIAL,6) = CloneWise_DC_SHADOW_MATRIX_SP(mat,i,j,MATCH,6);    
          CloneWise_DC_SHADOW_SPECIAL_SP(mat,i,j,TRUSTED_SPECIAL,3) = i; 
          CloneWise_DC_SHADOW_SPECIAL_SP(mat,i,j,TRUSTED_SPECIAL,4) = j; 
          CloneWise_DC_SHADOW_SPECIAL_SP(mat,i,j,TRUSTED_SPECIAL,5) = MATCH; 
          }  
        ...
        /* For state SKIP_QUERY */ 
        /* setting first movement to score */ 
        score = CloneWise_DC_SHADOW_MATRIX(mat,i-1,j-0,MATCH) + mat->query_skip_start + (mat->match->skip_iset[i]);  
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = CloneWise_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 0,MATCH,k);   
        /* From state SKIP_TARGET to state SKIP_QUERY */ 
        temp = CloneWise_DC_SHADOW_MATRIX(mat,i-1,j-0,SKIP_TARGET) + mat->query_skip_start +(mat->match->skip_iset[i]);  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CloneWise_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 0,SKIP_TARGET,k);   
        ...
        /* From state SKIP_QUERY to state SKIP_QUERY */ 
        temp = CloneWise_DC_SHADOW_MATRIX(mat,i-1,j-0,SKIP_QUERY) + 0 +(mat->match->skip_iset[i]);   
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CloneWise_DC_SHADOW_MATRIX_SP(mat,i - 1,j - 0,SKIP_QUERY,k);    
        ...
        /* From state START to state SKIP_QUERY */ 
        temp = CloneWise_DC_SHADOW_SPECIAL(mat,i-1,j-0,START) + mat->query_skip_start + (mat->match->skip_iset[i]);  
        if( temp  > score )  {  
          score = temp;  
        ...
        if( temp > CloneWise_DC_SHADOW_SPECIAL(mat,i,j,TRUSTED_SPECIAL) )    {  
          ...
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            CloneWise_DC_SHADOW_SPECIAL_SP(mat,i,j,TRUSTED_SPECIAL,k) = CloneWise_DC_SHADOW_MATRIX_SP(mat,i,j,SKIP_QUERY,k); 
          CloneWise_DC_SHADOW_SPECIAL_SP(mat,i,j,TRUSTED_SPECIAL,6) = CloneWise_DC_SHADOW_MATRIX_SP(mat,i,j,SKIP_QUERY,6);   
          CloneWise_DC_SHADOW_SPECIAL_SP(mat,i,j,TRUSTED_SPECIAL,3) = i; 
          CloneWise_DC_SHADOW_SPECIAL_SP(mat,i,j,TRUSTED_SPECIAL,4) = j; 
          CloneWise_DC_SHADOW_SPECIAL_SP(mat,i,j,TRUSTED_SPECIAL,5) = SKIP_QUERY;    
          }  
        ...
        /* For state SKIP_TARGET */ 
        /* setting first movement to score */ 
        score = CloneWise_DC_SHADOW_MATRIX(mat,i-0,j-1,MATCH) + mat->target_skip_start + (mat->match->skip_jset[j]);     
        /* shift first shadow numbers */ 
        for(k=0;k<7;k++) 
          localshadow[k] = CloneWise_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,MATCH,k);   
        /* From state SKIP_QUERY to state SKIP_TARGET */ 
        temp = CloneWise_DC_SHADOW_MATRIX(mat,i-0,j-1,SKIP_QUERY) + mat->target_skip_start +(mat->match->skip_jset[j]);  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CloneWise_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,SKIP_QUERY,k);    
        ...
        /* From state SKIP_TARGET to state SKIP_TARGET */ 
        temp = CloneWise_DC_SHADOW_MATRIX(mat,i-0,j-1,SKIP_TARGET) + 0 +(mat->match->skip_jset[j]);  
        if( temp  > score )  {  
          score = temp;  
          for(k=0;k<7;k++)   
            localshadow[k] = CloneWise_DC_SHADOW_MATRIX_SP(mat,i - 0,j - 1,SKIP_TARGET,k);   
        ...
        /* From state START to state SKIP_TARGET */ 
        temp = CloneWise_DC_SHADOW_SPECIAL(mat,i-0,j-1,START) + mat->target_skip_start + (mat->match->skip_jset[j]);     
        if( temp  > score )  {  
          score = temp;  
        ...
        if( temp > CloneWise_DC_SHADOW_SPECIAL(mat,i,j,TRUSTED_SPECIAL) )    {  
          ...
          /* Have to push only bottem half of system here */ 
          for(k=0;k<3;k++)   
            CloneWise_DC_SHADOW_SPECIAL_SP(mat,i,j,TRUSTED_SPECIAL,k) = CloneWise_DC_SHADOW_MATRIX_SP(mat,i,j,SKIP_TARGET,k);    
          CloneWise_DC_SHADOW_SPECIAL_SP(mat,i,j,TRUSTED_SPECIAL,6) = CloneWise_DC_SHADOW_MATRIX_SP(mat,i,j,SKIP_TARGET,6);  
          CloneWise_DC_SHADOW_SPECIAL_SP(mat,i,j,TRUSTED_SPECIAL,3) = i; 
          CloneWise_DC_SHADOW_SPECIAL_SP(mat,i,j,TRUSTED_SPECIAL,4) = j; 
          CloneWise_DC_SHADOW_SPECIAL_SP(mat,i,j,TRUSTED_SPECIAL,5) = SKIP_TARGET;   
          }  
      ...
      /* Special state TRUSTED_SPECIAL has special to speical */ 
      /* Set score to current score (remember, state probably updated during main loop */ 
      score = CloneWise_DC_SHADOW_SPECIAL(mat,0,j,TRUSTED_SPECIAL);  


      /* Source START is a special source for TRUSTED_SPECIAL */ 
      temp = CloneWise_DC_SHADOW_SPECIAL(mat,0,j - 1,START) + (0) + (0);     
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          CloneWise_DC_SHADOW_SPECIAL_SP(mat,i,j,TRUSTED_SPECIAL,k) = CloneWise_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,START,k);   
        }  


      /* Source TRUSTED_SPECIAL is a special source for TRUSTED_SPECIAL */ 
      temp = CloneWise_DC_SHADOW_SPECIAL(mat,0,j - 1,TRUSTED_SPECIAL) + ((mat->match->skip_jset[j]-1)) + (0);    
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          CloneWise_DC_SHADOW_SPECIAL_SP(mat,i,j,TRUSTED_SPECIAL,k) = CloneWise_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,TRUSTED_SPECIAL,k); 
        }  
      ...
      /* Source SKIP_TARGET for state TRUSTED_SPECIAL is not special... already calculated */ 
      /* Put back score... (now updated!) */ 
      CloneWise_DC_SHADOW_SPECIAL(mat,0,j,TRUSTED_SPECIAL) = score;  
      /* Finished updating state TRUSTED_SPECIAL */ 
      ...
      /* Special state END has special to speical */ 
      /* Set score to current score (remember, state probably updated during main loop */ 
      score = CloneWise_DC_SHADOW_SPECIAL(mat,0,j,END);  


      /* Source TRUSTED_SPECIAL is a special source for END */ 
      temp = CloneWise_DC_SHADOW_SPECIAL(mat,0,j - 1,TRUSTED_SPECIAL) + (0) + (0);   
      if( temp > score ) {  
        score = temp;    
        /* Also got to propagate shadows  */ 
        for(k=0;k<7;k++) 
          CloneWise_DC_SHADOW_SPECIAL_SP(mat,i,j,END,k) = CloneWise_DC_SHADOW_SPECIAL_SP(mat,i - 0,j - 1,TRUSTED_SPECIAL,k); 
        }  


      /* Put back score... (now updated!) */ 
      CloneWise_DC_SHADOW_SPECIAL(mat,0,j,END) = score;  
      /* Finished updating state END */ 
...
PackAln * PackAln_read_Expl_CloneWise(CloneWise * mat) 
{
    ...
    if( mat->basematrix->type != BASEMATRIX_TYPE_EXPLICIT)   {  
      warn("In CloneWise_basic_read You have asked for an alignment from a non-explicit matrix: c'est impossible [current type is %d - %s]", mat->basematrix->type,basematrix_type_to_string(mat->basematrix->type));    
      return NULL;   
      }  
    ...
    out->score =  find_end_CloneWise(mat,&i,&j,&state,&isspecial);   
    ...
    while( state != START || isspecial != TRUE)  { /*while state != START*/ 
      ...
      if( isspecial == TRUE )    
        max_calc_special_CloneWise(mat,i,j,state,isspecial,&i,&j,&state,&isspecial,&cellscore);  
      else   
        max_calc_CloneWise(mat,i,j,state,isspecial,&i,&j,&state,&isspecial,&cellscore);  
...
/* Function:  find_end_CloneWise(mat,ri,rj,state,isspecial)
 *
 * Descrip: No Description
 *
 * Arg:              mat [UNKN ] Undocumented argument [CloneWise *]
 ...
 *
 */
int find_end_CloneWise(CloneWise * mat,int * ri,int * rj,int * state,boolean * isspecial) 
{
    register int j;  
    register int max;    
    register int maxj;   
    ...
    max = CloneWise_EXPL_SPECIAL(mat,0,mat->t->length-1,END);    
    maxj = mat->t->length-1;     
    for(j= mat->t->length-2 ;j >= 0 ;j--)    {  
      if( CloneWise_EXPL_SPECIAL(mat,0,j,END) > max )    {  
        max = CloneWise_EXPL_SPECIAL(mat,0,j,END);   
        maxj = j;    
        }  
      }  
...
/* Function:  max_calc_CloneWise(mat,i,j,state,isspecial,reti,retj,retstate,retspecial,cellscore)
 *
 * Descrip: No Description
 *
 * Arg:               mat [UNKN ] Undocumented argument [CloneWise *]
 ...
 *
 */
int max_calc_CloneWise(CloneWise * mat,int i,int j,int state,boolean isspecial,int * reti,int * retj,int * retstate,boolean * retspecial,int * cellscore) 
{
    register int temp;   
    register int cscore; 
    ...
    *reti = (*retj) = (*retstate) = CloneWise_READ_OFF_ERROR;    
    ...
    /* Then you have to select the correct switch statement to figure out the readoff      */ 
    /* Somewhat odd - reverse the order of calculation and return as soon as it is correct */ 
    cscore = CloneWise_EXPL_MATRIX(mat,i,j,state);   
    switch(state)    { /*Switch state */ 
      case MATCH :   
        /* Has restricted position */ 
        if( (i-1) == 0 && (j-1) == 0 )   {  
          temp = cscore - (0) -  (mat->match->matrix[i][j]); 
          if( temp == CloneWise_EXPL_SPECIAL(mat,i - 1,j - 1,START) )    {  
            *reti = i - 1;   
            *retj = j - 1;   
            *retstate = START;   
            *retspecial = TRUE;  
            if( cellscore != NULL)   {  
              *cellscore = cscore - CloneWise_EXPL_SPECIAL(mat,i-1,j-1,START);   
              }  
            return CloneWise_EXPL_MATRIX(mat,i - 1,j - 1,START);     
            }  
          }  
        temp = cscore - (0) -  (mat->match->matrix[i][j]);   
        if( temp == CloneWise_EXPL_MATRIX(mat,i - 1,j - 1,SKIP_TARGET) ) {  
          *reti = i - 1; 
          *retj = j - 1; 
          *retstate = SKIP_TARGET;   
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CloneWise_EXPL_MATRIX(mat,i-1,j-1,SKIP_TARGET);    
            }  
          return CloneWise_EXPL_MATRIX(mat,i - 1,j - 1,SKIP_TARGET);     
          }  
        temp = cscore - (0) -  (mat->match->matrix[i][j]);   
        if( temp == CloneWise_EXPL_MATRIX(mat,i - 1,j - 1,SKIP_QUERY) )  {  
          *reti = i - 1; 
          *retj = j - 1; 
          *retstate = SKIP_QUERY;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CloneWise_EXPL_MATRIX(mat,i-1,j-1,SKIP_QUERY); 
            }  
          return CloneWise_EXPL_MATRIX(mat,i - 1,j - 1,SKIP_QUERY);  
          }  
        temp = cscore - (0) -  (mat->match->matrix[i][j]);   
        if( temp == CloneWise_EXPL_MATRIX(mat,i - 1,j - 0,MATCH) )   {  
          *reti = i - 1; 
          *retj = j - 0; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CloneWise_EXPL_MATRIX(mat,i-1,j-0,MATCH);  
            }  
          return CloneWise_EXPL_MATRIX(mat,i - 1,j - 0,MATCH);   
          }  
        temp = cscore - (0) -  (mat->match->matrix[i][j]);   
        if( temp == CloneWise_EXPL_MATRIX(mat,i - 0,j - 1,MATCH) )   {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CloneWise_EXPL_MATRIX(mat,i-0,j-1,MATCH);  
            }  
          return CloneWise_EXPL_MATRIX(mat,i - 0,j - 1,MATCH);   
          }  
        temp = cscore - ((mat->match->matrix[i][j]+1)) -  (mat->match->matrix[i][j]);    
        if( temp == CloneWise_EXPL_MATRIX(mat,i - 1,j - 1,MATCH) )   {  
          *reti = i - 1; 
          *retj = j - 1; 
          *retstate = MATCH; 
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CloneWise_EXPL_MATRIX(mat,i-1,j-1,MATCH);  
            }  
          return CloneWise_EXPL_MATRIX(mat,i - 1,j - 1,MATCH);   
          }  
        warn("Major problem (!) - in CloneWise read off, position %d,%d state %d no source found!",i,j,state);   
        return (-1); 
      case SKIP_QUERY :  
        ...
        if( (i-1) == 0 && (j-0) == 0 )   {  
          temp = cscore - (mat->query_skip_start) -  (mat->match->skip_iset[i]); 
          if( temp == CloneWise_EXPL_SPECIAL(mat,i - 1,j - 0,START) )    {  
            *reti = i - 1;   
            *retj = j - 0;   
            *retstate = START;   
            *retspecial = TRUE;  
            if( cellscore != NULL)   {  
              *cellscore = cscore - CloneWise_EXPL_SPECIAL(mat,i-1,j-0,START);   
              }  
            return CloneWise_EXPL_MATRIX(mat,i - 1,j - 0,START);     
            }  
          }  
        temp = cscore - (0) -  (mat->match->skip_iset[i]);   
        if( temp == CloneWise_EXPL_MATRIX(mat,i - 1,j - 0,SKIP_QUERY) )  {  
          *reti = i - 1; 
          *retj = j - 0; 
          *retstate = SKIP_QUERY;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CloneWise_EXPL_MATRIX(mat,i-1,j-0,SKIP_QUERY); 
            }  
          return CloneWise_EXPL_MATRIX(mat,i - 1,j - 0,SKIP_QUERY);  
          }  
        temp = cscore - (mat->query_skip_start) -  (mat->match->skip_iset[i]);   
        if( temp == CloneWise_EXPL_MATRIX(mat,i - 1,j - 0,SKIP_TARGET) ) {  
          *reti = i - 1; 
          *retj = j - 0; 
          *retstate = SKIP_TARGET;   
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CloneWise_EXPL_MATRIX(mat,i-1,j-0,SKIP_TARGET);    
            }  
          return CloneWise_EXPL_MATRIX(mat,i - 1,j - 0,SKIP_TARGET);     
          }  
        temp = cscore - (mat->query_skip_start) -  (mat->match->skip_iset[i]);   
        if( temp == CloneWise_EXPL_MATRIX(mat,i - 1,j - 0,MATCH) )   {  
          *reti = i - 1; 
      ...
      case SKIP_TARGET :     
        ...
        if( (i-0) == 0 && (j-1) == 0 )   {  
          temp = cscore - (mat->target_skip_start) -  (mat->match->skip_jset[j]);    
          if( temp == CloneWise_EXPL_SPECIAL(mat,i - 0,j - 1,START) )    {  
            *reti = i - 0;   
            *retj = j - 1;   
            *retstate = START;   
            *retspecial = TRUE;  
            if( cellscore != NULL)   {  
              *cellscore = cscore - CloneWise_EXPL_SPECIAL(mat,i-0,j-1,START);   
              }  
            return CloneWise_EXPL_MATRIX(mat,i - 0,j - 1,START);     
            }  
          }  
        temp = cscore - (0) -  (mat->match->skip_jset[j]);   
        if( temp == CloneWise_EXPL_MATRIX(mat,i - 0,j - 1,SKIP_TARGET) ) {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = SKIP_TARGET;   
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CloneWise_EXPL_MATRIX(mat,i-0,j-1,SKIP_TARGET);    
            }  
          return CloneWise_EXPL_MATRIX(mat,i - 0,j - 1,SKIP_TARGET);     
          }  
        temp = cscore - (mat->target_skip_start) -  (mat->match->skip_jset[j]);  
        if( temp == CloneWise_EXPL_MATRIX(mat,i - 0,j - 1,SKIP_QUERY) )  {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = SKIP_QUERY;    
          *retspecial = FALSE;   
          if( cellscore != NULL) {  
            *cellscore = cscore - CloneWise_EXPL_MATRIX(mat,i-0,j-1,SKIP_QUERY); 
            }  
          return CloneWise_EXPL_MATRIX(mat,i - 0,j - 1,SKIP_QUERY);  
          }  
        temp = cscore - (mat->target_skip_start) -  (mat->match->skip_jset[j]);  
        if( temp == CloneWise_EXPL_MATRIX(mat,i - 0,j - 1,MATCH) )   {  
          *reti = i - 0; 
...
/* Function:  max_calc_special_CloneWise(mat,i,j,state,isspecial,reti,retj,retstate,retspecial,cellscore)
 *
 * Descrip: No Description
 *
 * Arg:               mat [UNKN ] Undocumented argument [CloneWise *]
 ...
 *
 */
int max_calc_special_CloneWise(CloneWise * mat,int i,int j,int state,boolean isspecial,int * reti,int * retj,int * retstate,boolean * retspecial,int * cellscore) 
{
    register int temp;   
    register int cscore; 
    ...
    *reti = (*retj) = (*retstate) = CloneWise_READ_OFF_ERROR;    
    ...
    cscore = CloneWise_EXPL_SPECIAL(mat,i,j,state);  
    switch(state)    { /*switch on special states*/ 
      case START :   
      case TRUSTED_SPECIAL :     
        ...
        for(i= mat->q->length-1;i >= 0 ;i--) { /*for i >= 0*/ 
          ...
          if( temp == CloneWise_EXPL_MATRIX(mat,i - 0,j - 0,SKIP_TARGET) )   {  
            *reti = i - 0;   
            *retj = j - 0;   
            *retstate = SKIP_TARGET; 
            *retspecial = FALSE; 
            if( cellscore != NULL)   {  
              *cellscore = cscore - CloneWise_EXPL_MATRIX(mat,i-0,j-0,SKIP_TARGET);  
              }  
            return CloneWise_EXPL_MATRIX(mat,i - 0,j - 0,SKIP_TARGET) ;  
            }  
          } /* end of for i >= 0 */ 
        /* source SKIP_QUERY is from main matrix */ 
        for(i= mat->q->length-1;i >= 0 ;i--) { /*for i >= 0*/ 
          temp = cscore - (mat->target_special_s) - (0);     
          if( temp == CloneWise_EXPL_MATRIX(mat,i - 0,j - 0,SKIP_QUERY) )    {  
            *reti = i - 0;   
            *retj = j - 0;   
            *retstate = SKIP_QUERY;  
            *retspecial = FALSE; 
            if( cellscore != NULL)   {  
              *cellscore = cscore - CloneWise_EXPL_MATRIX(mat,i-0,j-0,SKIP_QUERY);   
              }  
            return CloneWise_EXPL_MATRIX(mat,i - 0,j - 0,SKIP_QUERY) ;   
            }  
          } /* end of for i >= 0 */ 
        /* source MATCH is from main matrix */ 
        for(i= mat->q->length-1;i >= 0 ;i--) { /*for i >= 0*/ 
          temp = cscore - (mat->target_special_s) - (0);     
          if( temp == CloneWise_EXPL_MATRIX(mat,i - 0,j - 0,MATCH) ) {  
            *reti = i - 0;   
            *retj = j - 0;   
            *retstate = MATCH;   
            *retspecial = FALSE; 
            if( cellscore != NULL)   {  
              *cellscore = cscore - CloneWise_EXPL_MATRIX(mat,i-0,j-0,MATCH);    
              }  
            return CloneWise_EXPL_MATRIX(mat,i - 0,j - 0,MATCH) ;    
            }  
          } /* end of for i >= 0 */ 
        /* source TRUSTED_SPECIAL is a special */ 
        temp = cscore - ((mat->match->skip_jset[j]-1)) - (0);    
        if( temp == CloneWise_EXPL_SPECIAL(mat,i - 0,j - 1,TRUSTED_SPECIAL) )    {  
          *reti = i - 0; 
          *retj = j - 1; 
          *retstate = TRUSTED_SPECIAL;   
          *retspecial = TRUE;    
          if( cellscore != NULL) {  
            *cellscore = cscore - CloneWise_EXPL_SPECIAL(mat,i-0,j-1,TRUSTED_SPECIAL);   
            }  
          return CloneWise_EXPL_MATRIX(mat,i - 0,j - 1,TRUSTED_SPECIAL) ;    
          }  
        /* source START is a special */ 
        temp = cscore - (0) - (0);   
        if( temp == CloneWise_EXPL_SPECIAL(mat,i - 0,j - 1,START) )  {  
