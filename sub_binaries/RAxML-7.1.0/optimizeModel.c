...
static void setRateModel(tree *tr, int model, double rate, int position)
{
  ...
  switch(tr->partitionData[model].dataType)
	...
	assert(position ==  tr->partitionData[0].symmetryVector[position]);*/
      numRates = 6;
      break;
    case SECONDARY_DATA:      
      assert(position >= 0 && position < SECONDARY_RATES);
      numRates = 120;
      break;
    case SECONDARY_DATA_6:
      assert(position >= 0 && position < SECONDARY_RATES_6);
      numRates = 15;
      break; 
    case SECONDARY_DATA_7:            
      assert(position >= 0 && position < SECONDARY_RATES_7);
      numRates = 21;
      break; 
    case BINARY_DATA:      
      assert(0);
      break;
    default:
      /* implicitly assumes BINARY will never get here */
      assert(0);
    }
  ...
  if(tr->partitionData[model].nonGTR)
      ...
      else
	{
	  if(k == tr->partitionData[model].symmetryVector[numRates - 1])
		...
		assert(tr->partitionData[model].substRates[i] == tr->partitionData[model].substRates[tr->partitionData[model].symmetryVector[i]]);
		}
		printf("\n\n");*/
	    }
	}
...
static void optRates(tree *tr, analdef *adef, double modelEpsilon, linkageList *ll, int numberOfModels, int numberOfRates)
{
  int i, k, j, pos;
...
static void optRatesGeneric(tree *tr, analdef *adef, double modelEpsilon, linkageList *ll)
{
  ...
  if(dnaPartitions > 0)
    optRates(tr, adef, modelEpsilon, ll, dnaPartitions, DNA_RATES);
  ...
  /* then SECONDARY */
   ...
   if(secondaryPartitions > 0)
	 ...
	 {
	 case SECONDARY_DATA:
	   optRates(tr, adef, modelEpsilon, ll, secondaryPartitions, SECONDARY_RATES - 1);
	   break;
	 case SECONDARY_DATA_6:
	   optRates(tr, adef, modelEpsilon, ll, secondaryPartitions, SECONDARY_RATES_6 - 1);
	   break;
	 case SECONDARY_DATA_7:
	   optRates(tr, adef, modelEpsilon, ll, secondaryPartitions, SECONDARY_RATES_7 - 1);
	   break; 
	 default:
	   assert(0);
	 }
  ...
  if(AAisGTR(tr))
      ...
      for(i = 0; i < ll->entries; i++)
	{
      ...
      optRates(tr, adef, modelEpsilon, ll, aaPartitions, AA_RATES - 1);
    }
...
double treeLengthRec(nodeptr p, tree *tr, int model)
{  
